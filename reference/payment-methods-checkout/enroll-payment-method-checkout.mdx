...
Dry-run enrollment is not available on this endpoint. Sending `X-Dry-Run: true` to this endpoint, or to the SDK enrollment flow, returns `400 INVALID_PARAMETERS`. Dry-run enrollment is only available on `POST /v1/customers/{customer_id}/payment-methods` with `workflow` set to `DIRECT`. See [Enroll Payment Method](/reference/payment-methods-direct-workflow/enroll-payment-method-api#dry-run-enrollment).
</Note>
