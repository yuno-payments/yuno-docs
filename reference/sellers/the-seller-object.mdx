...
| `phone` | object | The seller's phone. Contains `code` and `number`. |
| `phone.code` | string | Phone country code (e.g., `"55"`). |
...
| `address.line_1` | string | Street name. |
| `address.line_2` | string | Street number. |
...
| `payment_methods` | array | Payment method configurations with provider-specific details. |
...
```json
{
  ...
  "phone": { "code": "55", "number": "11999999999" },
