...
<Note>
  ...
  The checkout session expires 3 hours after its creation. Once expired, requests that reference it return HTTP 400 with an expired-session error code, so create the session close to the moment the shopper opens the checkout.
</Note>
