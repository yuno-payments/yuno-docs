...
<ParamField body="external_subscription" type="object">
  ...
  Present when the session was created or updated with an `external_subscription`; the subscription and customer identifiers from the merchant's billing platform (for example, Stripe Billing). When the session is used to create a payment, Yuno maps this object onto the resulting payment.

  <Expandable title="properties">

    <ParamField body="id" type="string">
      The unique identifier of the subscription in the merchant's billing platform. Required when `external_subscription` is sent: a blank or missing `id` is rejected with `400`.

      Example: sub_1QwZzz
    </ParamField>

    <ParamField body="customer_id" type="string | null">
      The unique identifier of the customer in the merchant's billing platform. Null when the session was created without it.
