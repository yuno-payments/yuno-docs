...
<ResponseField name="filtered" type="boolean">
  Whether your organization has at least one allowlist entry. **Branch on this, never on the length of `providers`** — the list is also non-empty when your only entries are instruction or logo overrides.
</ResponseField>
