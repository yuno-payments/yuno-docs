...
<ResponseField name="filtered" type="boolean">
  Whether your organization has at least one allowlist entry. **Branch on this, never on the length of `providers`** — the list is also empty when your only entries are instruction overrides.
</ResponseField>

<ResponseField name="providers" type="object[]">
  The union of your allowlist and your instruction overrides. A provider can carry custom wording without being offered, in which case it appears here with `allowed: false`.
