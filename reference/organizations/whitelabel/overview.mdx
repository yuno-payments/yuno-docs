...
- `filtered: false` — you have no allowlist entries at all.
- `filtered: true` — you have an allowlist, and `allowed` on each provider says whether that one is on it.

Branch on `filtered`, **never** on the length of `providers`. The list is also empty when your only entries are instruction overrides, and a provider that carries custom wording without being offered appears in it with `allowed: false`.
...
{/* TODO(editorial): the public-api annotation for `GET /v1/whitelabel/providers` currently reads
    `filtered: false` as "the whole catalogue is offered", which contradicts the opt-in rule stated
    by the owning service and by the PUT/DELETE annotations. The wording above follows the opt-in
    rule. Please reconcile with the platform team and, if the picker really does fall back to the
    whole catalogue when `filtered` is false, soften the Warning accordingly. */}
