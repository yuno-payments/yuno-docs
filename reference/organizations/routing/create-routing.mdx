...
```json 400 — Non-contiguous Index
{
  ...
  "messages": ["default_route.steps[].index must be contiguous starting at 1"]
}
```
...
```json 409 — Already Exists
{
  ...
  "messages": ["A routing already exists for account 'acc-uuid' and payment_method 'CARD'"]
}
```
</CodeGroup>
...
| HTTP | `code` |
| --- | --- |
| `400` | `ROUTING_VALIDATION_FAILED`<br />Schema or rule violation. `messages` names the offending field/path. |
| `400` | `ROUTING_PROVIDER_NOT_AVAILABLE`<br />A step references a `(provider_id, connection_id)` not active for the account on this `payment_method`. |
