...
- **`decision_source: INSUFFICIENT_DATA`** — no candidate accumulated enough recent reported events. The full ranking still comes back; candidates with **some** history keep their real (below-minimum) `sample_size`, and the recommendation falls back to the best partial data, or request order as a last resort. Only never-reported candidates report `0`.
