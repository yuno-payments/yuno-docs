...
3. Yuno computes statistics per candidate — approval rate, average provider latency, sample size — over a rolling **30-day** window of your reported payments, segmented by provider, country and payment method — by card BIN when you send one (see [BIN-level statistics](#bin-level-statistics)), and by `CIT`/`MIT` initiation when you send one (see [initiation-level statistics](#initiation-level-statistics)) — and returns the ranking ordered by your objective.
...
<ParamField body="payment" type="object" required>
  ...
  <Expandable title="properties">
    ...
    <ParamField body="initiation" type="enum">
      Optional. `CIT` (cardholder-initiated) or `MIT` (merchant-initiated). When you send it, and the same initiation has enough reported history in the segment, the ranking is scored on that initiation's own statistics (see [initiation-level statistics](#initiation-level-statistics)); otherwise it falls back to the whole segment. Omit it to rank on the whole segment exactly as before.
    </ParamField>
  </Expandable>
</ParamField>
...
## Initiation-level statistics

If you report `initiation` (`CIT` or `MIT`) on your events (in the [Transaction Reporting API](/reference/reporting/report-transactions)), send the same `payment.initiation` here. CIT and MIT approve very differently per provider, so — at each segment level above — the ranking first tries the history with the **same initiation**, and only if that does not reach the minimum sample does it use the whole level. The effective order is: BIN8 + initiation → BIN8 → BIN6 + initiation → BIN6 → country + initiation → country → payment method + initiation → payment method.

Events you reported without an initiation count only in the non-initiation (whole-level) statistics. A request **without** `payment.initiation` is scored on the whole segment, exactly as before this field was used — the response is unchanged. `approval_rate`, `avg_latency_ms` and `sample_size` always reflect the segment actually used.
