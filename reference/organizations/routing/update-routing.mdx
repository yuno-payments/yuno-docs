...
curl -X PATCH 'https://api.y.uno/v1/routing/r_8f2c1d3e-4b5a-6c7d-8e9f-0a1b2c3d4e5f' \
  ...
  -d '{
    ...
    "condition_sets": [
      {
        "sort_number": 1,
        "name": "Brazil — BRL 300 to 600 via Adyen",
        "description": "BR card transactions between BRL 300 and 600 go to Adyen directly",
        "conditions": [
          { "condition_type": "COUNTRY", "conditional": "EQUAL",   "values": ["BR"] },
          { "condition_type": "AMOUNT",  "conditional": "BETWEEN", "values": ["300.00", "600.00"], "currency": "BRL" }
        ],
        "route": {
          "steps": [
...
```json 400 — Immutable Payment Method
{
  ...
  "messages": ["payment_method cannot be changed on an existing routing"]
}
```
</CodeGroup>
...
| `400` | `ROUTING_VALIDATION_FAILED`<br />Schema or rule violation. |
| `400` | `ROUTING_PROVIDER_NOT_AVAILABLE`<br />A step references an inactive/unknown connection. |
...
| `401` | `NOT_AUTHENTICATED`<br />Missing `public-api-key` or `private-secret-key` header. |
| `401` | `INVALID_CREDENTIALS`<br />Wrong key pair, or a Dashboard **Customized API key** was used. Use an account key or organization key instead. |
