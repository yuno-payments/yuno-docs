...
| `payment_triggers` | `AUTHORIZE`, `CANCEL`, `CAPTURE`, `CHARGEBACK`, `PRECHARGEBACK`, `PURCHASE`, `REFUND`, `SPLIT_UPDATE` (pending catalog availability), `VERIFY` |
| `report_triggers` | `CREATE`, `UPDATE` |
| `subscription_triggers` | `ACTIVE`, `CANCEL`, `CLOSE_TO_RENEWAL`, `COMPLETE`, `CREATE`, `PAUSE`, `PLAN_CHANGE_CANCELED`, `PLAN_CHANGE_SCHEDULED`, `PLAN_CHANGED`, `RESUME` |
| `onboarding_triggers` | `BLOCKED`, `CANCELLED`, `CREATED`, `DECLINED`, `ERROR`, `EXPIRED`, `INACTIVE`, `PENDING`, `SUCCEEDED`, `TRANSFERRED`, `UNBLOCKED` |

`SPLIT_UPDATE` subscribes only to successful split updates and qualifying manual resolutions. It is pending production catalog availability; see [Split update notifications](/docs/webhooks#split-update-notifications).
