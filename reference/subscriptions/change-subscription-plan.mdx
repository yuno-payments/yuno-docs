...
<Note>
**`phases`, `current_phase`, and `billing_phases` are all omitted**

With `skip_trial: true` the response omits `phases`, `current_phase`, and `billing_phases`
entirely — all three keys are absent rather than returned as `null`, so test for absence, not
for `null`. The subscription-level signal is `status`, which stays `ACTIVE` instead of becoming
`TRIALING`. A default change (`skip_trial` omitted or `false`) returns all three populated.
</Note>
