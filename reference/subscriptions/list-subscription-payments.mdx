...
There was no payments subresource before Plans — the `payments` array on [the subscription object](/reference/subscriptions/the-subscription-object) is a legacy field that's always empty in webhook payloads and, even in the API response, carries bare payment ids with no plan/phase context. Since there's no prior merchant integration to preserve here, this endpoint ships as one clean `data[]` shape with full plan/phase context — no legacy fields.
</Note>

<Warning>
**`offset` must be a multiple of `limit`**

A non-aligned `offset` would re-serve rows you already saw and corrupt `has_more`, so it's rejected with `400 INVALID_PARAMETERS` instead of being silently floored.
</Warning>
