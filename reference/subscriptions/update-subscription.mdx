...
On a monthly subscription created without an explicit billing day, `RESTART` anchors every future renewal to the day of the restart — restarting on the 20th bills on the 20th from then on, not only for the next cycle. A subscription that declares its billing day keeps it: the date does not move to the restart day, only the time of day is re-anchored. The same rule applies to `RESTART` on [Resume Subscription](/reference/subscriptions/resume-subscription) — it is one rule, on both endpoints.
</Note>
