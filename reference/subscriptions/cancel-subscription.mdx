...
- **`GET /v1/subscriptions/{id}`** carries the *last computed* `refund` too, once one exists —
  canceling with a refund quote persists it, so it keeps showing up on every later read of that
  subscription there.
...
omitted, and no webhook is sent. When a schedule is cleared, `subscription.cancel_schedule_canceled`
fires instead.
...
- `cancel_scheduled` — omitted when no cancellation is pending, otherwise the schedule's `mode`,
...
| HTTP | `code` | When |
|---|---|---|
| `400` | `SUBSCRIPTION_NOT_FOUND` | `subscription_id` doesn't identify a subscription. |
...
| `400` | `BAD_REQUEST` | `refund_amount` is sent without `refund: CUSTOM`, or `refund: CUSTOM` is sent without `refund_amount` and the plan has no configured refund rule for it to fall back on. |
