...
A `CUSTOM` refund takes its amount from one of two places, in this order:

1. `refund_amount` on the cancel request, if you send one.
2. otherwise the `refund_rule` configured on the subscription.

If neither exists the cancellation is rejected with `400` — `CUSTOM` never falls back to a computed
amount. The rule lives on the **subscription**, is set on create or update, and is described on
[the Subscription object](/reference/subscriptions/the-subscription-object).

Whichever source is used, its currency must match both the subscription's currency and the currency
of the charged payment, and the error message names the source it was checking.

<Note>
**Availability**

A configured `refund_rule` is available to organizations enabled for it; contact Yuno. For an
organization that is not yet enabled, `CUSTOM` behaves exactly as it does today: `refund_amount` on
the request is required, and any rule stored on the subscription is ignored.
</Note>
...
- **`GET /v1/subscriptions/{id}`** and **`GET /v1/subscriptions?merchant_reference=...`** carry the
  *last computed* `refund` too, once one exists — canceling with a refund quote persists it, so it
  keeps showing up on every later read of that subscription through either of these.
- **`GET /v1/subscriptions`** (the paginated list) does **not** carry `refund` on any entry, whether
  or not that subscription was canceled with a quote — the list item is a smaller summary shape than
  the full subscription object, and `refund` isn't one of the fields it includes.
...
Under `EXHAUST_RETRIES` the cancellation is applied when the retry ladder resolves, which can be later than the date shown on `cancel_scheduled.at`: the date is the billing boundary the cancellation is due at, and a subscription still in `PAST_DUE` at that boundary keeps retrying until it recovers or the retries run out.
...
## Scheduling a cancellation on a paused subscription

A paused subscription can still be scheduled for cancellation, with one restriction: `schedule.after_cycles` is **not accepted while the subscription is PAUSED**. A pause freezes the billing clock, so counting cycles forward has no boundary to land on — the request is rejected with `400` and the message points at `schedule.at`, which needs no cycle clock and is accepted normally.

A schedule armed while the subscription was active survives a pause. How its date is reported afterwards depends on the mode it was armed with:

- `after_cycles` — `cancel_scheduled.at` is **projected from the subscription's current billing anchor** each time it is read. When a resume moves that anchor, the next read shows the boundary the anchor now implies, so the schedule keeps meaning "after N more cycles" rather than freezing on a date computed before the pause.
- `at` — the date is fixed as given and never moves.
...
| `400` | `BAD_REQUEST` | `refund_amount` is sent without `refund: CUSTOM`. |
| `400` | `BAD_REQUEST` | `refund: CUSTOM` is sent with neither `refund_amount` nor a configured `refund_rule` on the subscription. |
| `400` | `REFUND_RULE_CURRENCY_MISMATCH` | The winning source's currency does not match the subscription's currency or the charged payment's. The message names which source was used. |
