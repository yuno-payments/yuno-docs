...
changing its `status` to `CANCELED`. Billing stops immediately and this cancel cannot be undone,
but the subscription itself can later be brought back to `ACTIVE` with
[Reactivate Subscription](/reference/subscriptions/reactivate-subscription), which keeps the same
`id` and the same history.

You can cancel from `CREATED`, `ACTIVE`, `TRIALING`, `PAST_DUE` and `PAUSED`. `COMPLETED` and
`CANCELED` are terminal for this endpoint, so canceling from either returns `400 INVALID_STATE` —
including a second cancel on a subscription that is already canceled. See
[Subscription Status](/reference/subscriptions/status-subscriptions).
