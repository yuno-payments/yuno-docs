...
If neither exists, the cancellation is rejected with `400` `BAD_REQUEST` —
`"refund: CUSTOM requires refund_amount or a configured refund_rule."` `CUSTOM` never falls back to
a computed amount. `refund_rule` is set on [Create](/reference/subscriptions/create-subscription#setting-a-refund-rule)
or [Update](/reference/subscriptions/update-subscription#setting-a-refund-rule) and described on
[the Subscription object](/reference/subscriptions/the-subscription-object).

```json Cancel G — the request's refund_amount wins over the configured refund_rule
{
  "refund": "CUSTOM",
  "refund_amount": { "value": 5000, "currency": "COP" }
}
// refund.amount.value: 5000 — the configured refund_rule (a FIXED rule for 10000) is not used
```

```json Cancel H — no refund_amount sent, the configured refund_rule wins
{
  "refund": "CUSTOM"
}
// refund.amount.value: 10000 — the subscription's stored FIXED refund_rule
```
...
the request is required, and a `refund_rule` can never be configured for that organization in the
first place.
</Note>

Send `refund_amount` alongside `"refund": "CUSTOM"` to refund a specific amount instead of a
computed or rule-based one:
...
A paused subscription can still be scheduled for cancellation. `schedule.after_cycles` is rejected
while the subscription is `PAUSED`, because a pause freezes the billing clock and there is no cycle
boundary to count from:

```json 400 — after_cycles on a paused subscription
{
  "code": "BAD_REQUEST",
  "messages": [
    "schedule.after_cycles cannot be resolved while the subscription is paused; use schedule.at instead."
  ]
}
```

`schedule.at` has no such restriction — it needs no cycle clock, so it is accepted normally on a
`PAUSED` subscription. See [Pause Subscription](/reference/subscriptions/pause-subscription).
...
| `400` | `BAD_REQUEST` | `after_cycles` is sent while the subscription is `PAUSED` — `"schedule.after_cycles cannot be resolved while the subscription is paused; use schedule.at instead."` |
...
| `400` | `BAD_REQUEST` | `refund: CUSTOM` is sent with neither `refund_amount` nor a configured `refund_rule` on the subscription — `"refund: CUSTOM requires refund_amount or a configured refund_rule."` |
