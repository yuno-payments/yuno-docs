...
3. Each accepted event creates **one payment** with one transaction, with the status derived from `result` and the event type: a `PURCHASE` with `SUCCEEDED` lands `SUCCEEDED`; an `AUTHORIZE` with `SUCCEEDED` lands `PENDING` with sub-status `AUTHORIZED` (a succeeded authorization is not captured money — it mirrors how Yuno-processed authorizations behave); a `VERIFY` with `SUCCEEDED` lands `SUCCEEDED` with sub-status `VERIFIED`; `DECLINED` → `DECLINED`, `ERROR` → `ERROR`.

In this version the supported operations are `PURCHASE` (default), `AUTHORIZE` and `VERIFY`. Zero-amount events are valid (`amount.value >= 0`), so account verifications can be reported. `provider_id` is required and validated against Yuno's provider catalog, `payment_method_type` is validated against Yuno's payment-method catalog, and the optional `token_type_used` records how you charged the instrument (`FPAN`, `DPAN` or `PROVIDER_TOKEN`). Lifecycle events (captures, refunds, chargebacks) are on the roadmap.

## Retries and deduplication

There is **no idempotency header on this endpoint — and none is needed**. The `report_id` you generate per event is the deduplication key: an event whose `report_id` was already ingested is counted in `duplicates` and never ingested twice, no matter when or in which batch it arrives — including twice inside the same batch, where only the first occurrence is ingested even if the payloads differ. Retrying a whole batch after a network failure is always safe.
...
| 401 | `INVALID_AUTHORIZATION_HEADERS` / `INVALID_CREDENTIALS` | Missing or invalid API credentials. |
