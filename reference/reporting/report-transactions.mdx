...
On `CARD` events you can send the card's BIN in `payment_method_details.card.bin` — the first **6 or 8 digits** of the card number, exactly. The BIN is not cardholder data: it identifies the issuer and card range, never the card.

When present, Yuno stores it on the reported payment (`payment_method.detail.card.card_data.iin` on the transaction) and uses it to segment your statistics per BIN in the [Routing Recommendations API](/reference/organizations/routing/get-routing-recommendations), so a payment on the same BIN can be ranked on that BIN's own history instead of the country-wide one.
...
## CIT / MIT initiation for initiation-level routing statistics

You can tell Yuno how each transaction was initiated by sending `initiation` at the event level: `CIT` (cardholder-initiated — the customer was present at checkout) or `MIT` (merchant-initiated, e.g. a subscription renewal). CIT and MIT approve very differently per provider, so splitting your reported history by initiation lets the [Routing Recommendations API](/reference/organizations/routing/get-routing-recommendations) rank a payment on the history of its own initiation type.

- `initiation` is optional and flat at the event level, like `token_type_used`. It accepts only `CIT` or `MIT`; any other value rejects that event with `INVALID_EVENT` and the message `The field 'initiation' must be CIT or MIT.`
- Omitting `initiation` keeps the event exactly as before: it feeds the non-initiation statistics only, and a recommendation request that carries no `payment.initiation` is unaffected.
