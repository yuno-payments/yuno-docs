...
3. Each accepted event creates **one payment** with one transaction, with the status derived from `result` and the event type: a `PURCHASE` with `SUCCEEDED` lands `SUCCEEDED`; an `AUTHORIZE` with `SUCCEEDED` lands `PENDING` with sub-status `AUTHORIZED` (a succeeded authorization is not captured money — it mirrors how Yuno-processed authorizations behave); `DECLINED` → `DECLINED`, `ERROR` → `ERROR`.
