...
In this version the supported operations are `PURCHASE` (default), `AUTHORIZE` and `VERIFY`. Zero-amount events are valid (`amount.value >= 0`), so account verifications can be reported. `provider_id` is required and validated against Yuno's provider catalog, and `payment_method_type` is validated against Yuno's payment-method catalog. Lifecycle events (captures, refunds, chargebacks) are on the roadmap.
