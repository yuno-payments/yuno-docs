...
The notification's status is final at creation and never changes afterward. Yuno sends no webhook for it, so check the status directly in this response. When you reference the notification on a debit, Yuno validates only that it exists and is in `SENT` status. Meeting NPCI's minimum notice period before the debit is your responsibility.
