...
<ParamField body="account_ids" type="Array of strings">
  The additional accounts the plan is shared with, excluding the owner in `account_id`. Empty for a plan that belongs to a single account. Returned only, the field is not accepted when creating a plan.
