...
<ParamField body="account_ids" type="Array of strings">
  The accounts the plan is shared with, including the owner in `account_id`. Contains only the owner's id for a plan that belongs to a single account — never empty. Returned only, the field is not accepted when creating a plan.
...
<ParamField body="description" type="string">
  The plan description (MAX 255; MIN 3 when present).

  Example: Access to Pro features, billed monthly

  <Note>Not returned by [List Plans](/reference/plans/list-plans) — only present on [Create Plan](/reference/plans/create-plan) and [Retrieve Plan](/reference/plans/retrieve-plan) responses.</Note>
</ParamField>

<ParamField body="merchant_reference" type="string">
  Your own identifier for the plan (MAX 218; MIN 3 when present).
...
<ParamField body="frequency" type="object">
  ...
  <Expandable title="properties">
    <ParamField body="type" type="enum">
      Possible enum values: `DAY`, `WEEK`, `MONTH`, `QUARTER`, `HALF_YEAR`, `MINUTES`, or `YEAR`.
    </ParamField>
...
<ParamField body="retries" type="object">
  The default retry policy applied to subscriptions created from this plan (subscriptions may override it). `null` when not set. Same shape as the subscription's `retries` object — see [The Subscription Object](/reference/subscriptions/the-subscription-object) and [Retries](/docs/payment-features/subscriptions/retries).
...
<ParamField body="retry_split" type="object">
  Smart retry split configuration applied to subscriptions created from this plan. `null` when not set.

  <Expandable title="properties">
    <ParamField body="smart_pct" type="int">
      Percentage (0-100) of retries routed through Yuno's smart retry logic.
    </ParamField>
  </Expandable>
...
<ParamField body="meters" type="array of objects">
  Usage-based meters priced on this plan. Empty array when the plan has no metered pricing.
...
<ParamField body="phases" type="array of objects">
  ...
  <Expandable title="properties">
    ...
    <ParamField body="duration" type="object">
      Required on every non-terminal phase; `null` on the terminal phase.

      <Expandable title="properties">
        <ParamField body="type" type="enum">
          `DAY`, `WEEK`, `MONTH`, `QUARTER`, `HALF_YEAR`, `MINUTES`, or `YEAR`.
        </ParamField>
        <ParamField body="value" type="int" />
      </Expandable>
    </ParamField>
