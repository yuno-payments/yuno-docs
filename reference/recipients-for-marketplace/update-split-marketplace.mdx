...
- The provider still allows it: before settlement, and while the provider's limit of split changes per transaction is not reached. Both conditions belong to the provider; when one fails you get a `409` with `SPLIT_UPDATE_WINDOW_CLOSED` or `SPLIT_UPDATE_LIMIT_REACHED`, and the split is not changed.
- The payment has no refund (partial or total), no chargeback and no transfer reversal. Otherwise you get a `409` with `SPLIT_UPDATE_NOT_ALLOWED`.
...
| 409 | `SPLIT_UPDATE_NOT_ALLOWED` | The transaction is not a succeeded purchase, the provider is not enabled, the payment has a refund, chargeback or transfer reversal, or a previous update is under review. |
