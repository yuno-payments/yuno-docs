...
Yuno has added new fields in the Public API to clearly communicate why a payment was declined and what to do next. Use them to power routing rules, defer or avoid retries when advised, and measure declines with precision.

* **`transactions.merchant_advice_code`**

  * Yuno‑standardized guidance for retry behavior across networks.
  * Common values include `TRY_AGAIN_LATER`, `DO_NOT_TRY_AGAIN`, `INVALID_DATA`, and defer windows such as `RETRY_AFTER_24_H`.

* **`transactions.merchant_advice_code_message`**

  * A short, human‑readable explanation paired with the normalized advice.
  * Example: “Retry within 30 days”.

* **`transactions.provider_data.merchant_advice_code`**

  * The original MAC (or equivalent advisory code) exactly as sent by the provider/network.
  * Example (Mastercard): `"25"`.

* **`transactions.provider_data.merchant_advice_code_message`**
  * The original provider message that accompanies the advisory code.
  * Example: “Retry after 24 hours”.

Field usage

* **Normalized (use for routing/conditions)**

  * `transactions.merchant_advice_code`
  * `transactions.merchant_advice_code_message`

* **Raw (provider data; use for audits/provider‑specific checks)**
  * `transactions.provider_data.merchant_advice_code`
  * `transactions.provider_data.merchant_advice_code_message`
