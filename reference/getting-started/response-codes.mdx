...
| 422 Unprocessable Entity  | `PRE_DEBIT_NOTIFICATION_NOT_FOUND` | \[For UPI Autopay] The referenced pre-debit notification does not exist or does not belong to your account.                                                                                    |
|                           | `PRE_DEBIT_NOTIFICATION_NOT_SENT`  | \[For UPI Autopay] The referenced pre-debit notification exists but is not in SENT status.                                                                                                      |
|                           | `STORED_CREDENTIAL_NOT_FOUND`      | \[For UPI Autopay] The vaulted\_token has no stored credential for the resolved provider.                                                                                                       |
|                           | `PROVIDER_CONNECTION_REQUIRED`     | \[For UPI Autopay] The origin payment has no usable connection to derive provider routing from.                                                                                                 |
