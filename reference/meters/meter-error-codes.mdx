...
| 404 | `METER_NOT_FOUND` | Retrieve Meter, Update Meter, Create Plan (`meters[]`) | The `meter_id` does not exist or is not linked to your account, or the meter is `INACTIVE` (archived meters cannot be attached). |
...
| The field 'account_id' is required. | Create Meter | Send the `account_id` of the account the meter belongs to. |
...
| The field 'pricing_strategy' has an invalid value. | Attach Meter to Plan, Update Plan Meter, Create Plan | Send one of the three supported strategies: `PER_UNIT`, `PACKAGE`, `TIERED`. |
...
| The field 'limit' must be an integer between 1 and 100 | List Meters | `limit` accepts 1-100, defaults to 20. |
| The field 'offset' must be a multiple of 'limit' | List Meters | `offset` must be a non-negative multiple of the effective limit. |
