...
- **On creation:** pass `meters[]` (same shape as this request body) to [Create Plan](/reference/plans/create-plan). Duplicated `meter_id` entries are rejected with `400 BAD_REQUEST` ("Duplicated meters meter_id `<meter_id>`.").
