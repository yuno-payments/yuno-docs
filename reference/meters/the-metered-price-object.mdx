...
<ParamField body="price_per_credit" type="object">
  ...
  - `currency` must equal the plan's `base_amount` currency, else `400 BAD_REQUEST` ("The price_per_credit currency must match the plan base currency.").
...
<ParamField body="tiers" type="array of objects">
  Only for `pricing_strategy: TIERED`. Ordered, graduated pricing bands, each band billed at its own rate. Staging currently accepts an attach with `tiers` omitted or empty (no bands are billed until tiers are set). Set only at attach time; rejected with `400 PLAN_METER_PRICING_CONFIG_UNSUPPORTED` on Update Plan Meter.
