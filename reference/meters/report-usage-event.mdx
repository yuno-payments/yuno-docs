...
Send one event per request, with a unique `X-Idempotency-Key` header. Yuno resolves the meter from `event_name`, validates the event synchronously and stores it against the `subscription_id`. Usage is aggregated per subscription and billing cycle according to the meter's `aggregation`, and any usage beyond the included `credits` is billed on the subscription's renewal charge.

<Note>
**Subscription status is not currently enforced**

Verified against staging: a usage event sent against a `CANCELED` subscription's `subscription_id` returns `201` and is counted — not `400 SUBSCRIPTION_NOT_ACTIVE`, reproduced twice with fresh `event_id`s. The `SUBSCRIPTION_NOT_ACTIVE` error code below is defined but was not observed to fire for any subscription status in this round. Do not rely on subscription status to stop usage being counted until Yuno confirms the intended gate.
</Note>
