...
Send at least one of `credits`, `pricing_strategy`, `price_per_credit`, `country_prices`. The same currency rules as [Attach Meter to Plan](/reference/meters/attach-meter-to-plan) apply. If the meter has been archived (`INACTIVE`) the metered price is frozen and this call returns `404 PLAN_METER_NOT_FOUND` until the meter is reactivated.
