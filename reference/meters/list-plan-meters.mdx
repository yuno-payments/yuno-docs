...
<Note>
**`limit`/`offset` are not enforced**

This endpoint always returns every metered price attached to the plan; `limit` and `offset` are accepted but ignored. `pagination.limit` in the response always equals `pagination.total`, never the requested `limit`.
</Note>
