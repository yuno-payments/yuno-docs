...
<ParamField body="plan_id" type="string">
  ...
  The subscription's plan when this payment was created, for a payment created with `subscription.plan_id` and for the renewal payments Yuno generates for that subscription. Absent on every other payment. See [Start from a plan](/docs/payment-features/subscriptions/start-with-payment#start-from-a-plan).

  Example: 14898a9d-5a75-433d-ba17-f2ea5b33620c
</ParamField>

<ParamField body="plan_assigned_at" type="string">
  optional

  When the plan was assigned to the subscription, in UTC ([ISO 8601](https://en.wikipedia.org/wiki/ISO_8601)). Present only on payments that carry `plan_id`.

  Example: 2026-09-24T22:53:42.814482Z
</ParamField>
...
<ParamField body="shipping" type="object">
  The shipping selection attached to the payment. Present only when the payment has one — for example, when the customer picks a saved shipping address and delivery method inside a wallet widget ([Fast Checkout shipping](/docs/payment-features/fast-checkout-shipping)). The key is omitted when the payment has no shipping selection. Returned on `GET /payments/{id}` and payment webhooks. On `POST /payments` it is present only for one-time-token flows where the token already carries the selection — in Fast Checkout the selection arrives after creation, via the provider notification.

  <Expandable title="properties">

    <ParamField body="method" type="object">
      The delivery method the customer selected.

      <Expandable title="properties">

        <ParamField body="id" type="string">
          The merchant-defined identifier of the delivery method. Matches one of the `shipping_methods[].id` sent on the checkout session.

          Example: express
        </ParamField>

        <ParamField body="label" type="string">
          The display label of the delivery method.

          Example: Express shipping
        </ParamField>

        <ParamField body="amount" type="object">
          The delivery fee, already included in the payment amount.

          <Expandable title="properties">

            <ParamField body="currency" type="string">
              The delivery fee currency (ISO 4217). Equals the payment currency.
            </ParamField>

            <ParamField body="value" type="number">
              The delivery fee in major units.
            </ParamField>
    ...
    <ParamField body="address" type="object">
      The shipping address the customer selected.
        ...
        <ParamField body="legs" type="array of objects">
          ...
          <Expandable title="properties">
            ...
            <ParamField body="carrier_name" type="string">
              The name of the airline carrier (MAX 100; MIN 1). Optional.
