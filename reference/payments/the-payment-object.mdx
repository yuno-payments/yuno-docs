...
<ParamField body="applied_promotion" type="object">
  The promotion Yuno applied to this payment, present only when Yuno applied a promotion to
  the payment — with a quote, with a `promotion_id`, or automatically. Absent (never `null`)
  when no promotion applied, and the payment looks exactly as it does today. `amount.value` is
  the charged total, and `original_amount − discount_amount = charged_amount`, which equals
  `amount`.

  Promotions is enabled per organization. It's off by default; ask your Technical Account
  Manager or Key Account Manager to enable it. See
  [Promotions](/docs/payment-features/promotions) for the full flow.

  <Expandable title="properties">

    <ParamField body="id" type="string">
      The applied promotion's id.

      Example: b7c1a4f2-6e0d-4a3b-9c55-1f2e7d8a0b31
    </ParamField>

    <ParamField body="name" type="string">
      The applied promotion's name.

      Example: BBVA 10% off - Mother's Day
    </ParamField>

    <ParamField body="code" type="string">
      The promotion's code. Always `null`: promotion codes are not available yet.

      Example: null
    </ParamField>

    <ParamField body="application_id" type="string">
      Yuno's identifier for this use of the promotion on this payment.

      Example: 6f0e9c2a-1b7d-4e3f-8a21-0c5d9e7f4b12
    </ParamField>

    <ParamField body="quote_id" type="string | null">
      The quote the payment was charged with, when it used one. `null` when the promotion
      applied with a `promotion_id` or automatically, with no quote involved.

      Example: qte_0d91f7ac3b2e
    </ParamField>

    <ParamField body="discount" type="object">
      The promotion's discount definition.
      ...
      <Expandable title="properties">
        ...
        <ParamField body="type" type="enum">
          ...
          Possible enum values: `PERCENTAGE`, `FIXED`
        </ParamField>

        <ParamField body="value" type="number">
          The discount value: a percentage (0-100) for `PERCENTAGE`, or a currency amount for
          `FIXED`.
    ...
    <ParamField body="original_amount" type="object">
      The amount before the discount, the one sent on the quote.

      <Expandable title="properties">

        <ParamField body="currency" type="enum">
          The currency of the original amount (ISO 4217, 3 chars).
    ...
    <ParamField body="discount_amount" type="object">
      The amount Yuno discounted, always positive.

      <Expandable title="properties">

        <ParamField body="currency" type="enum">
          The currency of the discount amount (ISO 4217, 3 chars).
    ...
    <ParamField body="charged_amount" type="object">
      The amount charged: `original_amount` minus `discount_amount`. Equals the payment's
      `amount`.

      <Expandable title="properties">

        <ParamField body="currency" type="enum">
          The currency of the charged amount (ISO 4217, 3 chars).
    ...
    <ParamField body="order" type="object">
      ...
      <Expandable title="properties">
        ...
        <ParamField body="discounts" type="array of objects">
          Specifies the order's discounts array of objects: the discounts you sent, in the
          order you sent them. When Yuno applied a promotion, it appends one more entry, last
          in the array, whose `id` is the promotion's id and whose `unit_amount` is the
          discount amount.

          A reference to a promotion is optional: automatic promotions apply with no entry at
          all. To reference one yourself, send it on
          [Create Payment](/reference/payments/create-payment) as an entry with exactly one of
          `quote_id` or `promotion_id`. See [Promotions](/docs/payment-features/promotions) for
          the full flow.

          <Expandable title="properties">

            <ParamField body="id" type="string">
              ID of the discount (MAX 255; MIN 3). For the entry Yuno appends when a promotion
              applied, the promotion's id.

              Example: 10OFF
            </ParamField>

            <ParamField body="name" type="string">
              The name of the discount (MAX 255; MIN 3). For the entry Yuno appends when a
              promotion applied, the promotion's name.

              Example: Offer
            </ParamField>

            <ParamField body="unit_amount" type="float">
              The amount of the discount (multiple of 0.0001). For the entry Yuno appends when
              a promotion applied, the discount amount, always positive.

              Example: 100
            </ParamField>

            <ParamField body="promotion_id" type="string">
              Sent by you to name the promotion to apply; on the entry Yuno appends, it's the
              applied promotion's id.
            ...
            <ParamField body="applied" type="boolean">
              `true` on the entry Yuno appends when a promotion applied.

              Example: true
            </ParamField>

            <ParamField body="application_id" type="string">
              Yuno's identifier for this use of the promotion on this payment. Present only on
              the entry Yuno appends when a promotion applied.
              ...
              <Expandable title="properties">
                ...
                <ParamField body="three_d_secure" type="object">
                  ...
                  <Expandable title="properties">
                    ...
                    <ParamField body="liability_shift" type="boolean">
                      Indicates whether fraud liability shifted from the merchant to the card issuer as a result of the 3DS authentication. `true` means the issuer bears liability for fraud disputes on this transaction; `false` means the merchant retains it. This field can also be `null`, including in some successful-authentication cases; if you rely on it for chargeback handling and see `null` unexpectedly, contact Yuno support.
...
<ParamField body="transactions_history" type="array of objects">
  The list of the transactions that are part of a payment — one entry per attempt, so it depends on how many providers your payment method route tried. This array is opt-in: it is populated only when the payment was created with `response_additional_data.transactions_history: true`; otherwise it is returned as an empty array `[]`. Chargeback events always include the full history.
</ParamField>
