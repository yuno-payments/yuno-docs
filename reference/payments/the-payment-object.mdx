...
<ParamField body="plan_id" type="string">
  ...
  The plan the subscription was created from, when the payment was created with `subscription.plan_id`. Also present on the renewal payments Yuno generates for that subscription. Absent on every other payment. See [Start from a plan](/docs/payment-features/subscriptions/start-with-payment#start-from-a-plan).
