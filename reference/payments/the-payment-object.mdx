...
<ParamField body="metadata" type="array of objects">
  ...
  The key `gateway_options` is reserved for provider-specific options. Its value is a JSON-encoded array of `key` and `value` string pairs and can have up to 8192 characters, instead of the 512-character limit for other values. See `metadata` in [Create payment](/reference/payments/create-payment) for the format and limits.
