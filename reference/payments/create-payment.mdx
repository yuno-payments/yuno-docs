...
<Warning>
**INVALID_TOKEN error with raw card data**

If you include a `checkout.session` alongside raw `card_data` in `payment_method.detail.card.card_data` without specifying `workflow: "DIRECT"` or without using a proper SDK one-time token, the API will return `400 INVALID_TOKEN`. For DIRECT workflow payments with raw card data, omit the `checkout` object or ensure you're using the correct workflow.
</Warning>

<Info>
**Response shape differs from Retrieve Payment**

This endpoint returns the payment object at the top level, and `transactions` is a **single object** (the transaction just created). [Retrieve Payment by ID](/reference/payments/retrieve-payment-by-id-v2) wraps the payment under `payment` and returns `transactions` as an **array** of every transaction. Parse the two responses separately.
</Info>

<Info>
**`checkout` is conditional**

`checkout.session` is required when `workflow` is `SDK_CHECKOUT` or is omitted. It is not required for `DIRECT` or `REDIRECT` payments.
</Info>
