...
`headers` and `body` are sent **base64-encoded**. Yuno base64-decodes on ingest, applies deterministic PAN/CVV/auth-token redaction, and persists only the redacted form. See [Security & redaction](/docs/security-and-compliance/dry-run-redaction).
</Warning>
...
| Limit | Value |
|---|---|
| Events per call | MAX 10 |
| Decoded `headers` per event | MAX 64 KB |
| Decoded `body` per event | MAX 1 MB |
| Combined headers + body per event | MAX 1 MB |
| Total batch size | MAX 5 MB |
| Header count per event | MAX 200 |
| Header value length | MAX 8 KB |
| `X-Idempotency-Key` | MAX 64 chars, 24-hour replay window |
