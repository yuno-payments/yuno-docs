...
<Note>
This endpoint has a companion: `POST /v1/payments` and `POST /v1/customers/{customer_id}/payment-methods` (workflow `DIRECT`) accept an `X-Dry-Run: true` request header that tells Yuno to validate instead of routing. Both legs must share the same `merchant_reference` (or, for an enrollment, the same `X-Orchestrator-Dry-Run-Id`; see [Pairing an enrollment](#pairing-an-enrollment)) so validation can pair them.
</Note>
