...
- `X-Orchestrator-Dry-Run-Id` is a request header you can send as an alternative to `merchant_reference`. It must be 3 to 255 characters. When you send both `X-Orchestrator-Dry-Run-Id` and `merchant_reference`, they must match, otherwise the request is rejected.
...
1. Call the enrollment endpoint (direct or checkout) with the headers `X-Dry-Run: true` and `X-Orchestrator-Dry-Run-Id: <your id>`. Yuno accepts the request, makes no provider enrollment, sends no webhook, and stores it for comparison.
