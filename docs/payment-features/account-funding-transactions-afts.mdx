...
If you operate a staged digital wallet, where the customer first loads a balance and then spends it in a separate transaction, see [Staged Digital Wallet Operators (SDWO)](/docs/payment-features/staged-digital-wallet-operators-sdwo) for the card-network-specific requirements that apply to that flow.
</Note>