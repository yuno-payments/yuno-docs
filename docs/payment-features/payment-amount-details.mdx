...
<Note>
The discounts on this page are ones **you** computed: Yuno passes them on and doesn't change the amount. To have Yuno compute and apply a discount, use [Promotions](/docs/payment-features/promotions): an entry with only a `quote_id`.
</Note>
