...
Each line carries **`unit_amount`** (a discount you computed, with `id` and `name`) **or** **`quote_id`** (a [promotion quote](/docs/payment-features/promotions) Yuno computed), never both. Yuno fills a `quote_id` line on the response and webhooks (`quote_id`, `id`, `name`, `unit_amount` in the order currency); your own `unit_amount` lines are passed through unchanged.

In the following example you can see a request that clarifies that a 500 USD discount amount is part of a 5000 USD final transaction. Merchant-computed discounts are informational: the amount is already included in `amount` and is not added again.
