...
<Note>
`SMART` availability is enabled per account — contact your Yuno representative to opt in. Requests from accounts that are not enabled are rejected with `400`.
</Note>
...
<Note>
`cancel_on_exhausted_retries` can only be set at creation. An [Update Subscription](/reference/subscriptions/update-subscription) request that changes it is rejected with `400`; re-sending the current value is accepted.
</Note>
