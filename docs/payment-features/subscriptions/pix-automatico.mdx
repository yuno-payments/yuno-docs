...
* **Plan with a free trial:** send the regular price as the payment `amount`. The mandate setup is
  ...
  the mandate. The first paid debit comes after the trial.
...
### While the customer hasn't approved
...
* **The customer never approves:** Yuno cancels the subscription 31 days after the payment was
  created. If an approval still arrives after that, Yuno revokes the mandate at the provider.
...
* Pause, resume, reactivate and reschedule are rejected with `400 INVALID_STATE` until the mandate
  is approved.
