...
```json Create a monthly Pix Automático subscription
{
  ...
  "country": "BR",
  "merchant_reference": "streaming-br-monthly",
  "amount": {
    "currency": "BRL",
    "value": 29.90
  },
...
`payment_method.type` is matched case-insensitively by the cadence rule below, so `pix_automatic` and
`PIX_AUTOMATIC` are judged by the same cadence rules — but only an exact-case `PIX_AUTOMATIC` matches
the type stored on an enrolled `vaulted_token`; a lowercase type on a real token is rejected with
`400 INCORRECT_PAYMENT_METHOD_TYPE`.
...
Accepted today: **`WEEK × 1`, `MONTH × 1` and `YEAR × 1`** —
`frequency.type` is one of those three and `frequency.value` is exactly `1`. Daily does not exist for
...
| `QUARTER` or `HALF_YEAR`, any value | `400 INVALID_PARAMETERS` |
...
The `retries` block cannot be changed on a Pix Automático subscription, at creation or on
[Update Subscription](/reference/subscriptions/update-subscription). Sending `retry_on_decline`,
`amount`, `strategy`, `schedule`, `stop_on_hard_decline` or `cancel_on_exhausted_retries` — alone or
together — is rejected the same way:

```json Configuring the Pix ladder
{
  "code": "INVALID_PARAMETERS",
  "messages": [
    "Retries are fixed for Pix Automático: 3 attempts, 24 h apart, within 7 days of the debit date; only retry_on_decline is accepted"
  ]
}
```
