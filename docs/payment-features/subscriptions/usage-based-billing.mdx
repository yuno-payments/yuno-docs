...
<Steps>
  ...
  <Step title="Attach it to a plan">
    ...
    curl -X POST https://api-sandbox.y.uno/v1/subscriptions/plans/00000000-0000-4000-8000-000000000001/meters \
      ...
      -H 'public-api-key: <Your public-api-key>' \
      -H 'private-secret-key: <Your private-secret-key>' \
      -H 'X-Account-Code: <Your account ID>' \
  ...
  <Step title="Report usage as it happens">
    Send one event per unit of work. `event_id` is your idempotency key; `value` is required for `SUM` and `LAST` meters and defaults to 1 for `COUNT`. The intended rule is that the subscription must be `ACTIVE`, `TRIALING` or `PAST_DUE` — but that check is not currently enforced, so don't rely on the API to reject usage reported against a stopped subscription (see the note under [Idempotency and timestamps](#idempotency-and-timestamps)).
...
<Tabs>
  ...
  <Tab title="COUNT — per call">
    Counts events; each event is recorded as `1`. **Do not send `value`** — an explicit `value` on a `COUNT` meter is rejected with `400 BAD_REQUEST` ("The value must not be provided for a COUNT meter.").
...
For non-`PER_UNIT` strategies, the meter must be priced in every currency the plan supports — a plan with `country_prices` in currencies the metered price doesn't cover is rejected (`PLAN_METER_MISSING_CURRENCY`). `PLAN_METER_BASE_CURRENCY_MISMATCH` only checks the metered price's primary price or rate (`price_per_credit`, `package_price`, or a tier's `rate`) against the plan's base currency — as tested, it does not police extra `country_prices`/`package_country_prices`/`country_rates` entries priced in a currency the plan itself never lists; those are currently accepted as-is. All computed usage amounts (any strategy) round half-up to 4 decimal places.

<Warning>
**Pricing config is set once, at attach time.** `credits`, `price_per_credit` and `country_prices` can be updated on a live `PER_UNIT` metered price with [Update Plan Meter](/reference/meters/update-plan-meter) — the new values apply to usage rated from the next billing cycle onward. `package_size`, `package_price`, `package_country_prices` and `tiers` cannot be changed after attach — any [Update Plan Meter](/reference/meters/update-plan-meter) request containing one of them is rejected with `400 PLAN_METER_PRICING_CONFIG_UNSUPPORTED`, regardless of the metered price's current `pricing_strategy`; there is no supported way to switch `pricing_strategy` in place. There is no detach endpoint, so to correct a `PACKAGE`/`TIERED` config you cannot re-attach the same meter — create a new meter with a new `event_name`, attach it with the corrected config, point your integration at the new `event_name`, and archive the old meter. The plan's flat price stays [immutable](/docs/payment-features/subscriptions/plans) as before.
</Warning>
...
- **Subscription-status gating is not currently enforced.** The intended rule is that only `ACTIVE`, `TRIALING` and `PAST_DUE` subscriptions accept usage (a subscription in payment retries keeps metering) and that events for a `PAUSED`, `CANCELED` or `COMPLETED` subscription are rejected with `400 SUBSCRIPTION_NOT_ACTIVE`. As tested, events are currently accepted (`201`) and counted regardless of subscription status, including `PAUSED` and `CANCELED`. Don't rely on this check to stop usage from being billed after a subscription stops — gate reporting on your side until the restriction is confirmed live.
...
| Subscription is `PAUSED`, `CANCELED`, `COMPLETED` or otherwise not active | `400 SUBSCRIPTION_NOT_ACTIVE` — "The subscription is not active" (documented behavior; as tested, `PAUSED` and `CANCELED` subscriptions currently return `201` instead — see the note above) |
| Usage-based billing not enabled for your organization | `403 PRODUCT_NOT_ENABLED` |
| `value` missing on a `SUM`/`LAST` meter | `400 BAD_REQUEST` — "The value is required for a SUM meter" / "…for a LAST meter" |
...
| `price_per_credit` missing on a non-USD plan | `400 BAD_REQUEST` — "A price_per_credit is required when the plan base currency is not USD." |
| `price_per_credit` currency doesn't match the plan's base currency | `400 BAD_REQUEST` — "The price_per_credit currency must match the plan base currency." |
...
<AccordionGroup>
  ...
  <Accordion title="What happens if I report usage for a paused or canceled subscription?">
    The intended behavior is a `400 SUBSCRIPTION_NOT_ACTIVE` rejection — only `ACTIVE`, `TRIALING` and `PAST_DUE` subscriptions are meant to accept usage. As tested, that check is not currently enforced: the event is accepted and counted even for a `PAUSED` or `CANCELED` subscription. Don't depend on this — resume the subscription first ([Resume Subscription](/reference/subscriptions/resume-subscription)) before reporting usage for a paused one, since the rejection may start being enforced without notice; you can backdate `occurred_at` up to 35 days.
  </Accordion>
