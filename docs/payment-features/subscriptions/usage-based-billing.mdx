...
Call [Retrieve Subscription Usage](/reference/subscriptions/retrieve-subscription-usage) to read what a subscription has accrued in its current billing period before the renewal prices it — one item per meter, with the billable quantity, unit price and resulting amount, plus a period total. A subscription whose plan has no meters returns an empty `items` array and a zero total. Usage reporting is asynchronous, so a read taken moments after reporting an event can still lag behind it.
