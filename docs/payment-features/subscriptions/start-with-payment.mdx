...
With inline terms, only cards can be used. See [Subscriptions](/docs/payment-features/subscriptions/index). To sign up a Pix Automático customer with the first payment, send a `plan_id` instead — see [Start from a plan](#start-from-a-plan).
</Note>
...
The payment's `amount` must equal the plan's price for the payment's `country`, in the same currency. That is the plan's `country_prices` entry for that country, or its `base_amount` when the plan has none. On a free-trial plan you still send the regular price; the trial decides what is charged first.
...
| :--------------------- | :------- | :---------- |
| `plan_id`              | Yes      | UUID of an active plan in your account. |
...
| `billing_cycles.total` | No       | Same as with inline terms. |
...
| `retries`              | No       | Retry settings. Same object as on [Create Subscription](/reference/subscriptions/create-subscription). On Pix Automático only `retry_on_decline` is accepted — see [The ladder is not configurable](/docs/payment-features/subscriptions/pix-automatico#the-ladder-is-not-configurable). |

</div>

`id`, `amount`, `frequency.type`, `frequency.value`, `billing_date`, `trial`, `trial_period` and `phases` can't be sent with `plan_id`. The plan already defines them, so each one is rejected with `400 INVALID_REQUEST`, even when its value is `null`:
...
The same three fields are on [Retrieve Payment](/reference/payments/retrieve-payment-by-id) and on payment webhooks. On a payment made without a plan, `plan_id` and `plan_assigned_at` are absent, not `null`.

### Errors from a plan

All of these fail with `400` before the provider is called. No money moves and no subscription is created; the payment is stored with status `ERROR`.
...
| The plan is canceled | `INVALID_STATE` |
...
| A trial plan with a payment method other than Pix Automático | `INVALID_PARAMETERS` |
