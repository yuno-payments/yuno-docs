...
| `frequency.value`      | number | Yes (checked after the charge) | How many `type` units between charges. `{"type": "MONTH", "value": 1}` bills monthly, `"value": 3` bills quarterly. Always send it — omitting it isn't caught up front, it fails later when the subscription is created. Whole numbers only — a fractional value is not rejected up front and fails after the charge. |
...
<Note>
Illustrative — the values are placeholders, but the field names and nesting are the ones this route actually adds: a top-level `subscription_code` plus a nested `subscription` object carrying `billing_cycle` (which cycle this payment settled) and `retry` (which retry attempt this was, `null` on the first try).
</Note>
