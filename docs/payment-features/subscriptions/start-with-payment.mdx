...
* **[Payment links](/reference/payment-links/create-payment-link)**: the `subscription` block IS recognized on payment-link create — it is not ignored. It requires `vault_on_success: true`, but that requirement is only surfaced once `one_time_use: true` is also set: with `one_time_use` present and `vault_on_success` omitted the call 400s (`INVALID_PARAMETERS`, `"vault_on_success must be set true when a subscription is requested."`), while omitting both is accepted at create time. Send `vault_on_success: true` explicitly rather than relying on the error to catch it.
...
| :--------------------- | :------ | :--------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------- |
| `frequency.type`       | string  | Yes                          | Billing cadence unit. Accepts `WEEK`, `MONTH`, `QUARTER`, `HALF_YEAR` or `YEAR`. |
| `frequency.value`      | number | Yes                          | How many `type` units between charges. `{"type": "MONTH", "value": 1}` bills monthly, `"value": 3` bills quarterly. Whole numbers only — a fractional value is rejected before any charge (`400`, `"<value> is invalid for subscription_external.frequency.value field"`). Always send it: omitting it entirely is **not currently validated** — the subscription is created with `frequency.value: 0` and a zero-length billing period, with no error anywhere. |
...
| `billing_cycles.total` | number  | No                           | Documented to cap the subscription after this many cycles, but on this route staging silently drops it regardless of the value sent — the charge and subscription both succeed, and `GET` on the resulting subscription always shows `billing_cycles.total: null`, the same as if you had never sent it. No cap is ever enforced here. (The subscriptions-surface [Create Subscription](/reference/subscriptions/create-subscription) does persist this field correctly — the gap is specific to this payments-surface block.) Validate the cap on your own side, or create the subscription via Create Subscription instead if you need it enforced. |
...
<Warning>
**`customer_payer.id` is required for this flow**

A subscription always belongs to a customer, so `customer_payer` must be present when you send a `subscription` block — and on staging it must carry an existing customer's `id`. Sending the customer inline instead (`email`, `merchant_customer_id`, `document`, no `id`) is accepted on a plain Create Payment call but is rejected the moment a `subscription` block is attached: `400 MISSING_PARAMETERS`, `"Missing parameters: customer.code."`. Create the customer first with [Create Customer](/reference/customers/create-customer) and send its `id`.
</Warning>
...
<Note>
Illustrative — the values are placeholders, but `subscription_code` is the one field this route adds to the payment response. The nested `subscription` object (`billing_cycle`, `retry`) is not part of the Create Payment response — it appears when you retrieve the payment afterward; see [Retrieve Payment](/reference/payments/retrieve-payment-by-id).
</Note>
...
* **From the payment**: `subscription_code` on the Create Payment response is the subscription's `id`. [Retrieve Payment](/reference/payments/retrieve-payment-by-id) carries the same link under a different name, `subscription_id` — not `subscription_code` — alongside the nested `subscription` object (`billing_cycle`, `retry`).
...
The same `subscription` block has a second shape: send `id` with the UUID of a subscription that already exists, and the payment is meant to attach to that subscription instead of creating a new one. In that shape you omit `frequency`, `availability`, and `amount` — the subscription already defines them.

<Warning>
**This shape currently rejects every subscription we tested**

Sending `subscription.id` for an existing subscription fails with `400 INVALID_REQUEST` — `"The execution type defined for this subscription is not compatible with the current flow."` — regardless of how that subscription was created: a subscription raw-created with the default `execution`, one raw-created with `execution: MERCHANT`, and one created by this very route (`execution: YUNO`, see [How the one call works](#how-the-one-call-works)) all 400 the same way. No `execution` value has been found that this shape accepts.

Treat the JSON below as the documented request shape, not a working example — do not build on it until it has been verified end to end. To attach a payment to an existing subscription today, take the payment on the payments surface without a `subscription` block, then reconcile it against the subscription with [List Subscription Payments](/reference/subscriptions/list-subscription-payments).
</Warning>

```json Create Payment for an existing subscription — currently rejected, see warning above
{
  "account_id": "493e9374-510a-4201-9e09-de669d75f256",
...
* `frequency.type` set to anything other than `WEEK`, `MONTH`, `QUARTER`, `HALF_YEAR`, or `YEAR`.
...
* `subscription.id` for an existing subscription — `INVALID_REQUEST`, `"The execution type defined for this subscription is not compatible with the current flow."` See [Charging an existing subscription](#charging-an-existing-subscription): this has been observed on every subscription tested, regardless of `execution`.
* `availability.start_at` in the past — `"The availability start date must be after the current date."`
* `availability.finish_at` at or before `start_at` — `"The availability start date must be before the end date."`
* `frequency.value` set to a fractional number — `"1.5 is invalid for subscription_external.frequency.value field"`.

### Known validation gaps

These are not caught at all, before or after the charge — no `400`, no error surfaced anywhere. The charge succeeds and a broken or unbounded subscription is created silently:

* `frequency.value` missing entirely — the subscription is created with `frequency.value: 0`, so `current_period_start` equals `current_period_end`: a zero-length billing period.
* `billing_cycles.total` — accepted for any value (valid or not) but never persisted on this route; the subscription is created normally with `billing_cycles.total: null` and no cap enforced.

Validate these on your side before sending the call; do not rely on the API to reject them.

<Warning>
**If a payment succeeds but `subscription_code` comes back `null`**

No currently-tested input on this route fails *after* the charge — every case above either 400s before any money moves, or (the two gaps just listed) is silently accepted. But the mechanism still exists on the API, so if you ever do see a terminal, successful payment with `subscription_code: null`, treat it as a signal to reconcile: either refund the payment, or create the subscription separately with [Create Subscription](/reference/subscriptions/create-subscription). Do not retry the original call — see [Retrying the call](#retrying-the-call).

If the payment comes back `PENDING`, the subscription has not been created **yet** and `subscription_code` is `null` because the outcome is still open — wait for the terminal status before doing anything.
</Warning>
