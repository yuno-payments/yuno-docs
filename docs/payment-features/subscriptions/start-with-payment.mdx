...
See [Subscriptions](/docs/payment-features/subscriptions/index) for the payment methods subscriptions support. To sign up a Pix Automático customer from a plan with the first payment, see [Start from a plan](#start-from-a-plan).
</Note>

## The `subscription` block

Send `subscription` alongside the normal Create Payment fields. The block has three shapes, and you send exactly one of them:

* **New subscription with inline terms:** send `frequency`, `availability`, and `amount`, and leave `id` and `plan_id` out. The table below describes this shape.
* **New subscription from a plan:** send `plan_id`. See [Start from a plan](#start-from-a-plan).
* **Existing subscription:** send `id`. See [Charging an existing subscription](#charging-an-existing-subscription).
...
`frequency.execution` tells Yuno who is responsible for generating each recurring charge. With inline terms it is required — there is no default. With `plan_id` it defaults to `YUNO`.
...
With inline terms, the `subscription` block on Create Payment reads no `retries` object, so a subscription created this way starts with `retry_on_decline: false` — a declined renewal is not retried, on either `execution` value. With `plan_id`, see [Start from a plan](#start-from-a-plan). Turn [Smart Retries](/docs/payment-features/subscriptions/retries) on afterwards with [Update Subscription](/reference/subscriptions/update-subscription).
</Note>
...
Send `subscription.plan_id` with the `id` of a [plan](/docs/payment-features/subscriptions/plans), and the subscription takes its price, cadence and trial from the plan. You don't send `amount` or a cadence. This route is built for [Pix Automático](/docs/payment-features/subscriptions/pix-automatico#signing-up-from-a-plan-with-the-first-payment): the payment is the mandate setup, and the plan decides what the customer approves.

With `plan_id`, the subscription is created **before** the charge, once the plan checks pass. The first response already carries `subscription_code`, even while the payment is still pending. So "no subscription unless the charge completes" does not apply here; see [If the first payment fails](#if-the-first-payment-fails).

```json Create Payment from a plan
{
  "account_id": "493e9374-510a-4201-9e09-de669d75f256",
  "merchant_order_id": "order-90220",
  "description": "Streaming BR — monthly",
  "country": "BR",
  "workflow": "DIRECT",
  "amount": {
    "currency": "BRL",
  ...
  "subscription": {
    "plan_id": "14898a9d-5a75-433d-ba17-f2ea5b33620c"
  }
}
```

The payment's `amount` must equal the plan's price for the payment's `country`, in the same currency. Yuno resolves that price in this order: the regular phase's price for the country, then the plan's `country_prices` entry for the country, then the plan's `base_amount`. On a free-trial plan you still send the regular price; the trial decides what is charged first.

**What you can send with `plan_id`:**

<div className="nowrap-col1-table dense-table">

| Field                  | Required | Description |
| :--------------------- | :------- | :---------- |
| `plan_id`              | Yes      | UUID (v4) of an active plan in your account. |
| `frequency.execution`  | No       | `YUNO` or `MERCHANT`. Defaults to `YUNO`. Only `execution` can be sent inside `frequency`: the plan owns `type` and `value`. |
| `availability`         | No       | Same as with inline terms. |
| `billing_cycles.total` | No       | Total number of cycles to bill. Must be `1` or greater; a lower value is rejected up front with `400 INVALID_REQUEST`. |
| `metadata`             | No       | Metadata stored on the subscription. Same format as on [Create Subscription](/reference/subscriptions/create-subscription). |
| `retries`              | No       | Retry settings. Same object as on [Create Subscription](/reference/subscriptions/create-subscription). When omitted, the subscription takes the plan's retry configuration; on a Pix Automático subscription that Yuno executes, for organizations with Pix Automático scheduling enabled, the [Pix retry ladder](/docs/payment-features/subscriptions/pix-automatico#the-retry-ladder) applies (`retry_on_decline: true`). Sending `retries` for a plan that defines its own retry configuration is rejected with `400 INVALID_PARAMETERS`. On Pix Automático only `retry_on_decline` is accepted. |

</div>

`id`, `amount`, `frequency.type`, `frequency.value`, `billing_date`, `trial`, `trial_period` and `phases` can't be sent with `plan_id`. The plan already defines them, so each one is rejected with `400 INVALID_REQUEST`, even when its value is `null`. The message names the field; for `frequency.type` and `frequency.value` it reads "Only 'subscription.frequency.execution' can be sent with 'subscription.plan_id'":

```json plan_id with a forbidden field
{
  "code": "INVALID_REQUEST",
  "messages": [
    "The field 'subscription.amount' cannot be sent with 'subscription.plan_id'"
  ]
}
```

The response carries three fields for this route:

* `subscription_code`: the `id` of the new subscription.
* `plan_id`: the subscription's plan when this payment was created.
* `plan_assigned_at`: when the plan was assigned to the subscription.

The same three fields are on [Retrieve Payment](/reference/payments/retrieve-payment-by-id) and on payment webhooks. The renewal payments Yuno generates for the subscription carry `plan_id` and `plan_assigned_at` too. On a payment made without a plan, `plan_id` and `plan_assigned_at` are absent, not `null`.

On a free-trial plan the payment shows the regular price in `amount.value`, while its transaction is `0.00`: that is the mandate setup, which collects nothing. A refund of that payment is rejected with `400 INVALID_PARAMETERS`, because nothing was collected.

### If the first payment fails

* **Free-trial plan:** see [Pix Automático](/docs/payment-features/subscriptions/pix-automatico#while-a-free-trial-setup-awaits-approval). A refused setup cancels the subscription automatically.
* **Plan without a trial:** a refused first payment leaves the subscription `CREATED`. It is never billed, but it is not canceled either: cancel it with [Cancel Subscription](/reference/subscriptions/cancel-subscription). An expired first payment is different: the customer's bank can still deliver an approval after the QR code expires, which completes the payment and activates the subscription. Cancel it only if you don't want that.

### Errors from a plan

The request checks fail with `400` before anything is created. The plan checks fail with `400` before the provider is called: no money moves, no subscription is created, and the payment is stored with status `ERROR`.

<div className="nowrap-col1-table dense-table">

| Condition | Code |
| :-------- | :--- |
| `plan_id` is not a valid UUID v4, a forbidden field is sent with it, or `billing_cycles.total` is lower than `1` | `INVALID_REQUEST` |
| The plan does not exist, or belongs to another account | `PLAN_NOT_FOUND` |
| The plan is not active (for example, canceled) | `INVALID_STATE` |
| The payment method is not allowed by the plan | `INCORRECT_PAYMENT_METHOD_TYPE` |
| The payment `amount` or currency differs from the plan's price for the country | `INVALID_PARAMETERS` |
| Pix Automático plan with a priced trial, or a trial on a different cadence than the regular phase | `INVALID_PARAMETERS` |
| A plan with phases (for example, a trial) and a payment method other than Pix Automático | `INVALID_PARAMETERS` |
| `retries` sent for a plan that defines its own retry configuration | `INVALID_PARAMETERS` |

</div>

## Charging an existing subscription

The same `subscription` block has a third shape: send `id` with the UUID of a subscription that already exists, and the payment is attached to that subscription instead of creating a new one. In that shape you omit `frequency`, `availability`, and `amount` — the subscription already defines them.
...
* `frequency`, `availability`, or `amount` missing while creating a new subscription (all three are required unless you send `id` or `plan_id`).
