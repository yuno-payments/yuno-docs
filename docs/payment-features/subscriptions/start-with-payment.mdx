...
As with every Yuno subscription, cards are the main payment method. See [Subscriptions](/docs/payment-features/subscriptions/index). To sign up a Pix Automático customer from a plan with the first payment, see [Start from a plan](#start-from-a-plan).
</Note>
...
* `plan_id`: the plan the payment was created from.
...
* **Plan without a trial:** a refused or expired first payment leaves the subscription `CREATED`. It is never billed, but it is not canceled either. Cancel it with [Cancel Subscription](/reference/subscriptions/cancel-subscription).
