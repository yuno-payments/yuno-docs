...
3. Call [Capture Authorization](/reference/capture-authorization) to take the funds, or [Cancel](/reference/payments/cancel-payment) to release them. Add `delayed_cancel_settings` to the payment so an authorization you never decide on is released automatically.
