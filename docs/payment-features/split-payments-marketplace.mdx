...
| `TERMINATED` | The onboarding was permanently terminated by the marketplace and the account was closed or rejected at the provider. Irreversible through the Yuno API. |

</div>

<Note>
After an onboarding is blocked ([Block Onboarding](/reference/recipients-for-marketplace/block-recipient)) or terminated ([Terminate Onboarding](/reference/recipients-for-marketplace/terminate-onboarding)), the recipient is excluded from split resolution: new split payments referencing them are declined with `RECIPIENT_NOT_FOUND`, and refund or cancel operations that carry splits referencing them fail with `INVALID_PARAMETERS`. Blocking can be reversed with [Unblock Onboarding](/reference/recipients-for-marketplace/unblock-recipient); termination is permanent.
</Note>
