...
### Implementation
 ...
 ```json Example Recipient with Auto Split
 {
...
<table class="code-nowrap-table dense-table">
  <thead>
    <tr>
      <th>
        Field
...
```json Example with Implicit Auto-calculation
{
  "split_marketplace": [
    {
      "recipient_id": "4b31a9b8-4cd2-4e47-93cf-03729241bd68",
...
When an order changes after payment (for example, a different seller delivers it), you can replace the split of the paid transaction with [Update the split of a paid transaction](/reference/recipients-for-marketplace/update-split-marketplace) instead of refunding and charging again. The legs you send replace the whole previous split.

<Note>
  The window to change a split is set by the provider, not by Yuno. With Zoop, the split can be changed until the end of the calendar day of the payment. After the window closes, the update is refused with `409` `SPLIT_UPDATE_WINDOW_CLOSED` and the split is not changed. The update is enabled per provider connection; contact Yuno to enable it for yours.
</Note>
...
* **[Update the split of a paid transaction](/reference/recipients-for-marketplace/update-split-marketplace)**: **PUT**: `https://api-sandbox.y.uno/v1/payments/{payment_id}/transactions/{transaction_id}/split-marketplace`
