...
For Mastercard, Yuno also stores the transaction link ID (TLID) returned by the provider on the customer-initiated payment and forwards it alongside the `network_transaction_id` on later merchant-initiated payments. Adyen supports this today, and Yuno is adding it to other providers as they expose the value. You do not need to send the TLID yourself.

<Note>
  Keep `reason` consistent within a series. Some providers (for example Adyen) decline a merchant-initiated payment whose `reason` differs from the one used on the first payment. The same rule applies when the series moves to another provider.
</Note>
