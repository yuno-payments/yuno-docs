...
For Mastercard, the transaction link ID (TLID) is separate from the `network_transaction_id`, which carries the Mastercard trace ID. Yuno stores the TLID returned by the initial payment for later charges on the same `vaulted_token`. Replay depends on support from the receiving provider; it is not limited to the provider that processed the first payment. Supported flows include Nuvei → Nuvei and Adyen → Nuvei.
