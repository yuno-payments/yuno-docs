...
For Mastercard, the transaction link ID (TLID) is separate from the `network_transaction_id`, which carries the Mastercard trace ID. Yuno stores the TLID returned on a successful payment sent with `usage: FIRST` and reuses it on later `USED` charges on the same `vaulted_token`. Another successful `FIRST` payment can replace the stored value with the TLID returned by that payment. TLID replay depends on support from the receiving provider. Verified replay examples include Adyen → Adyen and Adyen → Nuvei.

Continue sending `usage` and `reason` as usual. You do not need to send the TLID yourself or add a new field to your payment request.
