...
Where `stored_credentials` goes depends on the step. On the first charge the payment method is still the wallet, so the object sits under `detail.wallet`. Once the instrument is vaulted it is a `CARD` record, and every later charge on the `vaulted_token` uses `detail.card`, exactly as for cards:

| Step                                                     | `payment_method.type`                                         | Token field                                           | `stored_credentials` node          | `reason`       | `usage` |
| :------------------------------------------------------- | :------------------------------------------------------------ | :---------------------------------------------------- | :--------------------------------- | :------------- | :------ |
| First charge (wallet sheet shown, instrument vaulted)    | `APPLE_PAY` or `GOOGLE_PAY`                                   | `token` (SDK) or `detail.wallet.payment_token` (direct) | `detail.wallet.stored_credentials` | `SUBSCRIPTION` | `FIRST` |
| Later charges in the same recurring series (MIT)         | `CARD` (the wallet type is also accepted with a `vaulted_token`) | `vaulted_token`                                       | `detail.card.stored_credentials`   | `SUBSCRIPTION` | `USED`  |
| Customer-present one-click purchase on the stored token  | `CARD` (the wallet type is also accepted with a `vaulted_token`) | `vaulted_token`                                       | `detail.card.stored_credentials`   | `CARD_ON_FILE` | `USED`  |

<Warning>
**Do not place `stored_credentials` under `detail.card` on a wallet CIT**

Yuno reads `stored_credentials` for a wallet payment from `detail.wallet`. An object placed under `detail.card` on an Apple Pay or Google Pay first payment is not validated and may be dropped, which leaves the saved payment method without stored-credential flags and without a `network_transaction_id`. Later charges on that record then run as plain purchases instead of a recurring series.
</Warning>

<Note>
**PayPal is the exception**

An instrument vaulted through `PAYPAL_ENROLLMENT` stays a wallet record, so `stored_credentials` (including `subscription_agreement_id`) goes under `detail.wallet` on the later charges as well, not under `detail.card`.
</Note>

Yuno attaches the `network_transaction_id` from the `usage: FIRST` payment automatically, exactly as for cards. Keep `reason` consistent within a series. See the "Critical: Complete All Required Fields" note under [Create a payment with processing type](#create-a-payment-with-processing-type). Full request examples are on the [Apple Pay SDK](/docs/wallets/apple-pay/apple-pay-sdk-integration#customer-initiated-transaction-cit), [Apple Pay direct](/docs/wallets/apple-pay/apple-pay-direct-integration), [Google Pay SDK](/docs/wallets/google-pay/google-pay-sdk-integration#recurring-payments-cit-and-mit) and [Google Pay direct](/docs/wallets/google-pay/google-pay-direct-integration#recurring-payments-cit-and-mit) pages.
...
## Recurring payments across providers

The `network_transaction_id` belongs to the card network, not to the provider that processed the first payment. Yuno stores it on the `vaulted_token` and sends it on every merchant-initiated payment, whichever provider your routing selects. This is what makes a stored card portable: a subscription that started on one provider can be billed on another without asking the customer to authenticate again.

You always send the same fields (`usage`, `reason`, and optionally `network_transaction_id`); Yuno translates them into each provider's own stored-credential format and reference fields. Because of this, you can:

* Route renewals, retries, and fallbacks to the provider with the best approval rate or cost. The `TRANSACTION_TYPE` [routing condition](/reference/organizations/routing/routing-conditions) distinguishes `CIT`, `MIT`, and `RECURRING` payments, so a route can treat first charges and later charges differently.
* Recover failed renewals with [Smart Retries](/docs/payment-features/subscriptions/retries) when Yuno runs the subscription, or with the [Retry Subscription](/reference/subscriptions/retry-subscription) endpoint. If you run billing on your own side, retry the MIT on the vaulted token exactly as you would charge it the first time.
* Move a merchant-initiated series between providers during a migration, because the network reference travels with the token rather than with the connection.

For Mastercard, the provider returns a transaction link ID (TLID) on the customer-initiated payment. This is a separate value from the `network_transaction_id`, which carries the Mastercard trace ID. Yuno stores the TLID and sends it on later merchant-initiated payments that are routed to the same provider that returned it. Adyen supports this today, and Yuno is adding it to other providers as they expose the value. You do not need to send the TLID yourself.

<Note>
  The [`reason` consistency rule](#create-a-payment-with-processing-type) still applies when a series moves to another provider: keep the same `reason` you used on the first payment, whichever provider processes the later ones.
</Note>
