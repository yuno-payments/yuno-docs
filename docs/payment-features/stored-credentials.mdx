...
For Mastercard, Yuno also stores the transaction link ID (TLID). This is a separate value from the `network_transaction_id`, which carries the Mastercard trace ID. The provider returns the TLID on the customer-initiated payment, and Yuno forwards it alongside the `network_transaction_id` on later merchant-initiated payments. Adyen supports this today, and Yuno is adding it to other providers as they expose the value. You do not need to send the TLID yourself.
