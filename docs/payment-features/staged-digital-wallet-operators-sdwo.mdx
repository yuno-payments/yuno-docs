...
A **staged digital wallet** is a wallet where the customer first loads a balance and then spends it, in two separate card transactions. Both transactions are charged by the **wallet operator**, the company that runs the wallet, and not by the store the customer finally buys from. Because of that, the card networks require each transaction to be flagged as **SDWO (Staged Digital Wallet Operator)** traffic. Flagging it correctly is what gives the transaction the right interchange, the right compliance treatment and a clean approval rate.
