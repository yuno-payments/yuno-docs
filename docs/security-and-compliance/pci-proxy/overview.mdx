...
| `{{vaulted_token.<TOKEN>.network_transaction_id}}` | The network transaction ID (NTID) of the original transaction, for merchant-initiated transactions |

The `<TOKEN>` must be a `vaulted_token` that belongs to your organization. A token that does not exist or belongs to another organization is rejected with `EXPRESSION_RESOLUTION_FAILED`, before any card data is read. Network-token fields fail the same way when the card has no network token provisioned or no network transaction ID available. Card and network-token fields can be mixed in the same request.

<Note>
**Card and network token data — not provider tokens, not cryptograms**

These expressions resolve a vaulted token to its stored **card** data (PAN, expiration, holder name) and its **network token** data (DPAN, network token expiration, network transaction ID). Provider tokens are not resolved. Neither are **cryptograms**: a cryptogram is generated per transaction, so if your destination requires one, obtain it separately and include the value directly in the request body.
</Note>
