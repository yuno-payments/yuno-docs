...
| `{{vaulted_token.<TOKEN>.network_token_number}}` | The network token number (DPAN), when the card has a network token |
| `{{vaulted_token.<TOKEN>.network_token_expiration_month}}` | Network token two-digit expiration month (`MM`) |
| `{{vaulted_token.<TOKEN>.network_token_expiration_year}}` | Network token four-digit expiration year (`YYYY`) |
| `{{vaulted_token.<TOKEN>.network_transaction_id}}` | The network transaction ID (NTID): the card scheme's reference for the stored card's original transaction, required by many processors for merchant-initiated transactions. **Not part of the network token** — available independently of it |

The `<TOKEN>` must be a `vaulted_token` that belongs to your organization. A token that does not exist or belongs to another organization is rejected with `EXPRESSION_RESOLUTION_FAILED`, before any card data is read. Each field also fails the same way, independently, when its data is unavailable: the `network_token_*` fields when the card has no network token provisioned, and `network_transaction_id` when the card has no NTID. All fields can be mixed in the same request.

<Note>
**Card data, network token data, and the NTID — not provider tokens, not cryptograms**

These expressions resolve a vaulted token to three independent things it holds: the stored **card** data (PAN, expiration, holder name), the card's **network token** (DPAN and its expiration), and the card's **network transaction ID** (a scheme reference, distinct from the network token). Provider tokens are not resolved. Neither are **cryptograms**: a cryptogram is generated per transaction, so if your destination requires one, obtain it separately and include the value directly in the request body.
</Note>
