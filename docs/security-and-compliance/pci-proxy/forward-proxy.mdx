...
<Steps>
  <Step title="Build the destination request">
    ...
    The card fields you can reference are `number`, `expiration_month`, `expiration_year`, and `holder_name` — they resolve to the stored **card (PAN) data**. If the card has a **network token**, you can also reference `network_token_number` (the DPAN), `network_token_expiration_month`, `network_token_expiration_year`, and `network_transaction_id` (the scheme reference of the original transaction, used for merchant-initiated transactions). Card and network-token fields can be mixed in the same request — send your processor whatever it needs: PAN, DPAN + network transaction ID, or a combination. Provider tokens are not resolved, and neither are cryptograms — a cryptogram is generated per transaction, so if your destination requires one, obtain it separately and include the value directly in the body.
  ...
  <Step title="Handle errors">
    ...
    | `400` | `INVALID_REQUEST` | Missing or invalid `yuno-proxy-destination-url`, an expression or query string in the URL, an invalid `yuno-proxy-timeout`, more than 20 distinct tokens in one request, or an invalid signing setup (unknown `yuno-proxy-auth` value, missing `yuno-proxy-auth-secret-key`, or `DLOCAL_HMAC` without an `X-Login` header) |
    | `400` | `EXPRESSION_RESOLUTION_FAILED` | A `vaulted_token` does not exist, does not belong to your organization, or a referenced field is unavailable (for example, a network-token field on a card with no network token provisioned) |
...
After resolving your expressions, the proxy generates `X-Date` (ISO8601 UTC) and computes `Authorization: V2-HMAC-SHA256, Signature: <signature>` over `X-Login + X-Date + resolved body`, exactly as dLocal requires. What the destination receives:

```
X-Login:       <your value, verbatim>
X-Trans-Key:   <your value, verbatim>
...
An unknown `yuno-proxy-auth` value, a missing `yuno-proxy-auth-secret-key`, or a `DLOCAL_HMAC` request without an `X-Login` header is rejected with `400 INVALID_REQUEST` before the destination is contacted. A `401` from the destination itself (wrong credentials) comes back as a destination response, with `yuno-proxy-destination-status: 401`.
