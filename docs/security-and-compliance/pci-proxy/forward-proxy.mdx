...
<Steps>
  <Step title="Build the destination request">
    ...
    The card fields you can reference are `number`, `expiration_month`, `expiration_year`, and `holder_name` — they resolve to the stored **card (PAN) data**. If the card has a **network token**, you can also reference `network_token_number` (the DPAN), `network_token_expiration_month`, and `network_token_expiration_year`. Separately — it is a scheme reference, not a network-token field — `network_transaction_id` resolves to the card's NTID (the network's reference for its original transaction, required by many processors for merchant-initiated transactions), available independently of whether a network token exists. All fields can be mixed in the same request — send your processor whatever it needs: PAN, DPAN + NTID, or a combination. Provider tokens are not resolved, and neither are cryptograms — a cryptogram is generated per transaction, so if your destination requires one, obtain it separately and include the value directly in the body.
