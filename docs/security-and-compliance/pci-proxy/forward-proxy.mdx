...
Some destinations require every request to be **signed (HMAC) over the request body**. You cannot pre-compute that signature yourself: it would have to be calculated over the **resolved** body — the one containing the real card number — which only exists inside Yuno's secure environment. Instead, register a [signing profile on the destination](/docs/security-and-compliance/pci-proxy/allowlist#destination-authentication-request-signing) and the proxy signs each request for you, after resolving your expressions.

With a **dLocal** (`DLOCAL_HMAC`) profile registered, the proxy injects on every forward to that destination:

| Header | Value |
|---|---|
| `X-Date` | ISO8601 UTC timestamp, generated by the proxy at send time |
| `X-Login`, `X-Trans-Key` | From the destination's signing profile |
| `Authorization` | `V2-HMAC-SHA256, Signature: <signature computed over X-Login + X-Date + the resolved body>` |

Do not send these headers yourself on a signed destination — the proxy's values take precedence. (A signature you computed over a body still containing `{{vaulted_token…}}` placeholders would be rejected by the destination anyway.)
