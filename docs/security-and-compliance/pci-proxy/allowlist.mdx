...
Returns `{"destinations": [ ... ]}`. Without the header below, this returns only your org-wide
destinations (`account_id: null`).
...
The header is additive, not a strict filter: with it, you get that account's destinations
plus your org-wide ones. Without it, you get only the org-wide ones (`account_id: null`). An
account-scoped destination isn't deleted when it's missing from a plain, header-less list, it's
just left out. That's the most common cause of a destination that was registered successfully
but never seems to show up.
</Warning>
