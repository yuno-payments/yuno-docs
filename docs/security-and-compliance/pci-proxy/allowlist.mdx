...
that account only — the host is then usable solely on proxy requests that carry the matching
`yuno-account-id` header (see the [Forward Proxy guide](/docs/security-and-compliance/pci-proxy/forward-proxy)).
...
<Note>
**Where the account scope lives**

`account_id` in the create body is the only place you set an account scope. The management
endpoints (list, delete, enable, disable) are **organization-level** — they do not take a
`yuno-account-id` header. That header applies only to the `/pci-proxy/forward` request, where it
narrows which allowlisted destinations may be used.
</Note>
