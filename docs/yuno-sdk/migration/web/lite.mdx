...
| `renderMode: { type: 'modal' \| 'element', elementSelector }` | *(removed)* | The SDK always presents forms modally on web. If you need a custom layout, use `getPaymentMethodsViews` with a `paymentMethodList` view. |
