...
| `renderMode: { type: 'modal' \| 'element', elementSelector }` | `showView` controller (when embedding) | If you set `renderMode: { type: 'element' }` to embed the form, override [`showView`](/docs/yuno-sdk/controllers/show-view) instead. Default is modal. |
...
- **`elementSelector` removed at session level.** Mounting points are per-view (see [`getPaymentMethodsViews`](/docs/yuno-sdk/features/payment-methods-list)) or via `showView` for embedded forms.
