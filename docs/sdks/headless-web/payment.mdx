...
We recommend using the [Web Seamless SDK](/docs/sdks/full-checkout/web-payments) for a smooth integration experience. This option provides a flexible payment solution with pre-built UI components and customization options.
</Tip>
...
* Create a [One-Time Use Token](/docs/basic-concepts/tokens) using credit card information, then create a payment.
* Create a [One-Time Use Token](/docs/basic-concepts/tokens) using a `vaulted_token` from a previously enrolled credit card, then create a payment.
...
As described on the [3DS Card Verification](/docs/security-and-compliance/3d-secure) page, a payment with 3DS may require an additional challenge to check the customer's identity. If an additional verification step is necessary related to a 3DS verification challenge, the response to the [Create Payment](/reference/payments/create-payment) endpoint will have the following information:
