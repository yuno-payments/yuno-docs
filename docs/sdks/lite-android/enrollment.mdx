...
<GenericOrientation />
Follow this step-by-step guide to implement and enable Yuno's Lite Android SDK enrollment capability in your app.
...
override fun onCreate(savedInstanceState: Bundle?) {
    ...
    initEnrollment { state, message ->
        Log.d("YunoEnrollment", "Enrollment state: $state, error: ${message?.code}")
    }
}
```
...
When the enrollment process completes, the SDK invokes the callback provided in `initEnrollment` with the state and, on `INTERNAL_ERROR`, a [`StatusMessage`](/docs/sdks/resources/references/android#statusmessage-error-details) describing the failure (`null` on every other state).
