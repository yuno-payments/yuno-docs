...
<GenericOrientation />
This page provides a guide to the Yuno Lite SDK for Android payments. This SDK offers a streamlined integration process with essential payment capability, making it ideal for merchants who:
...
For merchants requiring more advanced features like multiple payment methods, custom UI, or advanced fraud prevention, consider using Yuno's [Full SDK](/docs/sdks/full-checkout/android-payments) instead.
...
| `language` | Defines the language (for example, `ES`, `EN`, `PT`). |
...
startCheckout(
  ...
  callbackPaymentState = { state, subState, message ->
    println("Payment State: $state, Substate: $subState, Error: ${message?.code}")
  }
)
```
...
| `REJECT` | Rejected (for example, insufficient funds). |
| `CANCELED_BY_USER` | User canceled the process. |
