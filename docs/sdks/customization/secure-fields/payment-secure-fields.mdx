...
| Parameter         | Description                                                                            |
| ----------------- | -------------------------------------------------------------------------------------- |
| `checkoutSession` | Optional (`generateToken` only — do not pass to `generateTokenWithInformation`): Different checkout session ID for card data persistence after payment errors |
...
<Warning>
Do not pass `checkoutSession` to `generateTokenWithInformation` — set it only on the `secureFields({ ... })` object. See [Payment retry](/docs/sdks/resources/references/web#payment-retry) for details.
</Warning>
...
| `paymentResult` | Callback function that receives payment status updates |
