...
│                  │     │    /icons,/css    → STATIC  │     │                     │
│                  │     │    /flags,/*.png  → ICONS   │     │  - prod.y.uno       │
│                  │     │  /sdk-static-bundles-ms/*   │     │    (fonts)          │
│                  │     │                   → FONTS   │     │                     │
│                  │     │    else           → SDK     │     │                     │
└──────────────────┘     └────────────────────────────┘     └─────────────────────┘
```
...
6. **SDK/asset catch-all** matches every remaining `GET`/`HEAD`. A small dispatcher (`pickSdkUpstream`) picks the right upstream based on path, checked top to bottom so the checkout/3DS/card rules win first:
   ...
   - Font paths (`/sdk-static-bundles-ms/*`) → `SDK_STATIC_BUNDLES_UPSTREAM` (`prod.y.uno`). Since SDK 1.10.9 the SDK loads its font stylesheet from `prod.y.uno/sdk-static-bundles-ms/v1/static/css/…`, and the stylesheet loads its `woff2` files with relative `../fonts/` URLs under the same prefix. Current builds also load the Forter script and the Riskified SRI beacon from this prefix.
...
A production partner gateway needs the same split as this harness. Every path arrives under the partner's base path, prefixed exactly as `apiUrl` / `assetUrl` carry it. Check this list before upgrading the SDK line.
...
| 6 | `/sdk-static-bundles-ms/*` (keep the query string) | `prod.y.uno` (`staging.y.uno` for sandbox and staging Riskified SRI) | **SDK 1.10.9+** (font stylesheet); also the Forter script and the Riskified SRI beacon on current builds |
| 7 | `/icons/*`, `/css/*`, `/brands/*`, `/c2p/*` | `sdk.prod.y.uno` | Always |
| 8 | `/wallets/*` (Samsung Pay Portuguese button), `/fonts/*` (Checkout Builder custom fonts) | `sdk.prod.y.uno` | Current SDK builds |
...
- **Fallback.** Current SDK builds load the fraud scripts, the Samsung Pay Portuguese button and Checkout Builder fonts from Yuno again when the gateway can't serve them, so a missing rule degrades instead of breaking. Keeping every request off `*.y.uno` still needs every rule above.
