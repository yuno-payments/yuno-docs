...
A small Express server that emulates a merchant's own origin (or sub-path) in front of the Yuno stack, so both white-label surfaces can be exercised end-to-end from a **non-Yuno origin** before shipping a partner-hosted integration:
...
The same harness also fronts the **hosted Payment Link checkout**: it serves the `sdk-checkout` SPA routes from the proxy origin, rewrites the app's asset origin back onto itself, and forwards the checkout BFF and status WebSocket upstream — so the hosted checkout can be white-labeled onto a merchant origin/sub-path without any `*.y.uno` request leaking from the page. See [Hosted checkout white-label (Payment Link)](#hosted-checkout-white-label-payment-link).
...
<Note>
  The BFF host is `<env>.y.uno` directly — **no** `api-` prefix (`sandbox.y.uno`, `prod.y.uno`), unlike the SDK's REST surface on `api[-env].y.uno`. Set `CHECKOUT_UPSTREAM` and `CHECKOUT_BFF_UPSTREAM` to the same environment as your checkout session.
</Note>
