...
A small Express server that emulates a merchant's own origin (or sub-path) in front of the Yuno stack. It lets both white-label surfaces be exercised end-to-end from a **non-Yuno origin** before shipping a partner-hosted integration:

- **SDK white-label**: the Yuno Web SDK loaded from a foreign host, verifying the renamed globals, events, and DOM tokens.
- **Hosted checkout white-label (Payment Link)**: the `sdk-checkout` app (normally `checkout.<env>.y.uno`) fronted by the merchant origin/sub-path, so `checkout.y.uno/payment?session=…` can be mapped onto `their.host/<base-path>/payment?session=…`.
...
The same harness also fronts the **hosted Payment Link checkout**: it serves the `sdk-checkout` SPA routes from the proxy origin, rewrites the app's asset origin back onto itself, and forwards the checkout BFF and status WebSocket upstream. This lets the hosted checkout be white-labeled onto a merchant origin/sub-path without any `*.y.uno` request leaking from the page. See [Hosted checkout white-label (Payment Link)](#hosted-checkout-white-label-payment-link).
...
│  (browser)       │     │   (Express :9090)          │     │  - checkout.*.y.uno │
│                  │ ◀───│                            │ ◀───│  - sdk-web.*.y.uno  │
│ <script src=     │     │   Routes by path:          │     │  - sdk-3ds.*.y.uno  │
│  localhost:9090/ │     │  /payment,/enroll → CHECKOUT│     │  - sdk-web-card.*   │
│  v1.7/main.js>   │     │  /checkout-bff/*  → BFF     │     │  - sdk.prod.y.uno   │
│   ─ or ─         │     │    /v1/*         → BACKEND  │     │  - icons.prod.y.uno │
│  /payment?       │     │    /v<n>/pages    → CARD    │     │  - <env>.y.uno (BFF)│
│  session=…       │     │    /challenge     → 3DS     │     │  - api[-env].y.uno  │
│                  │     │  /sdk-static-bundles-ms/*   │     │  - prod.y.uno       │
│                  │     │                → BUNDLES    │     │   (fonts, fraud JS) │
│                  │     │    /icons,/css    → STATIC  │     │                     │
│                  │     │    /flags,/*.png  → ICONS   │     │                     │
│                  │     │    else           → SDK     │     │                     │
└──────────────────┘     └────────────────────────────┘     └─────────────────────┘
...
4. **Checkout BFF pass-through** matches `/checkout-bff/*` and forwards to `CHECKOUT_BFF_UPSTREAM`, sharing the same forwarder as the backend (body re-serialized, hop-by-hop and `access-control-*` and `Cookie` stripped). The `sdk-checkout` app calls its BFF through the white-labeled `apiUrl` (`host/<base-path>/checkout-bff/v1`), so after the base-path strip the proxy sees `/checkout-bff/*` (`GET /checkout-bff/v1/checkout-info/{session}`, `POST /checkout-bff/v1/checkout/payment`). See [Hosted checkout white-label (Payment Link)](#hosted-checkout-white-label-payment-link).
5. **Checkout SPA routes** match `/payment`, `/payment/status`, and `/enroll` (`GET`), registered before the SDK catch-all. Each fetches `CHECKOUT_UPSTREAM/index.html` and rewrites the app's absolute asset origin (`PUBLIC_URL`, e.g. `checkout.<env>.y.uno`) to `BASE_PATH`, so the CRA bundle, favicon, and manifest load back through the proxy instead of leaking to the Yuno host. react-router resolves the actual route client-side from `window.location`.
6. **SDK/asset catch-all** matches every remaining `GET`/`HEAD`. A small dispatcher (`pickSdkUpstream`) picks the right upstream based on path, checked top to bottom, each rule anchored at the start of the path (after the base path), so the checkout/3DS/card rules win first:
   - Checkout app assets: the CRA bundle (`/static/(js|css|media)/*`) and root public files (`/favicon.ico`, `/manifest.json`, `/robots.txt`, `/asset-manifest.json`) → `CHECKOUT_UPSTREAM`. Checked **first** so they don't fall through to the SDK icon upstream.
   ...
   - Static-bundles paths (`/sdk-static-bundles-ms/*`) → `SDK_STATIC_BUNDLES_UPSTREAM` (`prod.y.uno`). Since SDK 1.10.9 the SDK loads its font stylesheet from `prod.y.uno/sdk-static-bundles-ms/v1/static/css/…`, and the stylesheet loads its `woff2` files with relative `../fonts/` URLs under the same prefix. From SDK 1.11.16 the Forter script and the Riskified SRI beacon are also requested from this prefix on the white-label host (earlier releases request them straight from `prod.y.uno`, bypassing the proxy). Checked before the static rule, because its paths also contain `/static/css/`, `/static/js/` and `/fonts/`.
   - Static-asset paths (`/icons/*`, `/css/*`, `/brands/*`, `/c2p/*`, `/wallets/*`, `/fonts/*`) → `SDK_STATIC_UPSTREAM` (`sdk.prod.y.uno`).
   ...
   The static / icons branches exist because the SDK used to load these straight from `icons.prod.y.uno` / `sdk.prod.y.uno`, bypassing the white-label host. Recent SDK builds host-swap them onto the proxy origin (path preserved), so the proxy must forward them back out to the CDNs by path prefix. `sdk.prod.y.uno` and `icons.prod.y.uno` are the same in every environment; `/sdk-static-bundles-ms/*` defaults to `prod.y.uno` (only the Riskified SRI beacon uses `staging.y.uno` outside production).
7. **WebSocket upgrades** are handled on the underlying `http.Server`, not Express. `/checkout-websocket-notification-ms/ws/{payment,enrollment}` → `BACKEND_WS_URL` (the hosted-checkout status stream); everything else follows the same SDK / card / 3DS split as HTTP.
...
## Hosted checkout white-label (Payment Link)

A merchant white-labels Yuno's hosted checkout by mapping `checkout.<env>.y.uno/payment?session=…` onto their own `host/<BASE_PATH>/payment?session=…`. The harness emulates this locally with two upstreams: `CHECKOUT_UPSTREAM` (the `sdk-checkout` app shell) and `CHECKOUT_BFF_UPSTREAM` (the BFF it calls):

- **SPA routes**: `/payment`, `/payment/status`, and `/enroll` each fetch `CHECKOUT_UPSTREAM/index.html`. The served HTML's asset links are absolute (`PUBLIC_URL` = `checkout.<env>.y.uno`), so the proxy rewrites that origin to `BASE_PATH`, keeping the CRA bundle, favicon, and manifest on the proxy origin instead of leaking back to the Yuno host. react-router resolves the actual route client-side from `window.location`.
- **BFF calls**: the checkout app's axios `baseURL` is the white-labeled `apiUrl` (`host/<BASE_PATH>/checkout-bff/v1`). After the base-path strip the proxy sees `/checkout-bff/*` and forwards it to `CHECKOUT_BFF_UPSTREAM` (`GET /checkout-bff/v1/checkout-info/{session}`, `POST /checkout-bff/v1/checkout/payment`).
- **Status WebSocket**: `/checkout-websocket-notification-ms/ws/{payment,enrollment}` is forwarded to `BACKEND_WS_URL`.

<Note>
  The BFF host is `<env>.y.uno` directly, with **no** `api-` prefix (`sandbox.y.uno`, `prod.y.uno`), unlike the SDK's REST surface on `api[-env].y.uno`. Set `CHECKOUT_UPSTREAM` and `CHECKOUT_BFF_UPSTREAM` to the same environment as your checkout session.
</Note>

<Warning>
  The `sdk-checkout` app gates the white-labeled `apiUrl` / `assetUrl` against an allowlist (`REACT_APP_WHITE_LABEL_ALLOWED_HOSTS`). To exercise this through the proxy, `localhost` must be on that allowlist in the target environment; otherwise the app ignores the override and calls Yuno directly.
</Warning>

To try it, open `http://localhost:9090<BASE_PATH>/payment?session=<checkout-session>` with a matching `CHECKOUT_UPSTREAM` / `CHECKOUT_BFF_UPSTREAM` for the session's environment. The checkout renders under the non-Yuno origin and calls the BFF back through the proxy.
...
1. **Template injection.** The proxy serves `pages/index.html` with the literal `__SDK_MAIN_JS__` placeholder replaced by this path. A partner page can write `<script src="__SDK_MAIN_JS__">` and get, for example, `/v1.9.4/main.js`.
...
If steps 1 and 2 both fail, you get the fallback and a warning is logged. Setting `SDK_MAIN_JS` explicitly is the only way to skip the upstream fetch entirely. This is useful when `versions.json` is unreachable, you want a deterministic startup, or the upstream's `latest` is not what you want to test against.
...
The path **must** start with `/v` and contain `/main.js` for normalization to work. If the regular expression does not match, the path is used for template injection but version rewriting is disabled.
...
As long as `v1.7.4` is still published on `SDK_UPSTREAM`, every browser that hits the proxy loads it, with no rebuild required.
...
| :--- | :--- |
| `CHECKOUT_UPSTREAM` | `/payment`, `/payment/status`, `/enroll` (SPA `index.html`), the CRA bundle `/static/(js\|css\|media)/*`, and root public files (`/favicon.ico`, `/manifest.json`, `/robots.txt`, `/asset-manifest.json`) |
| `CHECKOUT_BFF_UPSTREAM` | `/checkout-bff/*` (checkout BFF; host is `<env>.y.uno`, **no** `api-` prefix) |
...
| `SDK_STATIC_UPSTREAM` | `/icons/*`, `/css/*`, `/brands/*`, `/c2p/*`, `/wallets/*`, `/fonts/*` (host-swapped CDN assets) |
| `SDK_ICONS_UPSTREAM` | `/sdk-web/*`, `/flags/*`, bare root brand images (`/Visa.png`, …) |
| `SDK_STATIC_BUNDLES_UPSTREAM` | `/sdk-static-bundles-ms/*` (font stylesheet and font files, Forter script, Riskified SRI beacon) |
...
`SDK_CARD_UPSTREAM` and `SDK_3DS_UPSTREAM` default to `SDK_UPSTREAM` when unset, and `BACKEND_WS_URL` defaults to `BACKEND_URL`. Keep this fallback behaviour, it makes single-env testing easier. `SDK_STATIC_UPSTREAM` / `SDK_ICONS_UPSTREAM` / `SDK_STATIC_BUNDLES_UPSTREAM` are the exception: they default to the fixed `sdk.prod.y.uno` / `icons.prod.y.uno` / `prod.y.uno` hosts, **not** to `SDK_UPSTREAM`. The first two are the same in every environment; set `SDK_STATIC_BUNDLES_UPSTREAM` to `staging.y.uno` to test the Riskified SRI beacon outside production. `CHECKOUT_UPSTREAM` / `CHECKOUT_BFF_UPSTREAM` default to sandbox (`checkout.sandbox.y.uno` / `sandbox.y.uno`); the checkout-asset rule is checked **before** the SDK icon rules so the CRA bundle and root files (`/favicon.ico`, …) don't fall through to `icons.prod.y.uno`.
...
async function proxyToUpstream(req, res, next) {
  const upstreamBase = pickSdkUpstream(req.path)   // checkout / 3DS / card / static-bundles / static / icons / SDK
...
- The WebSocket service has **no** `api-` prefix; it's `<env>.y.uno` directly (`y.uno`, `staging.y.uno`, `dev.y.uno`).
...
Serve `index.html` at `/` and a `/whitelabel-info` JSON endpoint that returns the resolved upstream config. The landing page calls the JSON endpoint and renders it, so anyone debugging the proxy sees exactly what's routed where without grepping logs.
...
- **Static CDN assets are host-swapped too.** Recent SDK builds rewrite their hard-coded `icons.prod.y.uno` / `sdk.prod.y.uno` asset URLs onto the white-label host, so the proxy must forward `/icons`, `/css`, `/brands`, `/c2p`, `/wallets`, `/fonts` → `sdk.prod.y.uno` and `/sdk-web`, `/flags`, bare `/*.png` → `icons.prod.y.uno`. Older SDK builds load these straight from the CDN and never reach the proxy. Exercising this end-to-end needs a recent SDK build **and** a partner page that inits with `{ apiUrl: '<proxy origin>' }` (or `assetUrl`).
- **Fonts need their own route from SDK 1.10.9.** The SDK loads its font stylesheet from `prod.y.uno/sdk-static-bundles-ms/…`, host-swapped onto the white-label host. Without a `/sdk-static-bundles-ms/*` → `prod.y.uno` rule, the request falls through to `SDK_UPSTREAM` and returns `404`. The SDK then renders in the system font, with no error.
...
A production partner gateway needs the same split as this harness (this table is the reference; the lists above describe the harness). Every path arrives under the partner's base path, prefixed exactly as `apiUrl` / `assetUrl` carry it. Check this list before upgrading the SDK line.

**Match the rules in this order; the first match wins.** Several shapes overlap (`/v1/*`, `/v<major.minor>/*`, `/v<card-semver>/pages/*`), so an unordered set of prefix or regex rules sends card pages or API calls to the wrong host.

<div className="dense-table">

| # | Path | Forward to | Needed from |
| :--- | :--- | :--- | :--- |
| 1 | `/v1/*`, `/v2/*` (HTTP and WebSocket) | `api[-<env>].y.uno` | Always |
| 2 | `/checkout-websocket-notification-ms/ws/*` (WebSocket) | `<env>.y.uno` (`prod.y.uno` in production) | Always |
| 3 | `/payment`, `/payment/status`, `/enroll`, `/static/(js\|css\|media)/*`, `/favicon.ico`, `/manifest.json`, `/robots.txt`, `/asset-manifest.json`, `/checkout-bff/*` | `checkout[.<env>].y.uno` / `<env>.y.uno` | Payment Link white-label |
...
| 6 | `/sdk-static-bundles-ms/*` (keep the query string) | `prod.y.uno` (`staging.y.uno` for sandbox and staging Riskified SRI) | **SDK 1.10.9+** (font stylesheet); **SDK 1.11.16+** (Forter script, Riskified SRI beacon) |
| 7 | `/icons/*`, `/css/*`, `/brands/*`, `/c2p/*` | `sdk.prod.y.uno` | SDK 1.9.32+ |
| 8 | `/wallets/*` (Samsung Pay Portuguese button), `/fonts/*` (Checkout Builder custom fonts) | `sdk.prod.y.uno` | **SDK 1.11.16+** |

The **Needed from** column is the SDK release from which the SDK sends that request to the white-label host. The assets themselves are older (the Forter script has lived under `/sdk-static-bundles-ms/` since SDK 1.5.19, the Riskified SRI beacon since 1.6.4, the Samsung Pay Portuguese button under `/wallets/` since 1.10.9), but before the release listed the SDK requested them directly from `*.y.uno`, so a partner proxy never saw them and needed no rule. Adding the rules before upgrading is harmless.
| 9 | `/sdk-web/*`, `/flags/*`, bare brand images (`/Visa.png`, …) | `icons.prod.y.uno` | SDK 1.9.32+ |
| 10 | `/v<major.minor>/*` (SDK bundle and chunks) and anything else | `sdk-web[.<env>].y.uno` | Always |

</div>

- **CORS.** Fonts (`/sdk-static-bundles-ms/*`, `/fonts/*`) and the SRI fraud scripts are fetched in CORS mode from the merchant page, so the gateway must return `Access-Control-Allow-Origin` for the merchant origin. `sdk.prod.y.uno/fonts/*` sends none, so forwarding the response untouched is not enough.
- **Reject `.` and `..` path segments** before forwarding. Otherwise `/sdk-static-bundles-ms/../…` resolves to any path on the upstream host, including the production API gateway.
- **Regions.** With an `apiUrl` override the SDK no longer adds the public API key's region to hosts, so a gateway serving EU or MENA keys must route them to the regional hosts itself (`api.eu.y.uno`, `prod.eu.y.uno`, …).
- **Versioned `assetUrl`** (for example `…/orchestrator/v1.0`). Chunks are requested under that version segment, so the gateway must map it to a published `sdk-web` path (the harness normalizes it to the latest published version).
- **Payment Link.** Rewrite the absolute `checkout.<env>.y.uno` origin in the served `index.html` to your base path, as the harness does, or the bundle loads from the Yuno host.
- **Fallback.** SDK 1.11.16+ loads the fraud scripts, the Samsung Pay Portuguese button and Checkout Builder fonts from Yuno again when the gateway can't serve them, so a missing rule degrades instead of breaking. Keeping every request off `*.y.uno` still needs every rule above.
