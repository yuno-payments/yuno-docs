...
6. **SDK/asset catch-all** matches every remaining `GET`/`HEAD`. A small dispatcher (`pickSdkUpstream`) picks the right upstream based on path, checked top to bottom so the checkout/3DS/card rules win first:
   ...
   - Font paths (`/sdk-static-bundles-ms/*`) → `SDK_STATIC_BUNDLES_UPSTREAM` (`prod.y.uno`). Since SDK 1.10 the SDK loads its font stylesheet from `prod.y.uno/sdk-static-bundles-ms/v1/static/css/…`, and the stylesheet loads its `woff2` files with relative `../fonts/` URLs under the same prefix.
...
| `SDK_STATIC_BUNDLES_UPSTREAM` | `/sdk-static-bundles-ms/*` (font stylesheet and font files, SDK 1.10+) |
...
- **Fonts need their own route from SDK 1.10.** The SDK loads its font stylesheet from `prod.y.uno/sdk-static-bundles-ms/…`, host-swapped onto the white-label host. Without a `/sdk-static-bundles-ms/*` → `prod.y.uno` rule, the request falls through to `SDK_UPSTREAM` and returns `404`. The SDK then renders in the system font, with no error.
...
| `/wallets/*` (Samsung Pay Portuguese button), `/fonts/*` (Checkout Builder custom fonts) | `sdk.prod.y.uno` | Current SDK builds |
| `/sdk-web/*`, `/flags/*`, bare brand images (`/Visa.png`, …) | `icons.prod.y.uno` | Always |
| `/sdk-static-bundles-ms/*` | `prod.y.uno` | **SDK 1.10+** (font stylesheet); also the Forter script and the Riskified SRI beacon on current builds |
