...
- From SDK 1.10, a `404` for `/sdk-static-bundles-ms/…` on your origin (the font stylesheet). Your proxy must forward `/sdk-static-bundles-ms/*` to `prod.y.uno`. See [Routes a partner proxy must forward](/docs/sdks/customization/web/white-label-proxy-server#routes-a-partner-proxy-must-forward).
