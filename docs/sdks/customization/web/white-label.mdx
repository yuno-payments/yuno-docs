...
White-label support lets a partner serve the Yuno Web SDK from their own origin without the string `Yuno` leaking into the merchant page: not in the DOM, globals, dispatched events, or outgoing network traffic.
...
Both the new and legacy globals reference the same object, and both events are dispatched on bundle load, so you can mix them during a migration without breaking either form.
...
- From SDK 1.10.9, a `404` for `/sdk-static-bundles-ms/…` on your origin (the font stylesheet). Your proxy must forward `/sdk-static-bundles-ms/*` to `prod.y.uno`. See [Routes a partner proxy must forward](/docs/sdks/customization/web/white-label-proxy-server#routes-a-partner-proxy-must-forward).
...
<Note>
  The `v1.7` version above and the `v1.9` version in the [Integration example](#integration-example) are both intentional, not a typo: `v1.9` shows a real production SDK version, while `v1.7` is just the local test harness's own hard-coded fallback version (see [Version normalization](/docs/sdks/customization/web/white-label-proxy-server#version-normalization)).
</Note>
...
The same harness also fronts the **hosted Payment Link checkout** (normally `checkout.<env>.y.uno`), so that surface can be white-labeled onto a merchant origin/sub-path too. For the merchant-facing flow (rewriting the redirect URL and appending `apiUrl` / `assetUrl`), see [Payment Link White Label](/docs/sdks/customization/web/payment-link-white-label).

See [White Label Proxy Server](/docs/sdks/customization/web/white-label-proxy-server) for the full architecture, the [hosted checkout white-label](/docs/sdks/customization/web/white-label-proxy-server#hosted-checkout-white-label-payment-link) flow, the `SDK_MAIN_JS` version-pinning behaviour, the env-var matrix (prod / staging / dev upstreams), and the routing table.
