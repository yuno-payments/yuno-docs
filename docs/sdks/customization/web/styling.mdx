...
| Mode | Description |
| :--- | :--- |
| `modal` | Displays the payment flow in an overlay centered on the screen. **Default mode.** |
| `element` | Renders the SDK inside a specific container in your app. Requires `elementSelector`. |

### Example: Embedding as an Element

If you choose `element`, you must provide a selector for the container. This is where the APM form and the Action form (for example, PIX QR code) will be mounted.
...
The `card` configuration object allows you to modify the look and feel of the sensitive card data fields (Card Number, Expiry, CVV). These fields are rendered inside a secure iframe for PCI compliance.

### CSS Injection

Use the `styles` property to inject custom CSS directly into the card form iframe. This lets you match the SDK to your site's aesthetic.
...
You can override the default placeholder and button labels to better fit your app's tone or language requirements.
...
By default, Yuno shows a loading spinner during transitions. You can hide it if you prefer to use your own app's loading state.
...
| `updateCheckoutSession` | Method to inject a new checkout session without unmounting the SDK. Use it to switch payment methods after a failure, or to refresh totals mid-flow. See [updateCheckoutSession](/docs/sdks/resources/references/web#updatecheckoutsession) for details. |
