...
- `AppCompatCheckBox`: A checkbox users can use to choose whether to save the credit card for future purchases.
...
Merchants can turn address autocomplete capability on or off in the Android SDK. When it's on, the SDK automatically fills address fields based on postal code lookup. When it's off, users must manually enter all address information.