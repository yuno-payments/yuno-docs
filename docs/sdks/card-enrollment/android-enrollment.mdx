...
| `callbackEnrollmentState` | Callback `(state, message)`: enrollment state and, on `INTERNAL_ERROR`, a [`StatusMessage?`](/docs/sdks/resources/references/android#statusmessage-error-details). Optional; requires `initEnrollment` in onCreate. Second parameter added in SDK `2.22.0`. |
...
fun ComponentActivity.initEnrollment(
    callbackEnrollmentState: ((String?, StatusMessage?) -> Unit)? = null
)
```
...
| `callbackEnrollmentState` | A function `(state, message)` that returns the current state of the enrollment process and, on `INTERNAL_ERROR`, a [`StatusMessage?`](/docs/sdks/resources/references/android#statusmessage-error-details) describing the failure (declined enrollments carry no message). Optional, defaults to `null`. To register this callback, you must call `initEnrollment` in the `onCreate` method of the activity. Check the possible states that can be returned. |
