...
<Tabs>
  ...
  <Tab title="Yuno (Lite SDK / Secure Fields)">
    ...
    3. The SDK gives you a one-time token in `createPayment`; your server calls [Create Payment](/reference/payments/create-payment) with `payment_method.token` and `checkout.session`.
  ...
  <Tab title="Yuno (Seamless SDK)">
    ...
    4. For 3DS or a redirect, the SDK handles the challenge automatically and returns the result in `paymentResult`.
  </Tab>
</Tabs>
