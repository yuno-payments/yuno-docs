...
* **Seamless payment creation**: the SDK creates the payment from the checkout session on your account — your server does not call Create Payment. The lifecycle callbacks let you react to the result.
...
export function CardForm({ checkoutSession, countryCode }) {
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      const pan = secureFields.create({ name: 'pan', options: { placeholder: 'Card number' } }).render('#yuno-pan');
      const expiration = secureFields.create({ name: 'expiration', options: { placeholder: 'MM/YY' } }).render('#yuno-expiration');
      const cvv = secureFields.create({ name: 'cvv', options: { placeholder: 'CVV' } }).render('#yuno-cvv');

      if (!cancelled) fieldsRef.current = [pan, expiration, cvv];
    }
    ...
    return () => {
      cancelled = true;
      fieldsRef.current.forEach((field) => field.unmountSync());
      fieldsRef.current = [];
    };
  }, [yuno, checkoutSession, countryCode]);
  ...
  async function handleSubmit(event) {
    ...
    const oneTimeToken = await fieldsRef.current[0].generateToken({
...
| The browser refuses to run the script | Subresource Integrity rejected the script | Upgrade `@yuno-payments/sdk-web` to the latest version that matches the published hash, or drop `sri: true` while you isolate the problem. With `loadScript` the hash is hardcoded inside the npm package — there is no external URL to check. |
