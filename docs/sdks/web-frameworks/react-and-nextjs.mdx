...
`loadScript` injects the SDK script and resolves with the initialized instance. Pass the `integrity` hash published in `versions-sri.json` so the browser verifies the script (see [Subresource Integrity](/docs/sdks/resources/references/web#subresource-integrity-sri)).
...
export function useYuno() {
  ...
  useEffect(() => {
    sdkPromise ??= loadScript({
      apiKey: process.env.NEXT_PUBLIC_YUNO_PUBLIC_API_KEY,
      integrity: process.env.NEXT_PUBLIC_YUNO_SDK_INTEGRITY,
    });
    let active = true;
...
| `NEXT_PUBLIC_YUNO_SDK_INTEGRITY` | Browser, the `sha384-...` hash for the pinned SDK version |
...
| The browser refuses to run the script | The `integrity` hash does not match the loaded version | Copy `latest.integrity` from `https://sdk-web.y.uno/versions-sri.json` for the version you pin, or drop `integrity` when you use the global `v1.10` path |
