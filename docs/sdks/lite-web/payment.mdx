...
To present the checkout process based on the selected payment method, use the `yuno.mountCheckoutLite()` function. This step ensures the SDK is properly mounted on your chosen HTML element.
...
Call `yuno.startPayment()` immediately after `yuno.mountCheckoutLite()` to open the selected payment method UI:
