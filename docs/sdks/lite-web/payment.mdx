...
For all APMs, including Google Pay, Apple Pay, and PayPal, `yunoPaymentMethodSelected` is triggered as soon as the customer chooses the payment method (before the payment flow begins). Define `yunoPaymentMethodSelected` in `await yuno.startCheckout()` before `await yuno.mountCheckoutLite()`.
</Note>
