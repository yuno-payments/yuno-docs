...
For a smooth integration experience, we recommend using the [Android Seamless SDK](/docs/sdks/full-checkout/android-payments). This option provides a flexible payment solution with pre-built UI components and customization options.
</Tip>
...
* You need your Yuno API credentials (`account_id`, `public-api-key`, and `private-secret-key`), which you can obtain from the [Developers section of the Yuno dashboard](/docs/using-yuno/settings/developers-credentials). These credentials are required to authenticate requests to the Yuno API. The API is used to:
