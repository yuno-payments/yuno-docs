...
| `callbackPaymentState` | `((String?, String?, StatusMessage?) -> Unit)?` | Invoked on the main thread with the final payment state, sub-status, and an optional `StatusMessage?` once the flow completes. Available from SDK `2.22.0` with three parameters. |
...
Yuno.apiClientPayment(
    ...
    when (paymentState) {
        ...
        "PROCESSING" -> { }
        "REJECT", "FAIL" -> {
            // statusMessage?.reason contains the error explanation
        }
        "CANCELED_BY_USER" -> { }
        "INTERNAL_ERROR" -> { }
        else -> { }
