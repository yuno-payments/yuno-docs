...
| `callbackPaymentState` | `((String?, String?, StatusMessage?) -> Unit)?` | Invoked on the main thread with the final payment state, its sub-status and, on error outcomes, a [`StatusMessage?`](/docs/sdks/resources/references/android#statusmessage-error-details). Third parameter added in SDK `2.22.0`. |
...
) { paymentState, paymentSubState, message ->
    when (paymentState) {
        "SUCCEEDED" -> { }
        "PROCESSING" -> { }
        "REJECT", "FAIL" -> {
            // message is present when the backend returned a response_code
            val declineCode = message?.code
        }
        "CANCELED_BY_USER" -> { }
        "INTERNAL_ERROR" -> {
            // message is always present: source "sdk" or "backend", code, reason, context
            val failure = message
        }
        else -> { }
    }
