...
We recommend using the [Android Seamless SDK](/docs/sdks/full-checkout/android-payments) for a smooth integration experience... Wait, actually, let me adjust the copy slightly based on the original.
</Tip>
...
You can obtain these credentials from the [Developers section of the Yuno dashboard](/docs/using-yuno/settings/developers-credentials). They are required to:
...
1. Create a customer using the [Create Customer](/reference/customers/create-customer) endpoint. The response will include the customer `id`.

2. Create a customer session using:
   * The customer `id` from step 1
   * The [Create Customer Session](/reference/customer-sessions-enrollment/create-customer-session) endpoint
