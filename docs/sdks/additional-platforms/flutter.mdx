...
YunoPaymentListener(
  listener: (context, state) {
    ...
    final substatus = state.substatus;  // e.g. CLOSED_BY_USER (nullable)
...
A finer-grained status. On iOS it maps to `Yuno.Result.substatus`; on Android to the native `PaymentSubStates` value (for example `CLOSED_BY_USER`). It is `null` when the native layer does not provide one, including Android enrollment.
