...
Create a customer using the [Create customer endpoint](/reference/customers/create-customer). This customer is required for checkout sessions and saved cards.

Create a new `checkout_session` using the [Create checkout session](/reference/checkout-sessions/create-checkout-session) endpoint. Keep the returned `checkout_session` ID for use in Step 4.
...
Use the one-time token from Step 5 to create the payment in your backend. Call the [Create payment endpoint](/reference/payments/create-payment) from your server with:
...
Create a customer and a `customer_session` using the [Create customer session endpoint](/reference/customer-sessions-enrollment/create-customer-session). Keep the returned `customer_session` ID for use in Step 4.
...
For Android deep link configuration, see [Android deep linking documentation](/docs/sdks/full-checkout/android-payments#step-3-create-the-checkout-session).
