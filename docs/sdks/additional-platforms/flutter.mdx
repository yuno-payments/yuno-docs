...
<Note>
`state.message` is only populated when the transaction is **declined** or **rejected**, or when the status is an **internal error**. For any other outcome (such as a successful or processing payment) it is `null`.
</Note>

Both `message` and `substatus` can be `null`, so always null-check before use.
