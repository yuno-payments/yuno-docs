...
description: "Track features, improvements, and fixes across Android SDK versions."
deprecated: false
hidden: false
metadata:
  robots: index
...
|           | **NEW**: Display `payment_parent_method_type` in OTT component                                                                                                             |
|           | **NEW**: Add `payment_parent_method_type` to OTT response                                                                                                                  |
|           | **NEW**: Add dynamic CVV max length by card type from API                                                                                                                  |
|           | **NEW**: Map brands and `default_network` from card-info MS                                                                                                                |
|           | **NEW**: Add sub-platform header and refine SDK type tracking                                                                                                              |
|           | **NEW**: Increase cardholder name max length from 50 to 255 characters                                                                                                     |
|           | **NEW**: Add warning banner to dynamic action screen                                                                                                                       |
|           | **IMPROVE**: Migrate headless flows to coroutines                                                                                                                          |
|           | **IMPROVE**: Map brands as objects with per-brand `security_code_length`                                                                                                   |
|           | **FIX**: Fix crash when PSE issuers response is null or empty                                                                                                              |
|           | **FIX**: Support required field card holder name                                                                                                                           |
|           | **FIX**: Add `PAYMENT_CODE` type to dynamic action screen rendering                                                                                                        |
|           | **FIX**: Fix CVV state desync when switching card brands in step-by-step                                                                                                   |
|           | **FIX**: Fix `WebViewActivity` toolbar overlapping status bar on Android 15+                                                                                               |
|           | **FIX**: Fix enrolled card skipping form when `billingAddress` or `cardholderName` are required                                                                           |
|           | **FIX**: Fix continue redirect enrollment custom tab                                                                                                                       |
|           | **FIX**: Fix C2P OTT flow calling `continuePayment`                                                                                                                        |
|           | **FIX**: Fix empty form when all `FieldsRequired` are false                                                                                                                |
| 2.11.2    | **CHANGE**: Support hiding the cardholder name based on backend response                                                                                                   |
| 2.11.1    | **NEW**: Added a disclaimer to the Punto Pago kiosk screen                                                                                                                 |
|           | **IMPROVE**: Updated the UI for the Cartes Bancaires option                                                                                                                |
