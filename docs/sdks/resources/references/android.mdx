...
| `callbackPaymentState`| function | No       | Callback invoked when the payment state changes. Receives the current state (`SUCCEEDED`, `FAIL`, `PROCESSING`, `REJECT`, etc.), a sub-status string, and an optional `StatusMessage?` with error details. Use for UI updates, analytics, or navigation. Available from SDK `2.22.0` with three parameters.                                                                                                  |
...
| `callbackEnrollmentState`| function | No       | Callback invoked when enrollment state changes. Requires registering `initEnrollment` in your Activity's `onCreate` (or equivalent). This lets the SDK deliver results. Optional when using `onActivityResult` with `requestCode`.                                               |
...
### `StatusMessage` (available since SDK 2.22.0)

Detailed error information returned alongside payment failures or errors. The third parameter in the payment callback.
...
| --------- | --------------------- | -------------------------------------------------------------------------------- |
| `source`  | `String?`             | Originating component or service that produced the status (e.g. `PROVIDER`).     |
| `code`    | `String?`             | Machine-readable error code.                                                     |
| `reason`  | `String?`             | Human-readable explanation of the error.                                         |
| `raw`     | `String?`             | Raw response from the source as a string.                                        |
| `context` | `Map<String, Any?>?`  | Additional metadata about the error or payment state.                            |
