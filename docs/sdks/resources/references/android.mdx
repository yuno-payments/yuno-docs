...
| `callbackPaymentState`| function | No       | Callback invoked when the payment state changes. Receives the current state: for example, `SUCCEEDED`, `FAIL`, `PROCESSING`, `REJECT`. Use for UI updates, analytics, or navigation.                                                                                                  |
...
The `keepLoader` parameter allows you to persist the SDK's loading indicator across the entire `startPayment` → `continuePayment` flow. This provides a seamless visual experience without the loader flickering between steps.

By default, the SDK hides its loader after generating the One-Time Token (OTT) and shows it again when `continuePayment` is called. With `keepLoader` enabled, the loader stays visible continuously from the moment the user submits the payment form. It remains visible until the payment is fully processed (or explicitly dismissed).
