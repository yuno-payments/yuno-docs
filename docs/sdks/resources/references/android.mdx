...
| Outcome | `message` |
| --- | --- |
| `SUCCEEDED`, `PROCESSING`, `CANCELED_BY_USER` (payments) / `CANCELED` (enrollment), including when the shopper leaves the flow before a result | Always `null`. |
...
2. **Add the third parameter to every `callbackPaymentState` lambda** (`startCheckout`, `startPayment`, `startPaymentLite`, `startPaymentSeamlessLite`, `continuePayment`, `continueCardPayment`): `{ state, subState -> }` becomes `{ state, subState, message -> }`. Kotlin lambdas must match the arity, so the old form fails to compile.
3. **Add the second parameter to every `callbackEnrollmentState` lambda** (`initEnrollment`, `startEnrollment`, `enrollmentStatus`, `setupEnrollmentInfo`): `{ state -> }` becomes `{ state, message -> }`.
