...
<Warning>
**Secure Fields:** set the `checkoutSession` on the `secureFields({ ... })` object, **not** only on the `generateTokenWithInformation({ ... })` call. Retry is armed when the SDK initializes the secure fields with a `checkoutSession`; passing it only to `generateTokenWithInformation` leaves retry disabled.

```javascript
// ✅ Correct — checkoutSession set on secureFields, so retry is armed
const secureFields = await yuno.secureFields({
  checkoutSession,
  // ...other options
});

const token = await secureFields.generateTokenWithInformation({ /* ... */ });

// ❌ Incorrect — checkoutSession only on generateTokenWithInformation; retry NOT armed
const secureFields = await yuno.secureFields({ /* no checkoutSession */ });
