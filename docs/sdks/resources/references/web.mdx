...
For these three flows, after your backend creates the payment you must inspect the payment response and, when the payment method is a card and the payment did not go through, resume the SDK so it runs the retry:
...
if (isCard && failed) {
  // Runs the retry flow: re-opens/keeps the card form with the decline shown.
  await yuno.continuePayment();
} else {
  // Normal completion (e.g. show the result screen).
