...
| `locale`       | `string`                                                                     | PayPal decides from the shopper's browser | Button language, in PayPal's `language_COUNTRY` format (for example `en_US`, `es_ES`, `pt_BR`). See PayPal's [supported locales](https://developer.paypal.com/sdk/js/configuration/#locale). |
...
On a retried attempt the SDK does **not** fire `yunoPaymentResult` and does **not** mount the status page. Instead, it delivers the cloned checkout session to your `onLoading` callback:
...
onLoading: ({ type, data }) => {
  if (type === 'PAYMENT_RETRY') {
    ...
    // The SDK will NOT fire yunoPaymentResult for this retry step.
    // The shopper stays on the card form — no additional action needed.
  }
}
```

Each retry attempt flows through `yunoCreatePayment` → your backend → `continuePayment` (or automatic in Seamless) without a `yunoPaymentResult` callback between attempts. `yunoPaymentResult` fires only when the flow finishes — either the payment succeeds, the shopper exits, or the payment fails without being retryable.
