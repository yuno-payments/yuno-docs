...
| `onLoading`                       | function | No       | Callback invoked when loading state changes (for example, SDK fetching config, processing payment). Use to show or hide your own loader.                                                                          |
...
Retry is enabled by the Checkout Builder / Dashboard setting `settings.card.enable_payment_retry` under [`styling.settings`](/reference/checkout-builder/publish-checkout-configuration). It is a server-controlled setting, not a `startCheckout()` argument. It applies to non-enrolled card payments only. Enrolled/vaulted cards are excluded.
...
`updateCheckoutSession(checkoutSession)` replaces the active checkout session without unmounting the SDK. Use it to let the shopper switch to a different payment method after a failed payment, or to refresh the checkout totals without restarting the flow.
...
The SDK keeps the current UI mounted and re-fetches the checkout configuration. The payment-method list, however, is fetched once at mount and is **not** refreshed by `updateCheckoutSession`. If the new session exposes a different set of payment methods, you must unmount and re-mount the SDK.

<Note>
This method is also available as `updateCheckoutSession` on the [Secure Fields](/docs/sdks/customization/secure-fields/index) instance.
</Note>
