...
Add these tags as the first elements in `<head>`, before your app bundle:
...
| `loadScript(...)` | Preload `href` |
| :--- | :--- |
| `loadScript()` | `https://sdk-web.y.uno/v1.11/main.js` |
| `loadScript({ region: 'eu' })` | `https://sdk-web.prod.eu.y.uno/v1.11/main.js`. Also preconnect to `https://sdk-web-card.prod.eu.y.uno` instead of `https://sdk-web-card.prod.y.uno` |
| `loadScript({ env: 'sandbox' })` | `https://sdk-web.sandbox.y.uno/v1.11/main.js` |
| `loadScript({ sri: true })` | The full-version URL your package pins, for example `https://prod.y.uno/sdk-static-bundles-ms/sdk-web/v1.11.1/main.js`, with the same `integrity` value and `crossorigin="anonymous"` |
...
* **Keep `crossorigin` on the API preconnect.** The SDK calls the API with CORS requests that carry no credentials. A preconnect without `crossorigin` opens a connection those requests cannot use.
...
* **Without SRI**, the URL contains only the major and minor SDK version (`v1.11`). Patch releases need no change. Update it when you move to a new major version of `@yuno-payments/sdk-web`, because each major version loads a new SDK minor version (9.x loads `v1.11`, 10.x loads `v1.12`).
...
<Note>
When you upgrade `@yuno-payments/sdk-web`, update the preload `href` (and `integrity` when you use SRI) to the URL the new version loads.
</Note>
