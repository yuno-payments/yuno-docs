...
| `savePaymentMethodEnabled` | boolean | No | When `true`, shows a **Save payment method for future payments** checkbox on direct-debit payment methods (ACH, SEPA Direct Debit, Bacs, PAD, iDEAL) that support saving. The checkbox renders only when the method's required fields include `save_on_success` and the session has a customer; when the shopper ticks it, the one-time token carries `vault_on_success: true` (the key is omitted when unticked). Default `false`. Card keeps its own `card.cardSaveEnable`. > **Available since Web SDK v1.10.12** |
