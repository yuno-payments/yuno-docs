...
Each retry attempt flows through `yunoCreatePayment` → your backend → `continuePayment` (or automatic in Seamless) without a `yunoPaymentResult` callback between attempts. `yunoPaymentResult` fires only when the flow finishes — either the payment succeeds or the shopper exits.
