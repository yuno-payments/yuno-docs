...
| `yunoPaymentResult(status, subStatus)` | function | No  | Callback invoked when the payment flow finishes. Receives the [payment status](/reference/getting-started/response-codes) (e.g. `SUCCEEDED`, `DECLINED`, `PENDING`, `REJECTED`, `CANCELED`, `ERROR`) and an optional `subStatus` qualifying it. Use for UI updates or analytics. See [Shopper-cancelled steps](#shopper-cancelled-steps).  |
...
<Warning>
`yunoPaymentResult` reports the sub-status `CANCELLED_BY_USER` (two `L`s). This is a different value from the `CANCELED_BY_USER` (one `L`) error type delivered through `yunoError` when a shopper closes a provider window, such as the PayPal popup. The two travel on different callbacks and are not interchangeable.
</Warning>
