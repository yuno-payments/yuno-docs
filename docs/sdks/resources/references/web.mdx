...
The `externalButtons` property in `startCheckout`, `startSeamlessCheckout`, and `mountEnrollmentLite` *(SDK 1.6+)* allows merchants to customize the appearance of Google Pay, Apple Pay, and PayPal express buttons. All properties are optional; sensible defaults are used when not provided.
...
<Note>
The `externalButtons` configuration applies to all flows that render express buttons (including `mountCheckout`, `mountExternalButtons`, `mountSeamlessExternalButtons`, and `mountEnrollmentLite`), as long as `startCheckout`, `startSeamlessCheckout`, or `mountEnrollmentLite` was called first with the configuration.
</Note>
