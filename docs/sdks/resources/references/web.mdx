...
* Hash mismatchs cause the script to fail execution with an integrity error (for example, you updated the file but not the hash). To fix, rebuild, retrieve the new sha384 from [versions-sri.json](https://sdk-web.y.uno/versions-sri.json), and update integrity.
...
```html
<head>
  <link rel="preload" as="script" href="https://sdk-web.y.uno/v1.11/main.js">
  <link rel="preconnect" href="https://api.y.uno" crossorigin>
  <link rel="preconnect" href="https://sdk-web-card.prod.y.uno">
  <link rel="dns-prefetch" href="https://sdk.prod.y.uno">
  <link rel="dns-prefetch" href="https://icons.prod.y.uno">
</head>
```
...
| `loadScript(...)` | Preload `href` | Preconnect hosts |
| :--- | :--- | :--- |
| `loadScript()` | `https://sdk-web.y.uno/v1.11/main.js` | `https://api.y.uno` and `https://sdk-web-card.prod.y.uno` |
| `loadScript({ region: 'eu' })` | `https://sdk-web.prod.eu.y.uno/v1.11/main.js` | `https://api.eu.y.uno` and `https://sdk-web-card.prod.eu.y.uno` |
| `loadScript({ env: 'sandbox' })` | `https://sdk-web.sandbox.y.uno/v1.11/main.js` | `https://api-sandbox.y.uno` and `https://sdk-web-card.sandbox.y.uno` |
| `loadScript({ sri: true })` | `https://prod.y.uno/sdk-static-bundles-ms/sdk-web/v1.11.1/main.js`, with the `integrity` value the package uses and `crossorigin="anonymous"` | Same as `loadScript()` |
| `loadScript({ sri: true, env: 'sandbox' })` | `https://sandbox.y.uno/sdk-static-bundles-ms/sdk-web/v1.11.1/main.js`, with the sandbox `integrity` value the package uses and `crossorigin="anonymous"` | Same as `loadScript({ env: 'sandbox' })` |
| `loadScript({ sri: true, region: 'eu' })` | `https://prod.eu.y.uno/sdk-static-bundles-ms/sdk-web/v1.11.1/main.js`, with the same `integrity` value as the US bundle (the file is byte-identical) and `crossorigin="anonymous"` | Same as `loadScript({ region: 'eu' })` |

The API host follows the environment and region of your public API key, so it is the same host your backend calls. The card form host follows the host `main.js` was loaded from.
...
<link
  rel="preload"
  as="script"
  href="https://prod.y.uno/sdk-static-bundles-ms/sdk-web/v1.11.1/main.js"
  integrity="sha384-<same-hash-the-package-uses>"
  crossorigin="anonymous">
```

<Note>
With `sri: true`, the package pins a specific SDK version and its hash. Use that version in the preload, not the `latest` entry in [versions-sri.json](https://sdk-web.y.uno/versions-sri.json), which is usually newer than the version your package loads.
</Note>

To find the exact URL and hash your app requests, open DevTools, go to **Elements**, and inspect the `<script id="sdk-payments-script">` tag that `loadScript()` adds. Copy its `src`, and its `integrity` when present.
...
<Note>
When you upgrade `@yuno-payments/sdk-web`, update the preload `href` (and `integrity` when you use SRI) to the URL the new version loads. The package's [README on npm](https://www.npmjs.com/package/@yuno-payments/sdk-web) states the SDK version it loads. For the hash, use the entry for that version in the `history` list of [versions-sri.json](https://sdk-web.y.uno/versions-sri.json) (sandbox: [sdk-web.sandbox.y.uno/versions-sri.json](https://sdk-web.sandbox.y.uno/versions-sri.json)), or copy both values from the `<script id="sdk-payments-script">` tag in DevTools.
</Note>
...
| `elementSelector`   | string  | No*      | CSS selector for the element where the SDK mounts (for example, `#root`). Deprecated when used alone; prefer `renderMode.elementSelector` object to control APM and action areas independently. *Required when `renderMode.type` is `'element'` and using the object form. |
| `countryCode`       | string  | Yes      | ISO country code where the payment runs (for example, `FR`, `US`). Determines available payment methods and compliance.                                                                                                                                                    |
| `language`          | string  | No       | Language code for the UI (for example, `fr-FR`, `en-US`). Defaults to browser language when available. See [Supported languages](/docs/sdks/resources/languages-supported).                                                                                                                     |
...
| `issuersFormEnable` | boolean | No       | When `true`, shows the issuer (bank) list form. Default `true`. Optional.                                                                                                                                                                                |
| `savePaymentMethodEnabled` | boolean | No | When `true`, shows a "Save payment method for future payments" checkbox on direct-debit payment methods that support saving (the method's required fields include `save_on_success`) when the session has a customer. Default `false`. When the shopper ticks the checkbox, the one-time token carries `vault_on_success: true`; the key is omitted when it's left unticked. Does not cover card; card keeps its own `card.cardSaveEnable`. See [Direct-debit consent](#direct-debit-consent) and [Enrolling payment methods](#enrolling-payment-methods). *Available since Web SDK v1.10.12* |
...
| Parameter                         | Type     | Required | Description                                                                                                                                                                                               |
| --------------------------------- | -------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `createPayment(oneTimeToken)` | function | Yes      | Callback invoked when the user completes the card/APM step. Receive the one-time token; send it to your backend to create the payment via the API; then `await yuno.continuePayment()` to resume the flow. |
| `paymentResult(status, subStatus)` | function | No  | Callback invoked when the payment flow finishes. Receives the [payment status](/reference/payments/status-and-response-codes/payment) (for example, `SUCCEEDED`, `DECLINED`, `PENDING`, `REJECTED`, `CANCELED`, `ERROR`) and an optional `subStatus` qualifying it. Use for UI updates or analytics. See [Shopper-cancelled steps](#shopper-cancelled-steps).  |
| `error(message, data)`        | function | No       | Callback invoked when an error occurs during the flow. Receives an error message and optional data object for debugging.                                                                                  |
| `onLoading`                       | function | No       | Callback invoked when loading state changes (for example, SDK fetching config, processing payment, payment retry cloning a session). Use to show or hide your own loader.                                                                          |
| `onChange`                        | function | No       | Callback invoked when form completeness changes. Receives `{ form: { isValid } }`. Use with `showPayButton: false` to control your own pay button. Covers every required field including secure iframe card fields. > **Available since Web SDK v1.10.11** |

</div>

<Note>
**Callback naming.** These callbacks were renamed to white-label-neutral names in Web SDK **v1.9.0** (see the [release notes](/changelog/web)). The previous `yuno`-prefixed names — `yunoCreatePayment`, `yunoPaymentMethodSelected`, `yunoPaymentResult`, `yunoError`, and `yunoEnrollmentStatus` — still work as aliases, so existing integrations need no changes. They are deprecated and will be removed in a future major version. On SDK versions earlier than v1.9.0, use the `yuno`-prefixed names.
</Note>
...
```javascript
paymentResult('PENDING', 'CANCELLED_BY_USER')
```
...
| `card.texts`                      | object   | No       | Custom text for card form buttons and labels. Keys depend on SDK version (for example, placeholders, button labels).                                                                                                |
...
| `renderMode.elementSelector` | object | No*      | Required if `type` is `'element'`. Provide `apmForm` (selector where APM UI is rendered) and `actionForm` (selector for button/area that opens provider-specific steps, for example PIX QR). |
...
| Parameter | Type   | Required | Description                                                                                                                                                         |
| --------- | ------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `texts`   | object | No       | Custom text for payment form buttons and labels (for example, `customerForm.submitButton`, `paymentOtp.sendOtpButton`). Use to match your app's language or branding. |
...
| `buttonLocale`   | `string`                                                                                               | SDK language     | ISO 639-1 alpha-2 lowercase (for example, `'en'`, `'es'`, `'pt'`)         |
...
| `locale`             | `string`                                                                                                                                                                                      | SDK language | ISO 639-1 alpha-2 lowercase (for example, `'en'`, `'es'`, `'pt'`) |
| `buttonWidth`        | `number`                                                                                                                                                                                      | `100%`       | Wrapper width in px                                        |
| `buttonHeight`       | `number`                                                                                                                                                                                      | `40`         | Wrapper height in px                                       |
...
<Note>Retry in Lite shipped in SDK v1.9.11.</Note>
...
```javascript
// In your createPayment callback, after creating the payment on your backend:
...
if (isCard && failed) {
  ...
  // Additional action needed (for example, 3DS or redirect).
...
Retry is armed when the SDK initializes the secure fields with a `checkoutSession`. On each retry, the SDK swaps in a freshly cloned session internally. If you also pass `checkoutSession` to `generateTokenWithInformation`, that fixed value overrides the cloned one and breaks the retry attempt.
...
onLoading: ({ type, data }) => {
  if (type === 'PAYMENT_RETRY') {
    // A fresh checkout session has been cloned for the retry.
    const newCheckoutSession = data.newCheckoutSession;
    // The SDK will NOT fire paymentResult for this retry step.
    // The shopper stays on the card form — no additional action needed.
  }
}
```

Each retry attempt flows through `createPayment` → your backend → `continuePayment` (or automatic in Seamless) without a `paymentResult` callback between attempts. `paymentResult` fires only when the flow finishes — either the payment succeeds, the shopper exits, or the payment fails without being retryable.

## Direct-debit consent

*Available since Web SDK v1.10.12.*

On direct-debit payment methods (ACH, SEPA Direct Debit, Bacs, PAD, and iDEAL when the account is saved), the checkout renders authorization/mandate text below the bank fields, when enabled via the Checkout Builder's [Authorization text](/docs/using-yuno/dashboard-overview/checkout-builder#authorization-text) panel.
...
```json
{
  "payment_method": {
    "bank_transfer": {
      "mandate": {
        "mandate_template_key": "...",
        "mandate_type": "ONE_OFF",
        "accepted_at": "2026-09-02T15:04:00Z",
        "creditor": "..."
      }
    }
  }
...
| Field                   | Description                                                                                     |
| ------------------------ | ------------------------------------------------------------------------------------------------- |
| `mandate_template_key`   | Identifies the authorization/mandate copy template the shopper was shown.                        |
| `mandate_type`           | `ONE_OFF` or `RECURRENT`, matching the copy variant rendered.                                      |
| `accepted_at`            | UTC timestamp of when the shopper accepted the mandate.                                            |
| `creditor`                | Creditor value as rendered in the authorization text. Omitted when the backend sends no creditor. |
...
```javascript
// After a failed payment, inject a new session so the shopper can pick another method:
await yuno.updateCheckoutSession(newCheckoutSession);
```
...
| `countryCode`          | string   | Yes      | ISO country code (for example, `US`, `BR`). Determines available enrollment options. See [Country coverage](/docs/sdks/resources/country-coverage).                                                                                                                          |
| `language`             | string   | No       | Language code for the UI (for example, `en`, `en-US`). Defaults to browser language when available.                                                                                                                                                     |
...
| `enrollmentStatus` | function | No       | Callback when enrollment ends. Receives `vaultedToken` and `status`. Status values: `CREATED`, `EXPIRED`, `REJECTED`, `READY_TO_ENROLL`, `ENROLL_IN_PROCESS`, `UNENROLL_IN_PROCESS`, `ENROLLED`, `DECLINED`, `CANCELED`, `ERROR`, `UNENROLLED`. |
