...
* **Without SRI**, the URL contains only the major and minor SDK version (`v1.11`). Patch releases need no change. Update it when you move to a new major version of `@yuno-payments/sdk-web`, because each major version loads a new SDK minor version (7.x loads `v1.9`, 8.x loads `v1.10`, 9.x loads `v1.11`).
...
<Note>
When you upgrade `@yuno-payments/sdk-web`, update the preload `href` (and `integrity` when you use SRI) to the URL the new version loads. You can read both from the package's [README on npm](https://www.npmjs.com/package/@yuno-payments/sdk-web) or from the `<script id="sdk-payments-script">` tag in DevTools.
</Note>
