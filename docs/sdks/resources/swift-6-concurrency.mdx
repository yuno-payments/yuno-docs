...
Protocols aren't marked as `@MainActor` because:
...
This approach uses immutable properties that are automatically thread-safe, making it best suited for simple apps with fixed configuration values that don't change during runtime.
...
This approach is best for apps where configuration values might change during runtime, like user preferences. It allows for mutable properties while maintaining thread safety by using `MainActor.assumeIsolated`.
...
This approach is suitable for service classes that don't require `MainActor` isolation. It's best for background services or utility classes that don't interact with the UI.
...
* `viewController`: The delegate protocol marks this as `@MainActor` because it should always be accessed from the main thread. UI components must always run on the main thread to prevent crashes.
