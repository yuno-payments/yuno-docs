...
Register the optional `onInstallmentSelected` callback to keep your cart total in sync with the installment plan the shopper picks in the card form. The SDK notifies you when a default installment is pre-selected and every time the shopper changes the selection. It also notifies you when the plan is recalculated (for example, after a BIN change).
