...
* *Switching payment methods* — call `updateCheckoutSession(newSession)` to inject a fresh checkout session without unmounting the SDK. The shopper can pick a different payment method after a failure. See [updateCheckoutSession](/docs/sdks/resources/references/web#updatecheckoutsession).
