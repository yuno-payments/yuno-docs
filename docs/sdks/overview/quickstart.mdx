...
```javascript
// Inside your createPayment callback:
...
<Note>
On a retried attempt the SDK does **not** fire `paymentResult`. It delivers the cloned checkout session to your `onLoading` callback as `{ type: 'PAYMENT_RETRY', data: { newCheckoutSession } }`. Register `onLoading` in your `startCheckout` configuration to receive it:
