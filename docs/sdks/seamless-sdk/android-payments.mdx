...
| `callbackPaymentState` | Optional callback `(state, subState, message)` with the payment state, its sub-state and, on error outcomes, a [`StatusMessage?`](/docs/sdks/resources/references/android#statusmessage-error-details). Third parameter added in SDK `2.22.0`. |
...
| `callbackPaymentState` | Optional callback `(state, subState, message)`; `message` is a [`StatusMessage?`](/docs/sdks/resources/references/android#statusmessage-error-details) on error outcomes. |
...
Since Android SDK version 2.16.0, the card number, cardholder name, expiration date, and CVV fields are eligible for Google Autofill. When the customer focuses on any of these fields, Android's Autofill UI may suggest a card saved to their Google account, and selecting it fills in the form automatically.
