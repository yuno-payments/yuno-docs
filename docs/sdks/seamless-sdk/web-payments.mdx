...
await yuno.startSeamlessCheckout({
  ...
  async yunoPaymentResult(status, subStatus) {
...
The Seamless SDK creates the payment itself from the checkout session, so there is no `yunoCreatePayment` callback and no Create Payment call from your server. You get the outcome in `yunoPaymentResult`, and your server gets it from the `payment.purchase` webhook. The SDK also runs [payment retry](#payment-retry) for declined cards on its own, so you do not call `continuePayment`.
