...
await yuno.startSeamlessCheckout({
  ...
  async paymentResult(status, subStatus) {
    console.log("Payment result:", status, subStatus);
...
The Seamless SDK creates the payment itself from the checkout session, so there is no `createPayment` callback and no Create Payment call from your server. You get the outcome in `paymentResult`, and your server gets it from the `payment.purchase` webhook. The SDK also runs [payment retry](#payment-retry) for declined cards on its own, so you do not call `continuePayment`.
...
await yuno.startCheckout({
  ...
  paymentResult(data) { /* final payment status */ },
  error: (error) => { /* flow errors (including widget cancellation) */ },
...
When a **card** payment comes back `DECLINED` or `ERROR`, payment retry keeps the card form open so the shopper can correct their details and try again without restarting checkout. This is disabled by default and enabled per organization by Yuno on request. It is a server-side flag, not a Dashboard or Checkout Builder setting.
