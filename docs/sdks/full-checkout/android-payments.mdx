...
| `callbackPaymentState`    | Callback: payment state (e.g. SUCCEEDED, FAIL, PROCESSING, REJECT), sub-status, and optional StatusMessage?. Available from SDK `2.22.0` with three parameters.                          |
...
startCheckout(
  checkoutSession = "checkout_session",
  countryCode = "country_code_iso",
  callbackPaymentState = { state, subState, statusMessage -> /* ... */ },
