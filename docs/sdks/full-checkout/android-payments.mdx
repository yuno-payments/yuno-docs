...
import ProGuardRules from '/snippets/proguard-rules.mdx';
...
* See [Render mode integration](#render-mode-integration) for advanced UI customization.
...
* Create a `checkout_session` and the payment via the API; create a customer using the [Create customer endpoint](/reference/customers/create-customer) before creating a payment
...
| `keepLoader`            | A boolean that specifies whether the loader should be kept visible after the payment is initiated.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
The `startPaymentRender` function allows you to integrate the payment process in a more detailed and customizable manner. This function provides full control over when and how payment forms are displayed, facilitating a smoother integration with your existing application UI.
...
```kotlin
fun startPaymentRender(
    paymentMethodSelected: PaymentChosen,
    callbackPaymentState: (onPaymentState: String?) -> Unit
): PaymentRender?
```
...
| ----------------------- | ----------------------- | ------------------------------------------------------------------------------------------------------- |
| `paymentMethodSelected` | `PaymentChosen`         | The payment method selected by the user. Must include the `vaultedToken` (if it exists) and the `type`. |
| `callbackPaymentState`  | `(String?) -> Unit`     | A callback to receive the final payment state.                                                          |

#### Return value

Returns an instance of `PaymentRender`, which provides methods to handle the payment rendering flow.

#### PaymentRender interface

The `PaymentRender` interface includes the following methods:

#### render(onResult: (View?) -> Unit)

* **Purpose**: Gets the form view to capture payment data.
* **Behavior**:
  * If the payment method requires a form, it provides a `View` with the corresponding form through the `onResult` callback.
  * If no form is needed (e.g., for already configured methods), `onResult` receives `null`.
...
* **When to use**: When the user triggers the "pay" action in your application.

#### continuePayment(onResult: (View?) -> Unit)

* **Purpose**: Continues the payment process after the one-time token is generated.
* **Behavior**:
  * If an additional view (e.g., 3DS authentication) is required, it provides a `View` through the `onResult` callback.
  * If no additional view is required, `onResult` receives `null`.
* **When to use**: After receiving the one-time token and creating the payment on your backend.

#### Implementation flow

**Step 1: Initial setup**

Ensure the SDK is properly initialized and you have a valid `checkoutSession`.
...
```kotlin
val paymentRender = Yuno.startPaymentRender(
    paymentMethodSelected = PaymentChosen(
        type = "CARD",
        vaultedToken = "optional_vaulted_token"
    ),
    callbackPaymentState = { state ->
        // Handle final state (SUCCEEDED, FAIL, etc.)
    }
)
```
...
```kotlin
paymentRender?.render { formView ->
    if (formView != null) {
        // Add formView to your layout
        myContainer.addView(formView)
    } else {
        // No form needed, you can proceed to startPayment() directly
        // or show a confirmation button
    }
}
```

**Step 4: Start payment and handle OTT**

When the user clicks your "Pay" button, call `paymentRender?.startPayment()`. You will receive the token through the `callbackOTT` provided during setup (or through the activity result, depending on your implementation).

```kotlin
// Inside your activity or fragment
override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
    super.onActivityResult(requestCode, resultCode, data)
    Yuno.onActivityResult(requestCode, resultCode, data)
}

// Ensure you handle the OTT to create the payment on your backend
fun onTokenUpdated(ott: String?) {
    if (ott != null) {
        createPaymentOnBackend(ott) { success ->
            if (success) {
                paymentRender?.continuePayment { additionalView ->
                    if (additionalView != null) {
                        // Display 3DS or other additional views
                        myContainer.removeAllViews()
                        myContainer.addView(additionalView)
                    }
                }
            }
        }
...
For styling, themes, form options, and additional configurations (including Click to Pay with Passkey), see [SDK customizations](/docs/sdks/customization/android).
