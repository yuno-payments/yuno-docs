...
* **Minimum SDK Version**: `minSdkVersion` 23 or above (Android 6.0). Required since Yuno Android SDK 2.24.0
* **Java**: Java 8 enabled
...
| `callbackPaymentState`    | Callback `(state, subState, message)`: payment state (e.g. SUCCEEDED, FAIL, PROCESSING, REJECT), its sub-state and, on error outcomes, a [`StatusMessage?`](/docs/sdks/resources/references/android#statusmessage-error-details). Third parameter added in SDK `2.22.0`. |
| `merchantSessionId`       | Optional merchant session identifier.                                                                        |
| `onInstallmentSelected`   | Optional callback fired when the shopper selects an installment option in the card form. See [Installment selection callback](#installment-selection-callback-oninstallmentselected). Available from SDK `2.20.0`. |
...
startCheckout(
  ...
  onInstallmentSelected: ((InstallmentSelected?) -> Unit)? = null
)
```
...
const val PAYMENT_STATE_STATE_CANCELED_BY_USER = "CANCELED_BY_USER"
```
...
| `CANCELED_BY_USER` | The user voluntarily canceled the transaction or abandoned the payment process.                                                 | No.                                                                                                           |

The callback receives three values: the state, its sub-state (the provider's detail, when any) and a `StatusMessage?` that is `null` on `SUCCEEDED`, `PROCESSING` and `CANCELED_BY_USER`, always present on `INTERNAL_ERROR`, and present on `REJECT` / `FAIL` when the backend returned a `response_code`:
...
startCheckout(
  ...
  callbackPaymentState = { state, subState, message ->
    when (state) {
      ...
      "REJECT", "FAIL" -> showDeclined(reason = message?.reason, code = message?.code)
      "INTERNAL_ERROR" -> reportSdkError(message)   // never null here
      else -> { }
    }
  },
)
...
See [StatusMessage (error details)](/docs/sdks/resources/references/android#statusmessage-error-details) for the fields and when each one is populated.

#### Installment selection callback (`onInstallmentSelected`)

Available from SDK `2.20.0`. Register the optional `onInstallmentSelected` callback to keep your cart total in sync with the installment plan the shopper picks in the card form:
...
* Fires with the recalculated default after the plan changes (for example, a BIN change).
...
* Does not fire when the card form renders without installments, and registering no callback keeps the SDK behavior unchanged.
* The callback runs on the main thread. Exceptions thrown inside it are caught by the SDK and never interrupt the payment flow.
...
* **SDK Status**: Returns `CANCELED_BY_USER`
