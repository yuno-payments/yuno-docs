...
For all APMs (Google Pay, Apple Pay, PayPal), `yunoPaymentMethodSelected` triggers when the customer chooses the payment method (before the payment flow begins). Define `yunoPaymentMethodSelected` in `await yuno.startCheckout()` before `await yuno.mountCheckout()`. For PayPal, the payment sheet opens immediately after selection—no extra confirmation click required.
