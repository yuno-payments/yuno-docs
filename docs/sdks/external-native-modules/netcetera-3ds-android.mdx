...
<Note>
  Native 3DS via Netcetera applies to both **payment** and **enrollment** flows (enrollment since Yuno Android SDK 2.22.0).
</Note>
...
This guide covers what is specific to Netcetera. Installation and activation follow the same rules as every other module, see [External Native Modules (Android)](/docs/sdks/external-native-modules/android-overview).

## Requirements

- Android **6.0 (API level 23)** or higher, the minimum of the Yuno Android SDK since 2.22.0.
...
<Tip>
  Once registered, the Yuno Payments SDK automatically routes any 3DS challenge during checkout or enrollment through the Netcetera provider.
</Tip>
