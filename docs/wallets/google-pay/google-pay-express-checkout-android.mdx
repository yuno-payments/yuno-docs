...
<Note>
  `shipping_methods` is shared with [Fast Checkout shipping](/docs/payment-features/fast-checkout-shipping) (Revolut Pay's wallet widget). The field is the same; delivery differs by wallet — here the selection comes back on the one-time token, while Fast Checkout attaches it to the payment asynchronously.
</Note>
