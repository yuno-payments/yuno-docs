...
For recurring charges, vault the instrument on the first payment and charge the resulting `vaulted_token` afterwards. On the CIT, `payment_token` and `stored_credentials` sit together under `detail.wallet`. On the MIT, the instrument is a stored card, so `stored_credentials` moves to `detail.card`. The full `reason` and `usage` matrix is on [Stored credentials](/docs/payment-features/stored-credentials#wallet-originated-tokens-apple-pay-and-google-pay).

CIT, first payment with the Google Pay sheet shown:
...
MIT, later charge on the vaulted instrument, no Google Pay token:
...
`vault_on_success` requires `customer_payer.id`, and the `vaulted_token` is returned only in the CIT payment response, so store it on your side. Do not place `stored_credentials` under `detail.card` on the CIT: it is not validated for a wallet payment and may be dropped, which leaves the saved record without stored-credential flags and without a `network_transaction_id`.
