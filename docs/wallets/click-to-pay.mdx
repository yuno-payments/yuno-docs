...
Every Click to Pay transaction returns an `authentication_method` field in the payment response, indicating exactly how the shopper was authenticated: for example, passkey (FIDO2), 3DS, OTP, SMS or email, federated login, and SPC, among others.

This is useful for fraud prevention, routing decisions, and tracking metrics such as approval rate by authentication method.
...
To read it, call [Get payment](/reference/payments/retrieve-payment-by-id-v2) and look for `authentication_method` in the transaction.
