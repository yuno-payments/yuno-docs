...
1. Return to your Apple Pay connection in the [Yuno Dashboard](https://dashboard.y.uno/auth/login) and go to the **Certificates & domains** step.
...
**Where to find it:** In the Yuno Dashboard, open your Apple Pay connection and go to the **Certificates & domains** step. When you are creating the connection, the step comes right after the credentials; on an existing connection, open it and go to the same step.
...
Domains belong to the Apple Pay connection you register them on, so register them on the connection your routing sends Apple Pay payments to.

1. In the Dashboard, open that Apple Pay connection and go to the **Certificates & domains** step.
2. Open **Association file**, click **Download file**, and host its content on every domain you are about to register (see the warning below).
3. Click **Add domain**.
4. Enter the domains where the Apple Pay button will appear, **one per line** (up to 10 per connection). Each subdomain is registered separately: `example.com`, `www.example.com` and `checkout.example.com` are three domains.

<Warning>
  Before adding a domain, you must host the verification file on it as plain text (do not leave it as a downloadable file), using the following hosting path:

  `/.well-known/apple-developer-merchantid-domain-association`

  Keep the file hosted permanently. Apple re-checks it from time to time, and Apple Pay can stop working on a domain whose file is no longer reachable. A domain can host only one such file, so a domain registered with Yuno's certificate cannot also be verified under your own Apple merchant ID.
</Warning>

5. Click **Add**. Each domain appears in the table with a status of **Verifying** while Yuno completes registration with Apple.
6. When registration succeeds, the status changes to **Active**: the domain is ready to show Apple Pay. If it shows **Failed**, check that the file is reachable on that domain and use **Retry** on the row.

<Note>
  When you are creating a new connection, at least one domain must reach **Active** before you can move past the step, because Apple Pay cannot process a payment until a domain is registered and verified. Editing an existing connection is never blocked, so a connection whose domains fail later can still be corrected.

  With your own certificate there is no domain list in this step: you register your domains directly with Apple, under your own merchant ID.
</Note>
...
2. Go to **Connections**, find **Apple Pay** in the provider catalogue and click **Connect**.
3. Provide a **Name** for the connection, select **Apple Pay** as **Payment method**, and in the **Use certificates** dropdown, select the option that matches your setup:
   ...
   - **Own — paste PEM values** (manual): paste the PEM values you generated in the previous steps.
   - **Own — Yuno generates the CSRs**: select this option, then generate the signing requests and upload the `.cer` files issued by Apple in the **Certificates & domains** step, as described in the [Own certificates with Yuno-generated CSRs](#own-certificates-with-yuno-generated-csrs) section.
4. Click **Next**. The **Certificates & domains** step asks for what the certificate type you chose needs: your domains with Yuno's certificate, your certificate setup with either own-certificate option.
5. Click **Next** again, then complete the two screens that follow: setup costs (optional) and accounts.
6. Click **Save**. Apple Pay will be added to your connections.

<Note>
  The certificate type cannot be changed once the connection exists, because what is attached to one type does not carry over to the other. To switch, create a new connection and point your routing at it.
</Note>
