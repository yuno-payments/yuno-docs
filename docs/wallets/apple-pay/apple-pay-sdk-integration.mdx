...
- **`customer_payer.id`**: Required together with `vault_on_success`. Without it nothing is vaulted, the response carries no `vaulted_token`, and no error is returned
...
- **`type: "CARD"`**: The vaulted instrument is a card record. `"APPLE_PAY"` is also accepted together with a `vaulted_token`
- **`detail.card.stored_credentials.usage: "USED"`**: Indicates this is a subsequent transaction in a recurring series
- **`workflow: "DIRECT"`**: A server-to-server charge; no checkout session and no `payment_token` are needed
