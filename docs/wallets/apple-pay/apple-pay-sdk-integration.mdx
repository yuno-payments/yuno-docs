...
Send `wallet_card_acceptance` when you create the checkout session to limit which cards the customer can select in the Apple Pay payment sheet. Cards that do not match are shown greyed out.
...
```json
{
  ...
  "wallet_card_acceptance": {
    "supported_countries": ["BR"],
    "supported_networks": ["VISA", "MASTERCARD", "ELO"],
    "supported_funding_types": ["CREDIT"]
  }
}
```
...
|-------|--------|-------------|
| `supported_countries` | ISO 3166-1 alpha-2 codes | Country that issued the card. It is not the customer's location. |
| `supported_networks` | Card networks, for example `VISA`, `MASTERCARD`, `AMEX`, `ELO` | Replaces the default list of networks. |
| `supported_funding_types` | `CREDIT`, `DEBIT` | Funding types the customer can select. |

- Every field is optional. A field you do not send keeps the default behaviour.
- Values must be uppercase. An empty list is rejected with `400 INVALID_PARAMETERS`.
- The object can only be set when the session is created. Updating the session does not change it.
- The filters apply only to the Apple Pay payment sheet. Cards entered in the card form are not affected.

<Note>
  The Apple Pay button is still displayed when none of the customer's cards is eligible. The customer sees every card greyed out in the payment sheet.
</Note>

<Note>
  If your account is set to process credit cards only, the `supported_funding_types` sent on the session takes precedence for Apple Pay.
</Note>
...
Older SDK versions apply `supported_networks` only.
...
Merchants can use this object to retrieve card details such as brand, type, issuer, and the `iin` field, which represents the DPAN BIN. You can use this to evaluate BIN-based promotions and discounts, and for reporting.

<Note>
  **Limitation**: the `iin` reflects the default card selected in the payment sheet when the payment is created. If the shopper switches cards in the payment sheet, the `iin` may not match the card used for the final payment. Avoid presenting BIN-based promotions as guaranteed until the payment is confirmed.
</Note>
