...
| `payment_method.type` | Required: `"APPLE_PAY"` | Required: `"APPLE_PAY"` | Required: `"CARD"` (`"APPLE_PAY"` also accepted) |
...
| `payment_method.vault_on_success` | Optional | Required: `true` | Not sent |
...
| `customer_payer` | Optional | Required: `customer_payer.id` | Optional |
...
<Note>
  ...
  * `vault_on_success` needs `customer_payer.id`. Without it nothing is vaulted, the response carries no `vaulted_token`, and no error is returned.
