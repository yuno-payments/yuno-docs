...
1. **Organization keys**
   ...
   * The organization key is what you send as the `X-Organization-Code` header on endpoints that require it (for example, [Create a Report](/reference/reports/create-a-report))
...
<Frame><img src="/images/reference/developers-credentials/image1.png" alt="Yuno dashboard Developers page showing the API keys tab with an Account key table listing account_id and user_id tokens, and an Organization keys section below it" /></Frame>
