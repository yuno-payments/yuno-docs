...
<CardGroup cols={2}>
  ...
  <Card title="Increased processing speed" icon="gauge-high">
...
<Tip>
Payment methods with no route yet don't show up under **Published** — look for the **Not published** tab at the top of the Routing section instead. That's where you'll find every payment method connected to your account that still needs a route.
</Tip>
...
<Frame>![Routing diagram showing a Smart Routing step splitting payments between Adyen and dLocal, connected to a Cybersource post-authorization step with Succeeded, Pending, Declined, and Error/Paused outcomes leading to a Refund/Cancel action](/images/reference/routing/image1.png)</Frame>
...
<Frame><img src="/images/reference/routing/image2.png" alt="List view of the Adyen route for Apple Pay conditions, showing a table with columns for Condition set ID, Condition set name, Conditions, and Primary providers" /></Frame>