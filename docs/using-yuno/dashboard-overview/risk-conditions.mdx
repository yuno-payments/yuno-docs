...
Currently, the Risk conditions section provides these tabs:
...
When a payment matches an entry on a review list that is active on the route it takes:
...
* The payment appears in the **Held payments** tab, oldest first, with a count badge on the tab.
* Approving lets the payment continue along its route. That is not the same as marking it successful: the provider still has to approve it. Declining stops the payment.
* Holds do not expire. The shopper waits until someone decides, so check the tab regularly.

Review lists follow the same precedence as the rest of screening:

* A payment that matches both a review list and a blocklist is declined. The stricter outcome wins.
* A payment that matches an allowlist skips the review, as it skips blocks.

Create a review list from the **Lists** tab, the same way as a blocklist: a name, the parameter to match on, the entries (typed or uploaded as CSV), an optional automatic removal period, and the accounts it applies to. Then add it to a route, or to a protection preset, and publish the route.

<Tip>
  Rules can also send a payment to review. Set the rule outcome to **Review** when you want a velocity or sameness condition, rather than a fixed list, to decide which payments a person looks at.
</Tip>

<Note>
  A payment that screening held **after** the provider already approved it is listed in **Held payments** so you can see it, but it cannot be approved or declined from the dashboard. Only payments held before they reach the provider can be decided there.
</Note>
