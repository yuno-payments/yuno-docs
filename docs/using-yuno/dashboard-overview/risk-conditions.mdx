...
A review list works like a blocklist, but instead of declining the payment Yuno holds it for a decision. Use it for customers, cards, or devices you want a person to look at without losing the sale.

When a payment matches an entry on a review list that is active on the route it takes:

* Yuno holds the payment before it reaches the provider. The payment shows status `PENDING` with sub-status `PENDING_FRAUD_REVIEW`.
* Someone on your team opens the payment in **Payments** and approves or declines it. Approving sends the payment to the provider; declining ends it without a charge.
* The customer is not charged while the payment is held.

Review lists follow the same precedence as rules:
...
From the **Review lists** tab, click **Create review list** and configure it the same way as a blocklist: a name, the parameter to match on, the entries (typed or uploaded as CSV), an optional automatic removal period, and the accounts it applies to. Then add the list to a route from **Routing**, as described below.
