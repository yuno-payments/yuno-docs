...
Create a review list from the **Lists** tab, choosing **Review list** as the type. The setup is the same as a blocklist: a name, the parameter to match on, the entries (typed or uploaded as CSV), an optional automatic removal period, and the accounts it applies to. Then add it to a route, or to a protection preset, and publish the route.
