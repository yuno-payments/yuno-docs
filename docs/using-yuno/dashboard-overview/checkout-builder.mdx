...
Direct-debit payment methods (ACH, SEPA Direct Debit, Bacs, PAD, and iDEAL when the account is saved) require the shopper to see mandate/authorization copy before completing the payment. The **Authorization text** panel controls whether Yuno renders that copy for you or leaves it to your own checkout implementation.

#### Where to find it

1. Open the **Checkout Builder** in the Yuno Dashboard.
2. In **Payment method settings**, click the **three dots (⋯)** on any direct-debit payment method (for example, **BACS Direct Debit**) and select **Set authorization text**.

<Frame><img className="maia-shot" src="/images/reference/checkout-builder/image7.png" alt="Payment method settings three-dots menu open on BACS Direct Debit, showing Edit name and logo, Set Conditions, Set Required fields, Set authorization text, and Copy to other accounts options" /></Frame>

This opens the **Debit authorization text · [Payment method name]** drawer (for example, *Debit authorization text · BACS Direct Debit*).

<Frame><img className="maia-shot" src="/images/reference/checkout-builder/image8.png" alt="Debit authorization text drawer for BACS Direct Debit, showing the Show authorization text (rendered by Yuno) toggle turned on" /></Frame>

<Note>
  This setting is **per payment method**, not global. Repeat the configuration for every direct-debit payment method you enable.
</Note>

#### How to configure

1. In the drawer, set the **Show authorization text (rendered by Yuno)** toggle.
2. Click **Save**.
3. Return to the Checkout Builder and click **Publish settings** for the changes to take effect in the live checkout.

The Builder preview shows the exact authorization copy live as you toggle the setting, so you can review it before publishing.

#### Available configuration

| Setting                                                      | Description                                                                                                        | What the customer sees in the SDK                                                                             |
| ------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| **Show authorization text (rendered by Yuno): ON**  | Yuno renders the scheme's authorization/mandate copy for that payment method.                                              | The checkout displays Yuno's authorization text below the bank fields. See [Direct-debit consent](/docs/sdks/resources/references/web#direct-debit-consent) for the full SDK behavior. |
| **Show authorization text (rendered by Yuno): OFF** | For merchants who render their own compliant authorization instead of Yuno's. The merchant remains responsible for presenting it, and for passing the mandate details to Yuno through the API (`payment_method.detail.bank_transfer.mandate` on [Create Payment](/reference/payments/create-payment)). | No authorization text is rendered by the checkout; the merchant is responsible for displaying compliant mandate copy elsewhere. |

<Note>
  This toggle choice is recorded in the audit trail.

  The toggle also stays **locked**, with an explanatory tooltip, while the payment method's connection setup is incomplete. The authorization text only renders with real business/creditor values — it never renders with placeholder values.
</Note>
