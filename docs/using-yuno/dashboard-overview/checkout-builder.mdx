...
1. Open the **Checkout Builder** in the Yuno Dashboard.
2. In **Payment method settings**, click the **three dots (⋯)** on any direct-debit payment method and select **Set authorization text**.
...
1. In the **Set authorization text** drawer, set the **Show authorization text (rendered by Yuno)** toggle.
...
| **Show authorization text (rendered by Yuno): OFF** | For merchants who render their own compliant mandate text instead of Yuno's.                                                | No authorization text is rendered by the checkout; the merchant is responsible for displaying compliant mandate copy elsewhere. |

<Note>
  The toggle stays **locked**, with an explanatory tooltip, while the payment method's connection setup is incomplete. The authorization text only renders with real business/creditor values — it never renders with placeholder values.
</Note>
