...
Depending on the type of event, you will receive a different type of webhook and event. The next table presents webhook event types. `split_update` requires explicit subscription and is pending production catalog availability; see [Split update notifications](/docs/webhooks#split-update-notifications).
