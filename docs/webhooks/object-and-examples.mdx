...
Sent when an onboarding is permanently terminated through [Terminate Onboarding](/reference/recipients-for-marketplace/terminate-onboarding). The payload uses the same envelope and `data` object as the other `onboarding.*` events shown above, with `type_event` set to `onboarding.terminated` and `data.status` set to `TERMINATED`. Termination is irreversible, so this is the final event for the onboarding.

To receive it, add `TERMINATED` to the webhook's `onboarding_triggers` (see [Webhooks Overview](/reference/webhooks)).
