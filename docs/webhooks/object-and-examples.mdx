...
The payload carries `resume_reason`, which says who ended the pause: `MERCHANT` when you called [Resume Subscription](/reference/subscriptions/resume-subscription), or `SCHEDULED` when the pause reached the `pause_until` it was given and the subscription returned on its own. The field is present only on this event.

Subscription payloads also carry `pause_until` and `refund_rule` when those are set on the subscription — see [the Subscription object](/reference/subscriptions/the-subscription-object).
