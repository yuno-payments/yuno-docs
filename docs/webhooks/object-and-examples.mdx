...
A scheduled resume — the pause reaching its `pause_until` on its own — sends the same event with `"resume_reason": "SCHEDULED"` instead, and no other structural difference in the payload.
