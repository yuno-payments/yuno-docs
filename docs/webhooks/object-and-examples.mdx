...
<ParamField body="data" type="object">
  ...
  <Expandable title="properties">
    <ParamField body="idempotency_key" type="string">
      Identifier used to skip duplicate deliveries. It stays stable across every retry of the same event. On `subscription.*` events it identifies the subscription rather than the event, so de-duplicate those on `type_event` plus this key. For `payment.split_update`, de-duplicate on `split_update.idempotency_key` instead. See [De-duplication](/docs/webhooks#de-duplication) for coverage details.
    </ParamField>
  </Expandable>
</ParamField>
...
These abbreviated examples show only the fields needed to interpret `payment.split_update`; the normal payment fields are also included. This event requires explicit subscription and is pending production catalog availability.
...
{
  "type": "payment",
  "type_event": "payment.split_update",
  "version": 2,
  "retry": 0,
  "data": {
    "split_update": {
      "idempotency_key": "4a518553-a6e7-4717-a415-06084ea85c99",
      "updated_at": "2026-09-22T04:20:00Z"
    },
    "payment": {
      "transactions": {
        "split_marketplace": []
...
This example clears the split, so the operation's `split_marketplace` is `[]`. An applied replacement carries that operation's split entries instead. `transactions` is a single object, not an array.
...
| Field | Meaning |
| --- | --- |
| `split_update.idempotency_key` | The original split update request key. Use it to distinguish operations and de-duplicate delivery, scoped by account and event type. |
| `split_update.updated_at` | The operation's recorded completion time, preserved on repeated resolution and delayed notification. |
| `payment.transactions.split_marketplace` | The selected operation's split snapshot, including `[]` for a clear. |

If `transactions_history` is enabled, its entry for the affected transaction carries the same operation split. The event does not enable history or replace the payment-level `payment.split_marketplace`; do not use that payment-level field as this operation's snapshot. Other fields reflect the payment read when the notification is prepared and may include later changes. Use `GET /v1/payments/{payment_id}` for the latest payment state.
...
Only this event adds `split_update`; purchase and refund payloads do not acquire the block. See [Split update notifications](/docs/webhooks#split-update-notifications) for subscription, emission, ordering, and delivery behavior.
...
<Info>
  When the payment carries a shipping selection — for example, picked inside a wallet widget with [Fast Checkout shipping](/docs/payment-features/fast-checkout-shipping) — the payload includes a top-level `payment.shipping` node (same shape as on [the payment object](/reference/the-payment-object)) in both V1 and V2 versions, and `payment.customer_payer.shipping_address` is filled from it when the payer had none. The `shipping` key is omitted when the payment has no selection.
</Info>
...
Standalone transfer notifications include the saved `direction` at the top level of the delivered JSON body. Existing transfers use `PLATFORM_TO_RECIPIENT`. Recipient-to-platform availability follows the [create endpoint](/reference/transfers/create-standalone-transfer).
...
```json
{
  ...
  "direction": "PLATFORM_TO_RECIPIENT",
  "amount": {
    "currency": "USD",
    "value": 4
  },
