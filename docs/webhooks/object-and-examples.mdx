...
Sent when a subscription is canceled. A canceled subscription can be brought back to `ACTIVE` with [Reactivate Subscription](/reference/subscriptions/reactivate-subscription), which keeps the same subscription and history.
