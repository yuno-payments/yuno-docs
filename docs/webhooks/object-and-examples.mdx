...
Sent when a plan change takes effect: at the renewal that first bills the new plan for a scheduled change, or immediately for `POST /v1/subscriptions/{id}/plan`. `previous_plan_id` identifies the plan the subscription moved away from; the embedded subscription already reflects the new plan.
