...
Sent when a pre-chargeback notice or fraud alert is received for a payment, ahead of a full chargeback. Currently sent to any webhook with at least one `payment_triggers` value, regardless of whether `PRECHARGEBACK` is selected; a dedicated trigger gate is planned (C2-17489).
