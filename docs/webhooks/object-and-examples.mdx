...
<Info>
  `verify.external_subscription` links the enrolled payment method to a subscription in your own system. The key is **omitted** (not sent as `null`) unless the enrollment was created with `verify.vault_on_success` set to `true` and carried an `external_subscription` object, which is why the example above does not show it. Both webhook versions carry the field.

  When present, `verify` looks like this:
