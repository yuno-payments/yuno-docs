...
| `subscription.cancel`           | Sent when a subscription is canceled. A canceled subscription can be brought back to `ACTIVE` with [Reactivate Subscription](/reference/subscriptions/reactivate-subscription). Carries `refund` when a refund quote was computed for the cancellation. |
