...
| `subscription.plan_changed`     | Sent when a plan change takes effect: at the renewal that first bills the new plan, or immediately after `POST /v1/subscriptions/{id}/plan`. |
