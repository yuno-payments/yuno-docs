...
| `subscription.resume`           | Sent when a paused subscription is resumed and returns to `ACTIVE` or `TRIALING`. Carries `resume_reason` — `MERCHANT` when [Resume Subscription](/reference/subscriptions/resume-subscription) was called, `SCHEDULED` when an automatic resume completed on its own — and the resulting `billing_cycles.next_at`. |
| `subscription.reactivate`       | Sent when a `CANCELED` or `COMPLETED` subscription returns to `ACTIVE` or `TRIALING` through [Reactivate Subscription](/reference/subscriptions/reactivate-subscription). |
| `subscription.auto_resume_failed` | Sent when the automatic resume of a paused subscription could not be completed after its retries; the subscription stays `PAUSED`. Available to organizations enabled for it; contact Yuno. |
| `subscription.cancel`           | Sent when a subscription is canceled. A canceled subscription can be brought back with [Reactivate Subscription](/reference/subscriptions/reactivate-subscription). Carries `refund` when a refund quote was computed for the cancellation. |
