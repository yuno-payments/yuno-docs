...
| `subscription.resume`           | Sent when a paused subscription is resumed. Carries the resulting `billing_cycles.next_at`, so a resume that moved the billing anchor reports the new date on the event. |
