...
To enable HMAC signature verification, check **Use HMAC Authentication** when creating or editing a webhook in the [Yuno dashboard](https://dashboard.y.uno/), or set `hmac_client_secret` via the [Webhooks API](/reference/webhooks/create-webhook). Verification works the same way either way.

See [Configure Webhooks](/docs/webhooks/configure-webhooks) for step-by-step instructions on both.
...
Here's what a webhook request looks like with the `x-hmac-signature` header added alongside the other configured authentication headers.
...
Verify the `x-hmac-signature` header using the **raw request body** (before parsing JSON) and the same secret configured for the webhook in the Yuno dashboard. Don't re-serialize the JSON first. A change in key order or whitespace produces a different signature than the one Yuno sent.
...
* [Payloads and Examples](/docs/webhooks/object-and-examples)
