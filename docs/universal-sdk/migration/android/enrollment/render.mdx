...
In the unified API the same pattern is expressed by overriding [`showView`](/docs/universal-sdk/controllers/show-view): the SDK delivers the fragment together with a `TransactionProcess` you call `onSubmit()` on. As with Lite enrollment, you pass a `PaymentMethodSelected` carrying the method type — `start(paymentSelected)` never accepts null.
...
- [Lite enrollment](/docs/universal-sdk/migration/android/enrollment/lite). SDK-driven, no view embedding.
