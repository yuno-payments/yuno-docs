...
| `callbackEnrollmentState: (String?) -> Unit` | `TransactionController.onStatus(status)` | `status` and `substatus` are passed through from the backend. See [`Status`](/docs/universal-sdk/reference/status). |
...
- [Render enrollment](/docs/universal-sdk/migration/android/enrollment/render). Embed the form yourself.
- [Headless enrollment](/docs/universal-sdk/migration/android/enrollment/headless). Collect data in your own UI.
