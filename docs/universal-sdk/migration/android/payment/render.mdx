...
| `YunoPaymentRenderController.returnOneTimeToken(token, additionalData)` | Override [`createPayment`](/docs/universal-sdk/controllers/create-payment) | Only relevant for the non-seamless headless path. See [Headless migration](/docs/universal-sdk/migration/android/payment/headless). |
...
- [ ] Rename `returnStatus(resultCode, status, subStatus)` to `onStatus(status: PaymentStatus)` and switch `"CANCELED_BY_USER"` → `"USER_CANCELLED"`. Drop the `resultCode` int.
...
- [Migration / Lite](/docs/universal-sdk/migration/android/payment/lite). When you don't need to embed the form yourself.
