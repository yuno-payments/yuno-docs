...
| `callbackPaymentState: (state, subState) -> Unit` | `TransactionController.onStatus(status: PaymentStatus)` | See [Result mapping in Lite](/docs/universal-sdk/migration/android/payment/lite#result-mapping) for the per-value rename. |
...
- [ ] Move the result-handling logic from the `callbackPaymentState` lambda into `onStatus(status)`. Switch `"CANCELED_BY_USER"` → `"USER_CANCELLED"`.
...
- [Migration / Lite](/docs/universal-sdk/migration/android/payment/lite). Almost identical migration; Seamless and Lite collapse into the same default path in the unified API.
