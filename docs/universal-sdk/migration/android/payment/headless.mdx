...
| Result returned from `generateToken` (`Map<String, Any?>`) | `OneTimeToken` (still a `Map<String, Any>`) | Cross-platform parity. See [`OneTimeToken` reference](/docs/universal-sdk/reference/one-time-token). |
...
- [`createPayment` / `createPayment`](/docs/universal-sdk/controllers/create-payment). The controller that receives the token.
...
- [Migration / Lite](/docs/universal-sdk/migration/android/payment/lite). For the simpler "SDK shows the form" case.
