...
| `callbackOTT: (String?) -> Unit` | Override [`createPayment`](/docs/universal-sdk/controllers/create-payment) on `TransactionController` (Non-seamless only) | In Lite, this lambda only fired in the headless variant. For the SDK-managed Lite flow, you don't need it. See [Headless migration](/docs/universal-sdk/migration/android/payment/headless). |
...
| `"CANCELED_BY_USER"` | `"USER_CANCELLED"` |
...
- **`CANCELED_BY_USER` was renamed `USER_CANCELLED`.** The only value-level change.
...
- [ ] Replace the `callbackPaymentState` lambda with `TransactionController.onStatus(status)`. Switch `"CANCELED_BY_USER"` → `"USER_CANCELLED"`.
