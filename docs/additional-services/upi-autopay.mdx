...
UPI Autopay covers the full lifecycle of a recurring debit, from the initial mandate-creating charge to notifications, cancellations, retries, and provider support.
...
* **Customer cancellation**: There is no cancellation link or Yuno-mediated opt-out. To stop the mandate, the customer must revoke it directly with their bank or UPI app. Yuno only learns about the revocation when the next debit attempt is declined by the provider.
...
1. **Send the notification** with [Create pre-debit notification](/reference/pre-debit-notifications/create-pre-debit-notification), passing the mandate's `vaulted_token`, the upcoming `amount`, and the `billing_date`. Keep the returned `id` and confirm the status is `SENT`. This status is final. It never changes after creation, and Yuno sends no webhook for it, so there's no need to poll for updates.
...
* **Above threshold (> INR 50,000)**: NPCI requires a customer-initiated payment. Yuno sends no reminder or notification for this case. Building your own CIT collection flow and collecting direct action from the customer is entirely your responsibility.
