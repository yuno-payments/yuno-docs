...
| **Digital wallets** | Apple Pay, Google Pay, Samsung Pay, Click to Pay, Nubank, PicPay, Mercado Pago, and other regional wallets. |
...
*   **Multi-seller (split) orders**: when a cart combines products from different sellers or franchises, VTEX splits it into separate orders and Yuno charges every one of them, not just the first. See [Advanced Features](/docs/plugins/vtex/advanced-features#split-orders-multiple-sellers).
*   **Modified orders (changed totals)**: when an order's total changes after checkout (for example, products sold by weight), Yuno adjusts to the final amount. If it goes **down**, only the final (lower) amount is captured; if it goes **up**, the difference is charged automatically (when enabled). See [Advanced Features](/docs/plugins/vtex/advanced-features#order-modifications-changed-order-totals).
*   **Network tokens for returning shoppers**: when enabled, repeat purchases with the same card are processed with a network token instead of the card number, improving approval rates. See [Advanced Features](/docs/plugins/vtex/advanced-features#network-tokens-repeat-card-purchases).
...
<CardGroup cols={2}>
  ...
  <Card title="Enhanced Wallets" icon="wallet" href="/docs/plugins/vtex/apple-pay-google-pay-enhanced-experience">
    Enable Apple Pay, Google Pay, and Samsung Pay buttons directly in VTEX checkout.
  </Card>
