...
*   [Network tokens for returning shoppers](/docs/plugins/vtex/advanced-features#network-tokens-repeat-card-purchases) work the same way here: the `Enable Network Tokens` and `Create Customer` fields live in the same provider configuration, and every card payment processed through the Connector is eligible, regardless of which flow rendered the checkout.
