...
<Steps>
  <Step title="Configure Yuno as a payment provider">
    ...
    <Accordion title="Field reference">
      ...
      | **Antifraud Providers for Split Orders** | Optional | Comma-separated antifraud providers (`RISKIFIED`, `CYBERSOURCE`, `SIGNIFYD`, `CIELO_CYBERSOURCE_FRAUD`) that receive the shared session for split orders. When left empty, all four providers receive it. If you fill it in, only the providers you list receive it. |
