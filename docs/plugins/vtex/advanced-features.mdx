...
If your business runs **multiple VTEX accounts** — a franchise model, a marketplace with affiliates, or separate storefronts sharing a checkout — the Yuno plugin needs to know which account owns the catalog, orders, and fulfillment state. The **Main Account** fields in the provider configuration tell it where to look.
...
| **Franchise fan-in** — many store accounts (franchisees) sharing a parent account that hosts the Payment App | Required on **every franchise affiliation**. Set Main Account Name to the parent account. |
| **Marketplace affiliate** — a storefront whose orders are paid through a different account's checkout, where the sellers process the payment | Required on the affiliation of the account that **authorizes** the order, not the one that processes the payment. The Main Account data is the Marketplace account. |
| **1-to-1 satellite** — two accounts where one authorizes and the other completes the payment (for example, a B2B storefront with a separate payment account) | Required on the authorizing account's affiliation. The Main Account Name is the payment-creation account. |

### What each field does

**Main Account Name:** the VTEX account name that owns the order data in OMS. The plugin reads catalog, order history, and fulfillment state from this account — not necessarily the account where the Payment App runs.
...
<Warning>
  **All three fields must be set together.** If you set the Main Account Name but leave the App key or App token empty, the plugin treats the affiliation as if no Main Account fields were configured — it reads catalog and order data from the current account, just like a single-account setup. The three fields are either all active or all absent; there is no intermediate state.
</Warning>

### How to obtain the main account credentials

In the main account's **VTEX License Manager**, create an application key with these roles:

*   **OMS** — read-only access to order data.
*   **Catalog** — read-only access to product and SKU information.
*   **Payments Gateway** — read access to payment provider configurations.

The key must be created in the main account, not in the franchise or satellite account. A key created in the wrong account cannot read the data the plugin needs.

### Symptoms of a misconfigured Multi-account

When the Main Account fields are wrong or missing in a multi-account setup, the failure is silent — no error appears in the VTEX dashboard or at checkout. Instead it shows up as missing data downstream:

| Symptom | What is happening | How to confirm |
| :--- | :--- | :--- |
| Orders never marked as fulfilled | The fulfillment webhook reads the order from the wrong account's OMS and finds nothing | Search the plugin logs for `sendFulfillment` steps that return empty results. |
| Wrong trade policy or pricing at checkout | Catalog lookups return data from the current account instead of the main one | Compare the trade policy on the order against the expected one in the main account's OMS. |
| Split orders not detected | The plugin does not see related sub-orders because it queries the wrong OMS | An antifraud session is not shared across sub-orders; each one gets its own. |
| Order lookups return empty in the Yuno dashboard | The order does not exist in the account the plugin is querying | Search for the order ID in both accounts' OMS to confirm which one holds it. |

### How to verify the setup

1.  Place a test order on the store and note the order ID.
2.  In Datadog, search the plugin audit logs (`service:sdk-event-log-audit-ms`) with the order's `application_session`.
3.  Confirm that `organization_name` in the authorization steps matches the **origin account** (the franchise or satellite), and that the `saveYunoKeys` hop targets the main account.
4.  If `saveYunoKeys` targets the origin account itself, or a different account from the expected main one, the Main Account fields are misconfigured — check that all three fields are populated and that the Main Account Name matches the account you intend.

<Note>
  Features that depend on per-account state — card vault for network tokens, order modifications, and corporate buyer data — must be enabled on **every** franchise affiliation individually. Enabling them on the parent account alone is not enough, because each franchise account's VBase holds its own records. See [Network tokens](#network-tokens-repeat-card-purchases), [Order modifications](#order-modifications-changed-order-totals), and [Corporate purchases](#corporate-purchases-brazil) for per-feature setup instructions.
</Note>
