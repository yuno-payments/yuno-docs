...
This works automatically; no extra setup is required. If you want to control which antifraud providers receive the shared session, use the **Antifraud Providers for Split Orders** field in the provider configuration (see the field reference in Step 1). Valid values are `RISKIFIED`, `CYBERSOURCE`, `SIGNIFYD`, and `CIELO_CYBERSOURCE_FRAUD`. When left empty, all four receive the shared session, the same providers the Yuno Payment App reports on the checkout page. If you fill it in, only the providers you list receive it: a provider left out gets no session on the suborders the plugin charges after the first one, and its screening of those payments fails. For example, if you use Cielo CyberSource, include `CIELO_CYBERSOURCE_FRAUD`.
