...
**Main Account Name:** the VTEX account that owns the order data in OMS. The plugin reads orders and order groups from this account, and sends the Yuno credentials to it so that fulfillment events are emitted from the correct OMS. Trade-policy resolution uses the current account's credentials.
...
*   **OMS**: read orders and order groups (including lookup by payment id); cancel denied orders (`POST /api/oms/pvt/orders/{id}/cancel`).
*   **Catalog**: read the seller record (`GET /api/catalog_system/pvt/seller/{sellerId}`).
...
<Accordion title="Verifying the setup (for Yuno TAMs)">
  Take the test order's VTEX order id (it is the `application_session` attribute in the plugin audit logs), then look it up in Datadog (`service:sdk-event-log-audit-ms`):
...
This works automatically; no extra setup is required. If you want to control which antifraud providers receive the shared session, use the **Antifraud Providers for Split Orders** field in the provider configuration (see the field reference in Step 1). Valid values are `RISKIFIED`, `CYBERSOURCE`, `SIGNIFYD`, and `CIELO_CYBERSOURCE_FRAUD`. When left empty, all four providers receive the shared session. If you fill it in, only the providers you list receive it, so make sure the list includes every antifraud provider your store uses. A provider missing from the list receives no session for the second and later suborders, so its screening of those payments can fail and leave them pending. For example, if you use Cielo CyberSource, include `CIELO_CYBERSOURCE_FRAUD`.
