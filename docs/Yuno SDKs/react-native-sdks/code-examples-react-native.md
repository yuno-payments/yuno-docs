...
## Parameters used in these examples

| Parameter | Description |
| --------- | ----------- |
| `apiKey` | Your Yuno public API key. From [Yuno Dashboard](https://dashboard.y.uno) > **Developers** > **Credentials**. |
| `countryCode` | ISO country code for the payment (e.g. `US`). See [Country coverage](country-coverage). |
| `checkoutSession` | Checkout session ID from your backend. Required for `YunoPaymentMethods` and payment flows. |
| `onPaymentMethodSelected` | Callback when the user selects a payment method. Receives an event with `isSelected`. |
| `customerSession` | Customer session ID for enrollment. Create via your backend. |
| `showPaymentStatus` | When `true`, the SDK shows payment result UI. |
