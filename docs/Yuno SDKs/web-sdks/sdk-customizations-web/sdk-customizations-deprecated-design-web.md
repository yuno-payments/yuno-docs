...
>
> This design system is deprecated and will be removed in the future. Please use the [Default Design](sdk-customizations-default-design-web) for all new customizations.
...
Example (dropdown). Add to `card` when calling `yuno.startCheckout`.
...
Example (button). Add to `card` when calling `yuno.startCheckout`.
...
Example (alert). Add to `card` when calling `yuno.startCheckout`.
