...
Payments can be customer-initiated (CIT) or merchant-initiated (MIT). See [Stored credentials](stored-credentials). These steps cover customer-initiated transactions (one-time; no recurrence).
