---
title: Copy of Flutter SDK Lite
deprecated: false
hidden: false
metadata:
  robots: index
...
Before initiating a payment through the SDK, create a customer and checkout session using Yuno's server-side API. The customer represents the person making the payment; the checkout session contains transaction details such as amount, currency, and available payment methods. Create these in your backend to ensure security and prevent client-side tampering.
...
Initialize the Yuno SDK in your Flutter app before any payment or enrollment operations. This configures the SDK with your API key and sets regional, language, and UI preferences for the payment flow.

Call `Yuno.init()` before `runApp` in your `main.dart` file:
...
Set up a payment listener in your widget tree to receive payment events and status updates from the SDK. Place the listener in the widget tree before initiating the payment flow so it can capture the one-time token and payment status.
...
The SDK returns different transaction states through the `YunoStatus` enum:

| Status          | Description                                                                                                                                                                                                                      | Additional action required                                                                                                                            |
| :-------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `succeeded`     | The transaction or payment process was completed successfully.                                                                                                                                                              | No. The payment was processed successfully.                                                                                                           |
...
| `processing`    | The transaction is currently being processed. Typically used when there is a delay, such as waiting for approval from a third-party service or financial institution.                                                            | No. Wait for the final status. For asynchronous payment methods, this may be the final SDK status while the payment completes in the background.      |
| `reject`        | The transaction was rejected. Rejection can occur for various reasons, such as insufficient funds, fraudulent activity, or a request that violates specific rules or policies.                                          | Yes. Inform the user of the rejection with a clear message. If possible, provide the reason and suggest alternative payment methods or actions.       |
| `internalError` | An unexpected error occurred within the system or infrastructure handling the transaction. This state suggests a problem on the server or backend side rather than the user's input or request. | Yes. Requires technical intervention to review logs and identify the issue. Allow the user to retry or provide alternative payment options.           |
| `cancelByUser`  | The user voluntarily canceled or aborted the transaction or payment process. Typically used when the user closes the payment form or explicitly cancels the operation.                                         | No. The user intentionally canceled the payment. You may want to track this for analytics purposes.                                                   |

### Step 4: Initiate payment

Start the Lite payment flow by calling `startPaymentLite` with the selected payment method. With Lite (Flutter), you are responsible for displaying available payment methods and capturing the user's selection. The SDK handles the payment form and processing for the selected method.
...
Some payment methods require additional customer actions to complete the transaction, such as 3D Secure authentication or bank redirects. When the Create Payment API response includes `sdk_action_required: true`, call `continuePayment` to display these additional screens to the user.
...
If you have not already initialized the SDK for payment flows, initialize it now with your API key and configuration preferences. This step can be shared between payment and enrollment flows.

Call `Yuno.init()` before any enrollment operations:
...
Set up an enrollment listener in your widget tree to receive enrollment status updates from the SDK. The listener captures the enrollment result and any errors during the process.
...
Some enrollment flows redirect users to external authentication pages (such as bank portals) and return to your app via deep links. This step is only required if you are enrolling payment methods that use external authentication or verification.
...
For more iOS customization options, see [SDK customizations (iOS)](ios-customizations).
...
Configure styles in your `Application` class during SDK initialization in `MyApp.kt`. For more Android customization options including XML themes and drawable customization, see [SDK customizations (Android)](android-customizations).
