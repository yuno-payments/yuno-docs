---
title: "Build Reports"
description: "Request and download payment, transaction, reconciliation, and settlement reports via the API."
---
...
You can check the detailed description for each one by accessing the [Reports Fields](/reference/reports-fields) documentation. The Transaction Reconciliation and Settlement reports provide information related to reconciliation. To examine all fields available on each report and their description, access the [Reports Fields page](/reference/reports-fields).
...
To initiate the report generation process, request Yuno to create the report using the [Create a Report endpoint](/reference/create-a-report). First, select the desired report type (`type`), which will determine the content of the generated report. Secondly, define the date range you wish to incorporate data into the report by setting the parameters `start_date` and `end_date`.
...
After requesting the report creation, you can use endpoints [Retrieve a Report](/reference/retrieve-a-report) or [List All Reports](/reference/list-all-reports) to check if it is ready for download. When using the [Retrieve a Report endpoint](/reference/retrieve-a-report), you need to provide the report `id`. On the other hand, with the  [List All Reports endpoint](/reference/list-all-reports), you need to inform your `account_id` to receive a list of all existing reports and their status.

When the report has the status `SUCCEEDED`, you can request its download. Access the [Report status page](/reference/report-status) to check all possible report status.

### Step 3: Download your report

Use the [Download a Report](/reference/download-a-report) to receive your report providing its `id`. You will receive the report in a .zip file. However, for Settlement reports, the API will return a .csv file.