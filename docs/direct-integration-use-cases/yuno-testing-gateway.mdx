...
Before following the steps described in this guide, you must complete these **dashboard configurations**. Failure to do so will cause payments to fail with `YUNO_ROUTING_REJECTED`.

<Warning>
  **Crucial Prerequisite**: You must enable the **Yuno Testing Gateway** in your Dashboard **Connections** and create a **Routing rule** for it. Test cards will not work unless Yuno knows how to route the "Card" payment method to the testing provider.
</Warning>

* Access your [API credentials](/docs/developers-credentials) on the Yuno Dashboard, which are composed of:
  * `public-api-key`
  * `private-secret-key`
  * `account_id` (Note: This is the same UUID as the `Account Code` in your credentials).
...
You will always get the OTT from the Yuno SDK on your production application. However, to make it easier for you to test the payment creation process via API, use the session-bound endpoint:

**[Create Session Token](/reference/create-one-time-token-sdk)**  
`POST /v1/checkout/sessions/{checkout_session}/token`

Provide the `checkout_session` received in [Step 2](/docs/yuno-testing-gateway#2-create-a-checkout-session) as a **path parameter**. In the request body, define the payment type as `CARD`.

<Warning>
  Do not use the standalone `POST /v1/tokens` endpoint for the checkout flow. That endpoint is for vaulted tokens and will cause a `TOKEN_NOT_EXISTS` error during payment.
</Warning>

In the response, you will receive the `one-time-token`, which you will use to create the payment.
