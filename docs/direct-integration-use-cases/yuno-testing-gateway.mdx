...
Yuno evaluates card data before the payment description. To reach the chargeback simulation path, use a [SUCCEEDED test card](/docs/direct-integration-use-cases/yuno-testing-gateway#222---card-detail) (or an equivalent SUCCEEDED card prefix) together with the `SUCCEEDED+CHARGEBACK <outcome>` description. If the card data maps to a decline or error, that card result wins and the chargeback simulation does not run.
</Note>
