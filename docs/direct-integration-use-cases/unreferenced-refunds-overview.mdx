...
|---|---|---|---|
| **Referenced refund** | [`POST /v1/payments/{id}/transactions/{transaction_id}/refund`](/reference/refund-payment) API | Yes — both the payment `id` and the `transaction_id` go in the path | Reverse a charge made through Yuno. Standard customer-service reimbursement flow. |
...
A referenced refund reimburses a Yuno-captured payment. You call [`POST /v1/payments/{id}/transactions/{transaction_id}/refund`](/reference/refund-payment) — both the payment `id` and the `transaction_id` you are refunding go in the path — with an optional `amount` object in the body. Leave `amount` empty for a full refund, or specify a `value` and `currency` for a partial refund. The refund is credited back to the **same payment method that was originally charged**.
