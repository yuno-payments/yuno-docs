...
* Initiate checkout
* Display or mount payment method selection (varies by mounting option)
...
7. **Merchant Client** → Send token to → **Merchant Server**
...
The workflow above is the same for all mounting options. The **only difference** is step 5 (how payment methods are displayed):
...
// SDK shows form → User fills → SDK generates token AND creates payment automatically
```

**Payment method display:** You control, payment creation automatic
...
2. **Merchant Client** → Initialize SDK with checkout session + vaulted token
3. **Yuno SDK** → Generate one-time token (using vaulted token)
4. **Merchant Client** → Send token to → **Merchant Server**
5. **Merchant Server** → Create payment → **Yuno Server**
6. **Yuno Server** → Process payment → Complete
