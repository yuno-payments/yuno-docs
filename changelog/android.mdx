...
- <ChangelogBadge type="added" /> **Feedback when Pay is pressed without a payment method**  
  In the Full checkout, if the shopper taps Pay before choosing a payment method, startPayment() no longer opens the checkout screen. When the SDK payment method list is on screen, every payment method in it is outlined in red and a short error message asks the shopper to select a method; both disappear as soon as a method is selected, and no payment status is reported to your callbackPaymentState, whether showPaymentStatus is true or false. When the SDK payment method list is not on screen, callbackPaymentState receives INTERNAL_ERROR with a StatusMessage whose source is sdk and whose code is PAYMENT_METHOD_NOT_SELECTED, and no checkout screen is shown. This INTERNAL_ERROR is not a final payment result: no payment was attempted and the checkout session is still usable, so check the StatusMessage code before treating INTERNAL_ERROR as a failed payment, ask the shopper to choose a payment method and call startPayment() again. Each time the payment method list is shown, it starts without a selection, so Pay never continues with a method chosen before that the shopper can no longer see; a preferred method is still preselected as before. Nothing changes when a method is already selected or preselected.

- <ChangelogBadge type="fixed" /> **Enrolled cards no longer expand into an empty area when installment options cannot be loaded**  
  In the unfolded payment method list, selecting an enrolled card that required installments could show a blank strip under the card header when the installment options service failed or returned no options and no other fields were required. The card now only expands when there is something to show; otherwise it stays as a regular selectable row and the payment continues as usual.

- <ChangelogBadge type="added" /> **Optional Koin antifraud module**  
  Koin device fingerprinting now ships as a separate opt-in module (com.yuno.payments:yuno-sdk-antifraud-koin). Add it to your app and the SDK registers the Koin provider automatically at startup and collects the fingerprint for any payment method whenever your checkout configuration enables Koin; apps that do not use Koin no longer bundle its SDK.

- <ChangelogBadge type="added" /> **Optional ClearSale antifraud module**  
  ClearSale device fingerprinting now ships as a separate opt-in module (com.yuno.payments:yuno-sdk-antifraud-clearsale) built on ClearSale's native Behavior SDK. Add it to your app and the SDK registers the ClearSale provider automatically at startup, collects the fingerprint for any payment method whenever your checkout configuration enables ClearSale, and sends ClearSale's own session id as the device fingerprint; the WebView script is no longer loaded when the module is present.

- <ChangelogBadge type="removed" /> **Legacy ClearSale module (com.yuno.fraud-prevention:clearsale)**  
  The legacy ClearSale module with manual lifecycle hooks (onCreateYunoClearSale, onResumeYunoClearSale, onStopYunoClearSale) is no longer published from this repository.  
  *Migration:* Replace the com.yuno.fraud-prevention:clearsale dependency with com.yuno.payments:yuno-sdk-antifraud-clearsale, remove the onCreateYunoClearSale / onResumeYunoClearSale / onStopYunoClearSale calls from your Activity, and stop passing the ClearSale session id as merchantSessionId; the SDK now collects and sends it automatically.

- <ChangelogBadge type="changed" /> **PIX_PARCELADO handled as a standard redirect APM**  
  PIX_PARCELADO is now handled as a standard redirect APM on Android (form from required_fields → token → redirect), consistent with iOS. Previously the SDK rendered Koin-specific installment UI for this type regardless of provider. Koin's native PIX Parcelado form is removed; Koin PIX Parcelado remains available on Web SDK and API.  
  *Migration:* If your checkout still routes PIX_PARCELADO through Koin, disable it in the Yuno dashboard before upgrading. If you rely on Koin for antifraud device fingerprinting, add the optional com.yuno.payments:yuno-sdk-antifraud-koin module.
...
- <ChangelogBadge type="breaking" /> **Minimum Android version raised to 6.0 (API 23) and Google Play services Wallet updated to 20.0.0**  
  The SDK now requires Android 6.0 (API level 23) or higher and ships with Google Play services Wallet 20.0.0, which powers the new real-time Google Pay shipping callbacks. Devices running Android 5.0 and 5.1 (API 21-22) are no longer supported.  
  *Migration:* Set minSdkVersion to 23 or higher in your app's build.gradle. If your app still declares minSdkVersion 21 or 22, the build fails at the manifest merge step with a clear error naming the Yuno SDK; raising minSdkVersion resolves it. If your app pins com.google.android.gms:play-services-wallet to a version below 20.0.0, remove the pin or align it to 20.0.0 or higher. If you call startCheckout, startPayment, startPaymentLite, startPaymentSeamlessLite or continuePayment passing every argument positionally, adjust the call: the new optional shipping callback parameter is placed before the last callback parameter. Calls using named arguments or a trailing lambda need no changes.

- <ChangelogBadge type="fixed" /> **Device fingerprint missing for new cards entered in the payment method list**  
  When the card form was displayed inline in the payment method list, paying with a new card did not run the anti-fraud device fingerprint collection, so fraud providers such as Cybersource received the payment without device data. The SDK now collects the device fingerprint for new cards in the inline form exactly as it does for the standalone card form.

- <ChangelogBadge type="fixed" /> **Payment status screen no longer shown in Arabic to buyers in other languages**  
  Unified the payment and enrollment status screen copies across all supported locales and completed the translations missing from 17 locales whose status screens previously fell back to Arabic text.

- <ChangelogBadge type="fixed" /> **Instruction step icons stretched and misaligned on wrapping steps**  
  Icons next to each step of the payment instructions were forced into a fixed square and centered against the whole step. Each icon now keeps its own proportions, is scaled down only when it exceeds the row, and sits level with the first line of text on steps that wrap.

- <ChangelogBadge type="fixed" /> **Google Pay asked for email and billing address that were not required**  
  When a Google Pay payment was started with continuePayment(), the Google Pay sheet asked the shopper for an email address and a full billing address even if the payment method's required fields did not request them. The sheet now only asks for the fields marked as required, matching the behavior of the checkout flow.

**Payment methods**

- <ChangelogBadge type="added" /> **Revolut Pay as a native payment method**  
  Shoppers can now pay with Revolut Pay natively inside your app. After you create the payment and call continuePayment(), the SDK hands the shopper to Revolut (the Revolut app when installed, otherwise Revolut's web flow) and brings them back automatically. The final payment status is always confirmed by Yuno: if Revolut's confirmation is still settling when the shopper returns, the SDK reports PROCESSING and the final status arrives through the payment webhook. Opt in by adding the yuno-sdk-revolut-pay module; supported in the Full, Lite, Seamless and Render checkout flows.

**Internal**

- <ChangelogBadge type="added" /> **Public API stability safeguards**  
  Internal safeguards that keep the SDK's public API stable across releases. No change to SDK behavior or to how you integrate.
...
- <ChangelogBadge type="breaking" /> **Structured error message on failed and internal-error statuses**  
  Error and internal-error outcomes now include an optional StatusMessage, so you can tell a backend rejection apart from an SDK failure and read its code and reason. It is always null on success, drop-off and user cancellation. Breaking change: the payment status callback and the render listeners' returnStatus now include a new StatusMessage? parameter.  
  *Migration:* Add the new StatusMessage? parameter to your status callbacks (callbackPaymentState / callbackEnrollmentState) and to your returnStatus overrides, then recompile against 2.22.0. Reading the message is optional.
...
- <ChangelogBadge type="fixed" /> **Native select dropdowns now open inside checkout WebViews**  
  Checkout WebViews now keep the host Activity so native select dropdowns open correctly.
...
- <ChangelogBadge type="added" /> **Card number length is now validated per card scheme**  
...
- <ChangelogBadge type="changed" /> **Saved cards now show a compact summary instead of full card artwork**  
...
- <ChangelogBadge type="changed" /> **Stricter card expiry date validation with auto-formatting**  
...
- <ChangelogBadge type="changed" /> **Stronger document and phone field validation**  
...
- <ChangelogBadge type="fixed" /> **Barcodes on flexible payment actions now render in landscape**  
...
- <ChangelogBadge type="fixed" /> **Click to Pay no longer stalls without a callback URL**  
...
- <ChangelogBadge type="fixed" /> **Google Pay payments now include the customer's language for 3DS**  
...
- <ChangelogBadge type="fixed" /> **Card number field no longer advances too early for longer card numbers**  
...
- <ChangelogBadge type="fixed" /> **Card type toggle no longer shows in the unfolded card form when credit_card_only_processing is enabled**  
  Fixed an issue where the credit/debit card type toggle was displayed for Brazilian cards in the unfolded card form (payment method list) even when the merchant had credit_card_only_processing enabled. The full-screen card form received the flag through its call chain, but the unfolded call site did not propagate it, so the composable fell back to its default and always showed the toggle. The visibility check now also resolves the flag directly from the SDK settings, so every card form honors it regardless of the call site.
...
- <ChangelogBadge type="fixed" /> **Host app no longer fails when a payment return deeplink cannot be opened**  
...
- <ChangelogBadge type="fixed" /> **Merchant callbacks registered before initialize are no longer lost**  
...
- <ChangelogBadge type="added" /> **startPayment and startPaymentLite accept an optional payment-state callback**  
