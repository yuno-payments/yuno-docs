...
- <ChangelogBadge type="breaking" /> **Structured error message on failed and internal-error statuses**  
  Payment and enrollment status callbacks and render listeners carry an optional StatusMessage on error outcomes (source "backend" or "sdk", code, reason, raw, context) to tell a backend rejection apart from an SDK-internal failure. Introduced in 2.22.0.  
  *Migration:* Upgrading from 2.21.x or earlier: implementers of YunoPaymentRenderListener.returnStatus and YunoEnrollmentRenderListener.returnStatus must add the message: StatusMessage? parameter, and callbackPaymentState / callbackEnrollmentState lambdas gain a StatusMessage? argument. Recompile against the new version: a drop-in AAR swap without recompiling a render-listener implementation fails with AbstractMethodError on the first error outcome. Reading the new value is optional. Upgrading from 2.22.0 or 2.23.0 requires no change.
