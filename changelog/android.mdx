...
- <ChangelogBadge type="added" /> **Feedback when Pay is pressed without a payment method**  
  Pay without a selected method now highlights the list instead of opening the checkout; without the list on screen, callbackPaymentState gets INTERNAL_ERROR with code PAYMENT_METHOD_NOT_SELECTED (no payment is attempted).

- <ChangelogBadge type="fixed" /> **Enrolled cards no longer expand into an empty area when installment options cannot be loaded**  
  An enrolled card in the unfolded list no longer expands into a blank strip when installments fail to load.

- <ChangelogBadge type="added" /> **Optional Koin antifraud module**  
  Koin fingerprinting moves to the opt-in module com.yuno.payments:yuno-sdk-antifraud-koin, registered automatically at startup.

- <ChangelogBadge type="added" /> **Optional ClearSale antifraud module**  
  ClearSale fingerprinting moves to the opt-in module com.yuno.payments:yuno-sdk-antifraud-clearsale (native Behavior SDK), registered automatically at startup.
...
- <ChangelogBadge type="removed" /> **Legacy ClearSale module (com.yuno.fraud-prevention:clearsale)**  
  ...
  *Migration:* Switch to com.yuno.payments:yuno-sdk-antifraud-clearsale and drop the onCreate/onResume/onStopYunoClearSale calls and merchantSessionId.

- <ChangelogBadge type="changed" /> **PIX_PARCELADO handled as a standard redirect APM**  
  PIX_PARCELADO is now a standard redirect APM, as on iOS; Koin's native PIX Parcelado form is removed from Android.  
  *Migration:* Disable Koin PIX_PARCELADO in the dashboard before upgrading; for Koin antifraud add yuno-sdk-antifraud-koin.
