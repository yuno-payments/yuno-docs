...
- <ChangelogBadge type="added" /> **Feedback when Pay is pressed without a payment method**  
  Tapping Pay without a payment method no longer opens the checkout: the on-screen list highlights the methods and asks the shopper to pick one, and no status is reported. When the SDK list is not on screen, callbackPaymentState receives INTERNAL_ERROR with StatusMessage code PAYMENT_METHOD_NOT_SELECTED; no payment was attempted, so ask the shopper to select a method and call startPayment() again.

- <ChangelogBadge type="fixed" /> **Enrolled cards no longer expand into an empty area when installment options cannot be loaded**  
  In the unfolded list, an enrolled card no longer expands into a blank strip when installment options fail to load or come back empty. It stays a regular selectable row and the payment continues as usual.

- <ChangelogBadge type="added" /> **Optional Koin antifraud module**  
  Koin device fingerprinting now ships as the opt-in module com.yuno.payments:yuno-sdk-antifraud-koin, registered automatically at app startup and used whenever your checkout configuration enables Koin. Apps that do not add it no longer bundle the Koin SDK.

- <ChangelogBadge type="added" /> **Optional ClearSale antifraud module**  
  ClearSale device fingerprinting now ships as the opt-in module com.yuno.payments:yuno-sdk-antifraud-clearsale, built on ClearSale's native Behavior SDK and registered automatically at app startup. When present, the SDK sends ClearSale's session id as the device fingerprint and no longer loads the WebView script.

- <ChangelogBadge type="removed" /> **Legacy ClearSale module (com.yuno.fraud-prevention:clearsale)**  
  The legacy ClearSale module with manual lifecycle hooks is no longer published.  
  *Migration:* Replace com.yuno.fraud-prevention:clearsale with com.yuno.payments:yuno-sdk-antifraud-clearsale, remove the onCreate/onResume/onStopYunoClearSale calls and stop passing the ClearSale session id as merchantSessionId.

- <ChangelogBadge type="changed" /> **PIX_PARCELADO handled as a standard redirect APM**  
  PIX_PARCELADO is now a standard redirect APM (form from required_fields, token, redirect), consistent with iOS. Koin's native PIX Parcelado form is removed from Android; it remains available on the Web SDK and API.  
  *Migration:* If your checkout routes PIX_PARCELADO through Koin, disable it in the Yuno dashboard before upgrading; for Koin antifraud fingerprinting, add com.yuno.payments:yuno-sdk-antifraud-koin.
