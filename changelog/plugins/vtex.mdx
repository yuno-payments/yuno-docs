...
- <ChangelogBadge type="fixed" /> **Samsung Pay express payments never reached authorization**  
  Samsung Pay payments started from the express checkout buttons were rejected before the store order was placed: the shopper authorized the payment on the Samsung Pay sheet, but no order was ever created. Samsung Pay now completes through the same flow as Apple Pay, Google Pay and Click to Pay, and the connector accepts the different spellings VTEX uses to register the payment method.

- <ChangelogBadge type="fixed" /> **Digital wallet payments could fail for guest shoppers due to masked buyer data**  
  For shoppers who were not logged in, VTEX masks profile data in the browser, and wallet payments (Apple Pay, Google Pay, Samsung Pay, Click to Pay) could be sent to Yuno with a masked document number and get rejected. The connector now builds the payer from the order's unmasked buyer information, so guest wallet payments go through with the correct identity.

- <ChangelogBadge type="fixed" /> **Payments could be rejected when the sales policy had no name**  
  When the VTEX trade policy associated with an order had no name configured, the payment could be rejected by Yuno. Orders from nameless trade policies are now processed normally.
...
- <ChangelogBadge type="added" /> **Corporate buyer identity for Brazilian company purchases**  
  New "Use Corporate Buyer Data" affiliation setting (default: No). When enabled and the shopper checks out as a company in Brazil, cards, boleto, PIX and the other checkout methods are issued against the company name and CNPJ that VTEX sends instead of the shopper's personal name and CPF, as Brazilian rules require for corporate purchases. Enable it on the affiliation and no storefront change is needed. Digital wallets whose payment is created before the order is placed (Apple Pay, Google Pay, Click to Pay via preflight) keep the identity collected by the storefront.  
  *Migration:* In VTEX Admin go to Payments > Settings > Affiliations, open the Yuno affiliation, set "Use Corporate Buyer Data" to Yes and save. The field only appears after the store is on version 4.2.350 or later.
...
- <ChangelogBadge type="added" /> **Marketplace seller and affiliate identity in payment metadata**  
...
- <ChangelogBadge type="fixed" /> **Recurring subscription charges are now linked to the original authorization**  
...
- <ChangelogBadge type="fixed" /> **Subscription cards are now saved for recurring charges**  
