...
- <ChangelogBadge type="added" /> **Two-digit expiration year for PCI Proxy expressions**  
  Two new `vaulted_token` expression fields are available on `POST /v1/pci-proxy/forward`: `expiration_year_2_digits` and `network_token_expiration_year_2_digits`, which resolve the card (or network-token) expiration year in the two-digit `YY` format (for example `30` for 2030), for destinations that require it. The existing four-digit `expiration_year` / `network_token_expiration_year` fields are unchanged.
  *See also: [Invoke Forward Proxy](/reference/pci-proxy/invoke-forward-proxy)*
