...
- <ChangelogBadge type="added" /> **CIT/MIT initiation on reported transactions**  
  `POST /v1/reporting/transactions` accepts a new optional `initiation` on each event — `CIT` (cardholder-initiated) or `MIT` (merchant-initiated). Yuno persists it on the reported payment and uses it to build initiation-level statistics. Events without it behave exactly as before; any other value is rejected per event with `INVALID_EVENT` and the message "The field 'initiation' must be CIT or MIT."
  *See also: [Report Off-Yuno Transactions](/reference/reporting/report-transactions)*

- <ChangelogBadge type="changed" /> **Initiation-level routing recommendations**  
  `POST /v1/routing/recommendations` now ranks candidates on the same-initiation segment when `payment.initiation` matches reported history — at each level it tries the initiation-specific history first and falls back to the whole level otherwise, so a request without `payment.initiation` returns the same ranking as before.
