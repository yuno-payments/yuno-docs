...
- <ChangelogBadge type="added" /> **Terminate Onboarding endpoint**  
  A new `POST /v1/recipients/{recipient_id}/onboardings/{onboarding_id}/terminate` endpoint permanently terminates a recipient's onboarding and closes the account at the provider. It requires a `reason` (`fraud`, `terms_of_service`, or `other`), is allowed from the `SUCCEEDED` and `BLOCKED` statuses, and is irreversible: the onboarding moves to the new `TERMINATED` status and the recipient is excluded from split resolution.
