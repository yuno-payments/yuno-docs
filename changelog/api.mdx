...
- <ChangelogBadge type="added" /> **Institution and transit numbers for Canadian pre-authorized debit**  
  `payment_method.detail.bank_transfer` accepts two new optional fields for Canadian pre-authorized debit (`PADS`): `institution_number` (exactly 3 digits) and `transit_number` (exactly 5 digits), both sent as strings so leading zeros are kept. They are available on `POST /v1/payments` and on `POST /v1/customers/{customer_id}/payment-methods`, and `GET /v1/payments/{payment_id}` and `GET /v1/payments` (by `merchant_order_id`) return them under `bank_transfer` when they were provided. `routing_number` is still accepted as an alternative, as 9 digits (`0` + institution number + transit number, for example `000211000`) or 8 digits (transit number + institution number); when both new fields are sent, they take precedence over `routing_number`.
  *See also: [Create Payment](/reference/payments/create-payment) · [Enroll Payment Method](/reference/payment-methods-direct-workflow/enroll-payment-method-api) · [Retrieve Payment by ID](/reference/payments/retrieve-payment-by-id-v2) · [The Payment Object](/reference/payments/the-payment-object)*

- <ChangelogBadge type="changed" /> **Validation of institution and transit numbers on bank transfers**  
  On `POST /v1/payments` and `POST /v1/customers/{customer_id}/payment-methods`, `payment_method.detail.bank_transfer.institution_number` and `transit_number` are validated when present: an explicit empty string, a value of the wrong length (other than 3 and 5 characters respectively), or a non-numeric value returns `400`, the same as other invalid `bank_transfer` fields such as `routing_number`. Requests that omit both fields are unaffected. If you do not have the values, omit the fields or send `routing_number` instead.
  *See also: [Create Payment](/reference/payments/create-payment) · [Enroll Payment Method](/reference/payment-methods-direct-workflow/enroll-payment-method-api)*
...
- <ChangelogBadge type="added" /> **Cryptogram-only mode for the Network Token Cryptogram API**  
  A new optional `include_network_token` field (default `true`) is available on `POST /v1/network-tokens/cryptograms`. With `false`, the response omits `network_token.number` (DPAN) and `network_token.token_data` while keeping the non-PAN-equivalent metadata, so merchants without PCI certification can run network-token payments by pairing the cryptogram with the PCI Proxy, which resolves the DPAN at forward time.
  *See also: [Generate Network Token Cryptogram](/reference/network-tokens/generate-network-token-cryptogram)*

- <ChangelogBadge type="changed" /> **Explicit error for non-PCI organizations requesting the network token**  
  On `POST /v1/network-tokens/cryptograms`, an organization without PCI certification that requests the full network token (`include_network_token` `true` or omitted) now receives `403 NETWORK_TOKEN_RETURN_NOT_ALLOWED` with guidance to use the cryptogram-only mode, instead of the generic `PCI_MERCHANT_REQUIRED`.
  *See also: [Generate Network Token Cryptogram](/reference/network-tokens/generate-network-token-cryptogram)*

**Payments**

- <ChangelogBadge type="added" /> **Card BIN on reported transactions**  
  `POST /v1/reporting/transactions` accepts a new optional `payment_method_details.card.bin` on `CARD` events — the card's BIN, exactly 6 or 8 digits. Yuno stores it on the reported payment and uses it to build BIN-level statistics. Events without it behave exactly as before; a value that is not 6 or 8 digits is rejected per event with `INVALID_EVENT`.
  *See also: [Report Off-Yuno Transactions](/reference/reporting/report-transactions)*

- <ChangelogBadge type="changed" /> **BIN-level routing recommendations**  
  `POST /v1/routing/recommendations` now ranks candidates on the BIN segment when `payment.payment_method.details.card.bin` matches reported history — 8-digit BIN first, then 6-digit — and falls back to the country and payment-method segments otherwise, so a BIN without history returns the same ranking as before.
  *See also: [Get a Routing Recommendation](/reference/organizations/routing/get-routing-recommendations)*
...
- <ChangelogBadge type="changed" /> **Transactions history becomes opt-in**  
  The `transactions_history` array in the payment object is now populated only when the payment is created with `response_additional_data.transactions_history: true`. When the flag is not sent, payment webhooks and responses deliver `transactions_history` as an empty array `[]` to keep payload sizes small. Chargeback events keep including the full history automatically.
  *See also: [Payment object reference](/reference/the-payment-object) · [Webhook object and examples](/docs/webhooks/object-and-examples)*
