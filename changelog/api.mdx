...
- <ChangelogBadge type="added" /> **Terminate Onboarding endpoint**  
  A new `POST /v1/recipients/{recipient_id}/onboardings/{onboarding_id}/terminate` endpoint permanently terminates a recipient's onboarding and closes or rejects the account at the provider (Adyen: permanent close; Stripe: platform reject). It requires a `reason` (`fraud`, `terms_of_service`, or `other`), is allowed from the `SUCCEEDED` and `BLOCKED` statuses, and is irreversible through the Yuno API: the onboarding moves to the new `TERMINATED` status and the recipient is excluded from split resolution.
  *See also: [Terminate Onboarding](/reference/recipients-for-marketplace/terminate-onboarding)*

- <ChangelogBadge type="changed" /> **Provider-level suspension on Block/Unblock Onboarding**  
  When provider-level suspension is enabled for the connection, blocking an onboarding now also suspends the account at the provider, and unblocking reactivates it. If the provider declines the reactivation, the unblock returns an error and the onboarding stays `BLOCKED`. Without the setting, block and unblock behave as before.
  *See also: [Block Onboarding](/reference/recipients-for-marketplace/block-recipient)*

**Webhooks**

- <ChangelogBadge type="added" /> **`onboarding.terminated` event**  
  A new `onboarding.terminated` webhook event is sent when an onboarding is permanently terminated. It uses the same envelope as the other `onboarding.*` events, and is opt-in: add `TERMINATED` to the webhook's `onboarding_triggers` to receive it.
  *See also: [Webhooks Examples](/docs/webhooks/object-and-examples#onboardings)*
